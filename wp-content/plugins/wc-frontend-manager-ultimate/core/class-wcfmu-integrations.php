...
class WCFMu_Integrations {
    ...
    public function __construct() {
        ...
        if ($wcfm_allow_rental = apply_filters('wcfm_is_allow_rental', true)) {
            if (WCFMu_Dependencies::wcfm_wc_rental_pro_active_check()) {
                ...
                // Disable add product popup
                add_filter('wcfm_blocked_product_popup_views', [&$this, 'disable_add_product_popup_in_rnb_inventory']);

                add_action( 'wp_ajax_delete_wcfm_rnb_inventory', [&$this, 'delete_wcfm_rnb_inventory'] );
            } else {
        ...
        if ($wcfm_allow_acf_fields = apply_filters('wcfm_is_allow_acf_fields', true)) {
            ...
            add_filter('wcfm_is_allowed_acf_field_group', [&$this, 'check_acf_field_group_access'], 10, 2);
        } //end if
        ...
        if ($wcfm_allow_rental = apply_filters('wcfm_is_allow_rental', true)) {
            if (WCFMu_Dependencies::wcfm_wc_rental_pro_active_check()) {
                $query_rental_vars = [
                    ...
                    'wcfm-rnb-inventory'        => !empty( $wcfm_modified_endpoints['wcfm-rnb-inventory'] ) ? $wcfm_modified_endpoints['wcfm-rnb-inventory'] : 'rnb-inventory',
                    'wcfm-rnb-inventory-manage' => !empty( $wcfm_modified_endpoints['wcfm-rnb-inventory-manage'] ) ? $wcfm_modified_endpoints['wcfm-rnb-inventory-manage'] : 'rnb-inventory-manage',
                ];
    ...
    function wcfmu_thirdparty_endpoint_title($title, $endpoint) {
        switch ($endpoint) {
            ...
            case 'wcfm-rnb-inventory':
                $title = __( 'RnB Inventory', 'wc-frontend-manager-ultimate' );
                break;
        ...
        if ($wcfm_allow_rental = apply_filters('wcfm_is_allow_rental', true)) {
            if (WCFMu_Dependencies::wcfm_wc_rental_pro_active_check()) {
                $menus = (array_slice($menus, 0, 3, true) + [
                    ...
                    'wcfm-rnb-inventory' => [
                        'label'                 => __('Inventory', 'wc-frontend-manager-ultimate'),
                        'url'                   => get_wcfm_rnb_inventory_url(),
                        'icon'                  => 'warehouse',
                        'has_new'               => 'yes',
                        'new_class'             => 'wcfm_sub_menu_items_rnb_inventory_manage',
                        'new_url'               => get_wcfm_edit_rnb_inventory_url(),
                        'capability'            => 'wcfm_rnb_inventory_menu',
                        'submenu_capability'    => 'wcfm_add_new_rnb_inventory_sub_menu',
                        'priority'              => 31,
                    ],
                    'wcfm-rental-quote'    => [
    ...
    public function wcfmu_thirdparty_load_scripts($end_point) {
        ...
        switch ($end_point) {
            ...
            case 'wcfm-rnb-inventory':
                ...
                $WCFM->library->load_datatable_lib();
                wp_enqueue_script('wcfm_wcrental_inventory_js', $WCFMu->library->js_lib_url . 'integrations/rental/wcfmu-script-wc-rental-inventory.js', array('jquery'), $WCFMu->version, true);

                wp_localize_script( 'wcfm_wcrental_inventory_js', 'rnb_inventory', [
                    'l10n' => [
                        'invetory_delete_confirm' => __( "Are you sure and want to delete this Inventory?\nYou can\'t undo this action ...", 'wc-frontend-manager-ultimate' ),
                    ]
                ] );

                break;
                
            case 'wcfm-rnb-inventory-manage':
                ...
                wp_enqueue_script( 'wcfm_wcrental_inventory_manage_js', $WCFMu->library->js_lib_url . 'integrations/rental/wcfmu-script-wc-rental-inventory-manage.js', array('jquery'), $WCFMu->version, true );
    
                wp_localize_script( 'wcfm_wcrental_inventory_manage_js', 'rnb_inventory_manage', [
                    'l10n' => [
                        'no_title' => __('Please insert Inventory Title before submit.', 'wc-frontend-manager-ultimate'),
                    ]
                ] );
    ...
    public function wcfmu_thirdparty_load_styles($end_point) {
        ...
        switch ($end_point) {
            ...
            case 'wcfm-rental-calendar':
                wp_enqueue_style('wcfmu_rental_calendar_css', $WCFMu->library->css_lib_url . 'integrations/rental/wcfmu-style-wcrental-calendar.css', [], $WCFMu->version);
                break;
            ...
            case 'wcfm-rnb-inventory':
                wp_enqueue_style( 'wcfm_wcrental_inventory_css', $WCFMu->library->css_lib_url . 'integrations/rental/wcfmu-style-wc-rental-inventory.css', array(), $WCFMu->version );
                break;

            case 'wcfm-rnb-inventory-manage':
                wp_enqueue_style( 'wcfm_wcrental_inventory_manage_css', $WCFMu->library->css_lib_url . 'integrations/rental/wcfmu-style-wc-rental-inventory-manage.css', array(), $WCFMu->version );
                break;
    ...
    public function wcfmu_thirdparty_load_views($end_point) {
        ...
        switch ($end_point) {
            ...
            case 'wcfm-rnb-inventory':
                $WCFMu->template->get_template('integrations/rental/wcfmu-view-inventory.php');
                break;
    
            case 'wcfm-rnb-inventory-manage':
                $WCFMu->template->get_template('integrations/rental/wcfmu-view-inventory-manage.php');
                break;
    ...
    public function wcfmu_thirdparty_ajax_controller() {
        ...
        if (isset($_POST['controller'])) {
            ...
            switch ($controller) {
                ...
                case 'wcfm-rnb-inventory':
                    include_once $controllers_path . 'rental/wcfmu-controller-inventory.php';
                    new WCFM_RnB_Inventory_Controller();
                    break;
                    
                case 'wcfm-rnb-inventory-manage':
                    include_once $controllers_path . 'rental/wcfmu-controller-inventory-manage.php';
                    new WCFM_RnB_Inventory_Manage_Controller();
                    break;
    ...
    /**
     * Checks if a specific ACF field group is accessible based on the current user's roles.
     *
     * @param bool $is_allowed The current access status for the field group.
     * @param int $group_id The ID of the ACF field group being checked.
     * @return bool Returns true if access is allowed, false otherwise.
     */
    function check_acf_field_group_access($is_allowed, $group_id) {
        $field_groups = acf_get_field_groups();
        $field_group_to_check = null;
        foreach ($field_groups as $field_group) {
            if ($field_group['ID'] != $group_id) continue;
            $field_group_to_check = $field_group;
            break;
        }
        $current_user = wp_get_current_user();
        if ($field_group_to_check) {
            foreach ($field_group_to_check['location'] as $group) {
                foreach ($group as $rule_id => $rule) {
                    if ($rule['param'] !== 'current_user_role') continue;
                    if ($rule['operator'] == '==' && !in_array($rule['value'], $current_user->roles)) {
                        return false;
                    } elseif ($rule['operator'] == '!=' && in_array($rule['value'], $current_user->roles)) {
                        return false;
                    }
                }
            }
    ...
    function wcfm_wcrental_product_inventory_manage($stock_fields, $product_id, $product_type) {
        ...
        if (apply_filters('wcfm_is_allow_rental', true)) {
            $args = array(
                'post_type'         => 'Inventory',
                'posts_per_page'    => -1,
                'orderby'           => 'date',
            ...
            // hide other vendor inventories
            if (wcfm_is_vendor()) {
                if ( 'wcfmmarketplace' === $WCFM->is_marketplace ) {
                    $vendor_id = get_current_user_id();
                    $list_of_vendors = $WCFM->wcfm_vendor_support->wcfm_get_vendor_list( true );

                    if (isset($list_of_vendors[$vendor_id])) unset($list_of_vendors[$vendor_id]);

                    $args['author__not_in'] = array_keys($list_of_vendors);
                }
            }

            $inventories = get_posts(apply_filters('wcfmu_rnb_product_inventory_args', $args));
    ...
    /**
     *  Disables add product popup
     *  Also prevents product javascript from loading in the inventory list & manage page
     * 
     *  @param array $endpoints
     *  @return array $endpoints
     */
    public function disable_add_product_popup_in_rnb_inventory($endpoints) {
        $rnb_endpoints = [
            'wcfm-rnb-inventory',
            'wcfm-rnb-inventory-manage'
        ];
    
        foreach ($rnb_endpoints as $rnb_endpoint) {
            if (!in_array($rnb_endpoint, $endpoints)) {
                $endpoints[] = $rnb_endpoint;
            }
        }
    ...
    /**
     *  Deletes a RnB Inventory
     *  @return void
     */
    public function delete_wcfm_rnb_inventory() {
        if (!check_ajax_referer('wcfm_ajax_nonce', 'wcfm_ajax_nonce', false)) {
        ...
        $inventory_id = isset($_POST['inventory_id']) ? absint($_POST['inventory_id']) : null;

        if ($inventory_id) {
            if ('inventory' !== get_post_type($inventory_id)) {
                wp_send_json_error(esc_html('failed'));
            }

            $inventory = get_post($inventory_id);

            // Check if vendor can delete the inventory
            if (wcfm_is_vendor()) {
                $is_rnb_inventory_from_vendor = (absint($inventory->post_author) === get_current_user_id());
            ...
            do_action('wcfm_before_rnb_inventory_delete', $inventory_id);

            $allow_delete = apply_filters('wcfm_is_allow_rnb_inventory_delete', false, $inventory_id);

            $result = $allow_delete ? wp_delete_post($inventory_id) : wp_trash_post($inventory_id);

            if ($result) {
                wp_send_json_success(esc_html('success'));
            }
