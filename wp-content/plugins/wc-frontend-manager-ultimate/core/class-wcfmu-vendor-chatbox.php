...
class WCFMu_Vendor_Chatbox
{
    ...
    public $wcfm_myaccount_chatbox_endpoint = 'chatbox';
...
<?php
    ...
    function fbc_validate_ipv6($str)
    {
        ...
        $groups    = 8;
        $collapsed = false;
        $chunks    = array_filter(preg_split('/(:{1,2})/', $str, -1, PREG_SPLIT_DELIM_CAPTURE));
    ...
    function wcfm_chats_offline_delete()
    {
        ...
        if ( ! $messageid ) {
            wp_send_json_error( __( 'Invalid Message ID', 'wc-frontend-manager-ultimate' ) );
        }

        $current_vendor_id = (int) apply_filters( 'wcfm_current_vendor_id', get_current_user_id() );
        if ( ! current_user_can( 'manage_woocommerce' ) ) {
            $owner_id = $wpdb->get_var( $wpdb->prepare(
                "SELECT vendor_id FROM {$wpdb->prefix}wcfm_fbc_offline_messages WHERE id = %d",
                $messageid
            ) );

            if ( (int) $owner_id !== $current_vendor_id ) {
                wp_send_json_error( __( 'Permission denied.', 'wc-frontend-manager-ultimate' ) );
            }
        }

        $result = $wpdb->delete( "{$wpdb->prefix}wcfm_fbc_offline_messages", array( 'id' => $messageid ), array( '%d' ) );

        if ( $result ) {
            wp_send_json_success( __( 'Deleted successfully', 'wc-frontend-manager-ultimate' ) );
        } else {
            wp_send_json_error( __( 'Deletion failed or message already removed.', 'wc-frontend-manager-ultimate' ) );
        }
    ...
    function wcfm_chats_history_delete()
    {
        ...
        $conversation_id = isset( $_POST['conversation'] ) ? sanitize_text_field( $_POST['conversation'] ) : '';
        if ( ! $conversation_id ) {
            wp_send_json_error( __( 'No conversation ID provided.', 'wc-frontend-manager-ultimate' ) );
        }
        $current_vendor_id = (int) apply_filters( 'wcfm_current_vendor_id', get_current_user_id() );
        
        if ( !current_user_can('manage_woocommerce') ) { //for vendor or below users
            $check_sql = $wpdb->prepare(
                "SELECT COUNT(sessions.conversation_id) 
                FROM {$wpdb->prefix}wcfm_fbc_chat_sessions AS sessions
                INNER JOIN {$wpdb->prefix}wcfm_fbc_chat_visitors AS visitors ON sessions.user_id = visitors.user_id
                WHERE sessions.conversation_id = %s AND visitors.vendor_id = %d",
                $conversation_id,
                $current_vendor_id
            );

            if ( ! $wpdb->get_var( $check_sql ) ) {
                wp_send_json_error( __( 'Permission denied: You do not own this conversation.', 'wc-frontend-manager-ultimate' ) );
            }
        }

        $table_rows     = $wpdb->prefix . 'wcfm_fbc_chat_rows';
        $table_sessions = $wpdb->prefix . 'wcfm_fbc_chat_sessions';

        $wpdb->query( $wpdb->prepare( "DELETE FROM {$table_rows} WHERE `conversation_id` = %s", $conversation_id ) );
        $wpdb->query( $wpdb->prepare( "DELETE FROM {$table_sessions} WHERE `conversation_id` = %s", $conversation_id ) );
