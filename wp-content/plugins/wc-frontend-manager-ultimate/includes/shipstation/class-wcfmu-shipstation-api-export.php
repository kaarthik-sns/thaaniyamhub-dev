...
class WCFMu_ShipStation_Api_Export extends WCFMu_ShipStation_Api_Request {
     ...
     * @param int $vendor_id
     */
    public function __construct($authenticated, $vendor_id) {
        if (!$authenticated) {
            exit;
        }
    ...
    public function request() {
        ...
        $wcfm_shipstation_setting = get_user_meta($this->vendor_id, 'wcfm_shipstation_setting', true);
        ...
        $orders    = wcfmu_shipstation_get_orders($this->vendor_id, $args);
        ...
        $count       = wcfmu_shipstation_get_orders($this->vendor_id, $count_args);
        ...
        foreach ($order_ids as $order_id) {
            ...
            if ('yes' == $order->get_meta('_shipstation_exported_' . $this->vendor_id)) {
                continue;
            }
            ...
            $commission_ids = get_order_commission_ids_by_vendor($this->vendor_id, $order_id);
            ...
            foreach ($order_items as $item_id => $item) {
                ...
                if (!$product_vendor_id || ($product_vendor_id != $this->vendor_id)) {
                    continue;
                }
            ...
            if ('yes' !== $order->get_meta('_shipstation_exported_'.$this->vendor_id)) {
                $comment_id = $order->add_order_note(sprintf(__('Order has been exported to Shipstation for store %s', 'wc-frontend-manager-ultimate'),wcfm_get_vendor_store_name($this->vendor_id)));
                add_comment_meta($comment_id, '_vendor_id', $this->vendor_id);
                $order->update_meta_data( '_shipstation_exported_'.$this->vendor_id, 'yes' );
                $order->save();
            }
        }//end foreach
