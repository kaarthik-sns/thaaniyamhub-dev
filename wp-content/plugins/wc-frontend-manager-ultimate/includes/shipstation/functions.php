...
function shipstation_order_mark_shipped($order_id, $vendor_id, $shipstation_data) {
    ...
    if (!empty($order_id)) {
        ...
        foreach ($order_item_ids as $index => $order_item_id) {
            ...
            do_action('wcfm_after_order_mark_shipped', $order_id, $order_item_id, $tracking_code, $tracking_url, $product_ids[$index], []);
        }
...
/**
 * Retrieve the vendor user ID based on a ShipStation auth key.
 *
 * @param string $auth_key The ShipStation authentication key to search for.
 * @return int The vendor user ID if found, or 0 if no vendor matches.
 */
function shipstation_vendor_id_from_auth_key(string $auth_key): int {
    if (empty($auth_key)) {
        return 0;
    }
    ...
    $args = [
        ...
        'meta_value'    => sanitize_text_field($auth_key),
        'fields'        => 'ID',
        'number'        => 1,
    ];
