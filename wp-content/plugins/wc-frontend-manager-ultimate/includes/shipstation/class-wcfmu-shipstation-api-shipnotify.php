...
class WCFMu_ShipStation_Api_ShipNotify extends WCFMu_ShipStation_Api_Request
{
    ...
    public function request()
    {
        ...
        $shop_name       = wcfm_get_vendor_store(absint($this->vendor_id));
        ...
        $total_item_count = $this->order_items_to_ship_count($order, $this->vendor_id);
        ...
        if (sizeof($shipped_items) > 0) {
            ...
            $current_shipped_items = max((int) $order->get_meta('_shipstation_shipped_item_count_' . $this->vendor_id), 0);
            ...
            $order->update_meta_data('_shipstation_shipped_item_count_' . $this->vendor_id, ($current_shipped_items + $shipped_item_count));
            $order->save();
        } else {
            ...
            // Update order status.
            if ($order_shipped) {
                shipstation_order_mark_shipped($order_id, $this->vendor_id, $shipstation_data);

                // shipstation_vendor_order_status_update( $this->vendor_id, $order_id, $shipped_status );
                /*
            ...
            do_action('wcfmu_shipstation_shipnotify', $order, $shipstation_data, $this->vendor_id);
