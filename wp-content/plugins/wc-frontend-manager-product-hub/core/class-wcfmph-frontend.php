...
class WCFMph_Frontend {
	...
	function wcfm_force_sales_product_manage_fields($fields, $product_id, $products_array) {
		...
		if ($product_id) {
			$force_sell_ids        = get_post_meta($product_id, '_force_sell_ids', true);
			$force_sell_ids        = $force_sell_ids ? (array) $force_sell_ids : [];
			$force_sell_synced_ids = get_post_meta($product_id, '_force_sell_synced_ids', true);
			$force_sell_synced_ids = $force_sell_synced_ids ? (array) $force_sell_synced_ids : [];
	...
	function wcfm_chained_product_product_manage_fields_variations($variation_fileds, $variations, $variation_shipping_option_array, $variation_tax_classes_options, $products_array, $product_id, $product_type) {
		...
		if (!empty($variations)) {
			foreach ($variations as $variation_data) {
				...
				$chained_product_detail	= get_post_meta($variation_id, '_chained_product_detail', true);
				$chained_product_detail = $chained_product_detail ? (array) $chained_product_detail : [];
		...
		$chained_product_fields = array(
			...
			array(
				"product_id" => array('label' => __('Product', 'wc-frontend-manager'), 'type' => 'select', 'custom_attributes' => array('placeholder' => __('Search for a product...', 'woocommerce-chained-products')), 'attributes' => array('style' => 'width: 60%;'), 'class' => 'wcfm-select wcfm_ele variable', 'label_class' => 'wcfm_title', 'options' => $product_arr),
	...
	function wcfm_chained_product_product_manage_data_variations($variations, $variation_id, $variation_id_key) {
		...
		if ($variation_id) {
			$chained_product_details    	= get_post_meta($variation_id, '_chained_product_detail', true);
			$chained_product_details 		= $chained_product_details ? (array) $chained_product_details : [];
