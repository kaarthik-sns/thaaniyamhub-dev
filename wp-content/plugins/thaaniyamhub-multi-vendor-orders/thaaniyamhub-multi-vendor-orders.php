...
if (!thaaniyamhub_is_woocommerce_active()) {
    add_action('admin_notices', function () {
        ...
        <div class="notice notice-error">
            <p><?php esc_html_e('Thaaniyam Hub - Multi-Vendor Orders requires WooCommerce to be installed and active.', 'thaaniyamhub-multi-vendor-orders'); ?>
            </p>
        </div>
        <?php
    });
...
// Initialize Comprehensive Order Duplication Debug Logger immediately
if (class_exists('ThaaniyamHub_Order_Debug_Logger')) {
    ThaaniyamHub_Order_Debug_Logger::init();
}
...
if (!function_exists('thaaniyamhub_render_customer_order_refund_card')) {
    function thaaniyamhub_render_customer_order_refund_card($order)
    {
        ...
        ?>
        <section class="woocommerce-order-refund-details"
            style="margin: 28px 0; padding: 20px 24px; background: #fef2f2; border: 1px solid #fecaca; border-left: 4px solid #dc2626; border-radius: 8px;">
            <h2 class="woocommerce-order-details__title"
                style="margin: 0 0 12px 0; font-size: 16px; color: #991b1b; display: flex; align-items: center; gap: 8px;">
                ...
                <span style="font-size: 13px; font-weight: normal; color: #7f1d1d;">
                    (Total Refunded:
                    <strong><?php echo wp_strip_all_tags(wc_price($refund_data['total_refunded'], ['currency' => $order->get_currency()])); ?></strong>)
                </span>
            </h2>
            <div style="overflow-x: auto;">
                <table class="woocommerce-table woocommerce-table--order-details shop_table order_details"
                    style="width: 100%; border-collapse: collapse; font-size: 13px; background: #ffffff; border-radius: 6px; overflow: hidden; border: 1px solid #fecaca;">
                    <thead>
                        ...
                        <?php foreach ($refunds as $ref): ?>
                            <tr style="border-top: 1px solid #fee2e2;">
                                ...
                                <td style="padding: 10px 14px; font-weight: 500;">
                                    <?php echo esc_html($ref['vendor_name'] ?: __('Main Order', 'thaaniyamhub-multi-vendor-orders')); ?>
                                </td>
                                ...
                                <td style="padding: 10px 14px; text-align: center;">
                                    <span
                                        style="background: #d1fae5; color: #065f46; padding: 2px 8px; border-radius: 4px; font-size: 11px; font-weight: 700;">
