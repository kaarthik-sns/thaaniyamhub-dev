	...
	public static function output( $attr ) {
		...
		<div class="wcfm_sc_notifications">
			...
			<?php if ( $notice && apply_filters( 'wcfm_is_pref_notice', true ) && apply_filters( 'wcfm_is_allow_notice', true ) && apply_filters( 'wcfm_is_allow_sc_notice_notifications', true ) ) { ?>
				<a href="<?php echo esc_url( get_wcfm_notices_url() ); ?>" class="wcfmfa fa-bullhorn text_tip" data-tip="<?php _e( 'Notice Board', 'wc-frontend-manager' ); ?>">
				<?php
				if ( wcfm_is_vendor() ) {
					?>
					<span class="unread_notification_count notice_count"><?php echo esc_attr( $unread_notice ); ?></span><?php } ?></a>
			<?php } ?>
		</div>
		<?php
	}
