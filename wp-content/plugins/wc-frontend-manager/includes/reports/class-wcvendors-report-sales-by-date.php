...
class WC_Vendors_Report_Sales_By_Date extends WC_Admin_Report {
	...
	public function query_report_data() {
		...
		foreach ( $results as $data ) {
			...
			if ( 'paid' === $data->status ) {
				...
				if ( version_compare( WCV_VERSION, '2.0.0', '<' ) ) {
					if ( WC_Vendors::$pv_options->get_option( 'give_tax' ) ) {
						$total_commission_amount += (float) sanitize_text_field( $data->tax ); }
					if ( WC_Vendors::$pv_options->get_option( 'give_shipping' ) ) {
						$total_commission_amount += (float) sanitize_text_field( $data->total_shipping ); }
				} else {
					if ( get_option( 'wcvendors_vendor_give_taxes' ) ) {
						$total_commission_amount += (float) sanitize_text_field( $data->tax ); }
					if ( get_option( 'wcvendors_vendor_give_shipping' ) ) {
		...
		if ( version_compare( WCV_VERSION, '2.0.0', '<' ) ) {
			if ( WC_Vendors::$pv_options->get_option( 'give_tax' ) ) {
				$total_sales += $total_tax_amount; }
			if ( WC_Vendors::$pv_options->get_option( 'give_shipping' ) ) {
				$total_sales += $total_shipping_amount; }
		} else {
			if ( get_option( 'wcvendors_vendor_give_taxes' ) ) {
				$total_sales += $total_tax_amount; }
			if ( get_option( 'wcvendors_vendor_give_shipping' ) ) {
	...
	public function get_chart_legend() {
		...
		} elseif ( get_option( 'wcvendors_vendor_give_shipping' ) && apply_filters( 'wcfm_sales_report_is_allow_shipping', true ) ) {
				$legend[] = array(
					'title'            => sprintf( __( '%s charged for shipping', 'wc-frontend-manager' ), '<strong>' . wc_price( $data->total_shipping ) . '</strong>' ),
					'color'            => $this->chart_colors['shipping_amount'],
					'highlight_series' => 2,
