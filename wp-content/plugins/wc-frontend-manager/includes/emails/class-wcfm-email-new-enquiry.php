	...
	class WCFM_Email_New_enquiry extends WC_Email {
		...
		function trigger( $args ) {
			...
			if ( ! empty( $args ) ) {
				...
				if ( $vendor_id ) {
					$enquiry_for = '<a target="_blank" class="wcfm_dashboard_item_title" href="' . get_wcfm_enquiry_url() . '">' . wcfm_get_vendor_store_name( $vendor_id ) . ' ' . apply_filters( 'wcfm_sold_by_label', $vendor_id, __( 'Store', 'wc-frontend-manager' ) ) . '</a>';
				}
				if ( $product_id ) {
					$enquiry_for = '<a target="_blank" class="wcfm_dashboard_item_title" href="' . get_wcfm_enquiry_url() . '">' . get_the_title( $product_id ) . '</a>';
				}
