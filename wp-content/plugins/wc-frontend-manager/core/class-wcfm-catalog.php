...
class WCFM_Catalog {
	...
	function wcfm_product_manage_fields_catalog_mode( $general_fields, $product_id, $product_type, $wcfm_is_translated_product = false, $wcfm_wpml_edit_disable_element = '' ) {
		...
		$general_fields = array_slice( $general_fields, 0, 1, true ) +
																	array(
																		'is_catalog' => array(
																			'desc'       => __( 'Catalog', 'wc-frontend-manager' ),
																			'type'       => 'checkbox',
																			'class'      => 'wcfm-checkbox wcfm_ele wcfm_half_ele_checkbox simple variable booking non-pw-gift-card' . ' ' . $wcfm_wpml_edit_disable_element,
																			'desc_class' => 'wcfm_title wcfm_ele virtual_ele_title checkbox_title simple variable booking non-pw-gift-card' . ' ' . $wcfm_wpml_edit_disable_element,
																			'value'      => 'yes',
																			'dfvalue'    => $is_catalog,
																		),
																	) +
		...
		<div class="wcfm-container simple variable booking catalog_options">
			<div id="wcfm_products_manage_form_catalog_expander" class="wcfm-content">
				...
				$WCFM->wcfm_fields->wcfm_generate_form_field(
					apply_filters(
						'wcfm_product_fields_catalog_options',
						array(
							'disable_add_to_cart' => array(
								'label'       => __( 'Disable Add to Cart?', 'wc-frontend-manager' ),
								'type'        => 'checkbox',
								'class'       => 'wcfm-checkbox wcfm_ele simple variable booking',
								'value'       => 'yes',
								'label_class' => 'wcfm_title wcfm_ele checkbox_title simple variable booking',
								'dfvalue'     => $disable_add_to_cart,
							),
							'disable_price'       => array(
								'label'       => __( 'Hide Price?', 'wc-frontend-manager' ),
								'type'        => 'checkbox',
								'value'       => 'yes',
								'class'       => 'wcfm-checkbox wcfm_ele simple variable booking',
								'label_class' => 'wcfm_title wcfm_ele checkbox_title simple variable booking',
								'dfvalue'     => $disable_price,
							),
						),
						$product_id,
						$product_type
