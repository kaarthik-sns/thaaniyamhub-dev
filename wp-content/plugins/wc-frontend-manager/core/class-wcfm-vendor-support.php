...
class WCFM_Vendor_Support {
	...
	function vendors_wcfm_menus( $menus ) {
		...
		if ( $WCFM->is_marketplace == 'wcfmmarketplace' ) {
			$menus = array_slice( $menus, 0, 3, true ) +
													array(
														'wcfm-vendors' => array(
															'label'              => apply_filters( 'wcfm_sold_by_label', '', __( 'Store', 'wc-frontend-manager' ) ) . ' ' . __( 'Vendors', 'wc-frontend-manager' ),
			...
			$menus = array_slice( $menus, 0, 3, true ) +
													array(
														'wcfm-vendors' => array(
															'label'    => __( 'Vendors', 'wc-frontend-manager' ),
															'url'      => get_wcfm_vendors_url(),
															'icon'     => 'user-alt',
															'priority' => 45,
														),
	...
	function wcfm_vendor_coupon_apply_validate( $valid, $product, $coupon, $values ) {
		if ( $valid ) {
			...
			if ( $coupon_vendor ) {
				...
				if ( $coupon_vendor != $product_vendor ) {
					$valid = false;
				}
			}
		}
	...
	function wcfm_associate_vendor( $product_id, $product_type, $wcfm_is_translated_product = false, $wcfm_wpml_edit_disable_element = '' ) {
		...
		if ( $is_marketplace ) {
			...
			$vendor_arr            = array();
			if ( $wcfm_associate_vendor ) {
				$vendor_arr = array( $wcfm_associate_vendor => $this->wcfm_get_vendor_store_name_by_vendor( $wcfm_associate_vendor ) );
			}
			?>
				...
				<div id="wcfm_products_manage_form_vendor_association_expander" class="wcfm-content">
					...
					$WCFM->wcfm_fields->wcfm_generate_form_field(
						apply_filters(
							...
							array(
								'wcfm_associate_vendor' => array(
									'label'       => apply_filters( 'wcfm_sold_by_label', $wcfm_associate_vendor, __( 'Store', 'wc-frontend-manager' ) ),
									'type'        => 'select',
									'options'     => $vendor_arr,
									'attributes'  => array( 'style' => 'width: 60%;' ),
									'class'       => 'wcfm-select',
									'label_class' => 'wcfm_title',
									'value'       => $wcfm_associate_vendor,
								),
							),
							$product_id
						)
			...
			<div id="wcfm_products_manage_form_commission_expander" class="wcfm-content">
				...
				$WCFM->wcfm_fields->wcfm_generate_form_field(
					apply_filters(
						'product_manage_fields_commission',
						array(
							'pv_commission_rate' => array(
								'label'       => __( 'Commission(%)', 'wc-frontend-manager' ),
								'type'        => 'number',
								'attributes'  => array(
									'min'   => '',
									'steps' => 1,
				...
				if ( $WCMp->vendor_caps->payment_cap['commission_type'] == 'fixed_with_percentage' ) {
					$WCFM->wcfm_fields->wcfm_generate_form_field(
						apply_filters(
							'product_manage_fields_commission',
							array(
								'_commission_percentage_per_product' => array(
									'label'       => __( 'Commission(%)', 'wc-frontend-manager' ),
									'type'        => 'number',
									'attributes'  => array(
										'min'   => '',
									...
									'class'       => 'wcfm-text wcfm_ele simple variable external grouped booking',
									'label_class' => 'wcfm_title wcfm_ele simple variable external grouped booking',
									'value'       => $commission_percentage_per_poduct,
								),
								'_commission_fixed_with_percentage' => array(
									'label'       => __( 'Fixed (per transaction)', 'wc-frontend-manager' ),
									'type'        => 'number',
									'attributes'  => array(
										'min'   => '',
										'steps' => 1,
									...
									'class'       => 'wcfm-text wcfm_ele simple variable external grouped booking',
									'label_class' => 'wcfm_title wcfm_ele simple variable external grouped booking',
									'value'       => $commission_fixed_with_percentage,
								),
							)
						)
					);
					...
					$WCFM->wcfm_fields->wcfm_generate_form_field(
						apply_filters(
							...
							array(
								...
								'_commission_fixed_with_percentage_qty' => array(
									'label'       => __( 'Fixed (per unit)', 'wc-frontend-manager' ),
									'type'        => 'number',
									'attributes'  => array(
										'min'   => '',
										'steps' => 1,
									...
									'class'       => 'wcfm-text wcfm_ele simple variable external grouped booking',
									'label_class' => 'wcfm_title wcfm_ele simple variable external grouped booking',
									'value'       => $commission_fixed_with_percentage_qty,
								),
							)
						)
					);
					...
					$WCFM->wcfm_fields->wcfm_generate_form_field(
						apply_filters(
							'product_manage_fields_commission',
							array(
								'_commission_per_product' => array(
									'label'       => __( 'Commission', 'wc-frontend-manager' ) . ' (' . $WCMp->vendor_caps->payment_cap['commission_type'] . ')',
									'type'        => 'number',
									'attributes'  => array(
										'min'   => '',
										'steps' => 1,
	...
	function wcmarketplace_product_commission_save( $new_product_id, $wcfm_products_manage_form_data ) {
		...
		} elseif ( isset( $wcfm_products_manage_form_data['_commission_per_product'] ) && ! empty( $wcfm_products_manage_form_data['_commission_per_product'] ) ) {
	...
	function wcmarketplace_product_variation_commission_save( $wcfm_variation_data, $new_product_id, $variation_id, $variations, $wcfm_products_manage_form_data ) {
		...
		} elseif ( isset( $variations['_commission_per_product'] ) && ! empty( $variations['_commission_per_product'] ) ) {
	...
	public function wcfm_get_vendor_store_name_by_vendor( $vendor_id ) {
		...
		if ( $marketplece == 'wcvendors' ) {
			...
			if ( $shop_name ) {
				$vendor_store = esc_attr( $shop_name ); }
		}
	...
	public function wcfm_get_vendor_store_by_vendor( $vendor_id ) {
		...
		if ( $marketplece == 'wcvendors' ) {
			...
			if ( $shop_name ) {
				$vendor_store = '<a ' . $store_open_by . ' href="' . apply_filters( 'wcv_vendor_shop_permalink', $shop_link ) . '">' . $shop_name . '</a>'; } else {
				$vendor_store = '<a ' . $store_open_by . ' href="' . apply_filters( 'wcv_vendor_shop_permalink', $shop_link ) . '">' . __( 'Shop', 'wc-frontend-manager' ) . '</a>'; }
		} elseif ( $marketplece == 'wcmarketplace' ) {
			$vendor = get_wcmp_vendor( $vendor_id );
			if ( $vendor ) {
				$shop_name  = get_user_meta( $vendor_id, '_vendor_page_title', true );
				$store_name = get_user_meta( $vendor_id, 'store_name', true );
				if ( $shop_name ) {
					$vendor_store = '<a ' . $store_open_by . ' href="' . apply_filters( 'wcmp_vendor_shop_permalink', $vendor->permalink ) . '">' . $shop_name . '</a>'; } elseif ( $store_name ) {
					$vendor_store = '<a ' . $store_open_by . ' href="' . apply_filters( 'wcmp_vendor_shop_permalink', $vendor->permalink ) . '">' . $store_name . '</a>'; } else {
						$vendor_store = '<a ' . $store_open_by . ' href="' . apply_filters( 'wcmp_vendor_shop_permalink', $vendor->permalink ) . '">' . __( 'Shop', 'wc-frontend-manager' ) . '</a>'; }
			}
		} elseif ( $marketplece == 'wcpvendors' ) {
			$vendor_data = get_term( $vendor_id, WC_PRODUCT_VENDORS_TAXONOMY );
			$shop_name   = $vendor_data->name;
			...
			if ( ! is_wp_error( $shop_link ) ) {
				if ( $shop_name ) {
					$vendor_store = '<a ' . $store_open_by . ' href="' . apply_filters( 'wcpv_vendor_shop_permalink', $shop_link ) . '">' . $shop_name . '</a>'; } else {
					$vendor_store = '<a ' . $store_open_by . ' href="' . apply_filters( 'wcpv_vendor_shop_permalink', $shop_link ) . '">' . __( 'Shop', 'wc-frontend-manager' ) . '</a>'; }
			}
		} elseif ( $marketplece == 'dokan' ) {
			$vendor_data = get_user_meta( $vendor_id, 'dokan_profile_settings', true );
			$shop_name   = isset( $vendor_data['store_name'] ) ? esc_attr( $vendor_data['store_name'] ) : '';
			...
			if ( $shop_name ) {
				$vendor_store = '<a ' . $store_open_by . ' href="' . apply_filters( 'dokan_vendor_shop_permalink', $shop_link ) . '">' . $shop_name . '</a>'; } else {
				$vendor_store = '<a ' . $store_open_by . ' href="' . apply_filters( 'dokan_vendor_shop_permalink', $shop_link ) . '">' . __( 'Shop', 'wc-frontend-manager' ) . '</a>'; }
		} elseif ( $marketplece == 'wcfmmarketplace' ) {
			$shop_name   = get_user_meta( $vendor_id, 'store_name', true );
			$vendor_user = get_user_by( 'id', $vendor_id );
			if ( empty( $shop_name ) && $vendor_user ) {
			...
			if ( $shop_name ) {
				$vendor_store = '<a class="wcfm_dashboard_item_title" ' . $store_open_by . ' href="' . apply_filters( 'wcfmmp_vendor_shop_permalink', $shop_link, $vendor_id ) . '">' . esc_attr( $shop_name ) . '</a>'; } else {
				$vendor_store = '<a class="wcfm_dashboard_item_title" ' . $store_open_by . ' href="' . apply_filters( 'wcfmmp_vendor_shop_permalink', $shop_link, $vendor_id ) . '">' . __( 'Shop', 'wc-frontend-manager' ) . '</a>'; }
		}
	...
	public function wcfm_get_vendor_address_by_vendor( $vendor_id ) {
		...
		if ( $addr_1 ) {
			$vendor_address .= $addr_1;
		}
		if ( $addr_2 ) {
			$vendor_address .= '<br />' . $addr_2;
		}
		if ( $city ) {
			$vendor_address .= '<br />' . $city;
		}
		if ( $state_name ) {
			$vendor_address .= ', ' . $state_name;
		}
		if ( $country_name ) {
			$vendor_address .= '<br />' . $country_name;
		}
		if ( $zip ) {
			$vendor_address .= ' - ' . $zip;
		}
	...
	public function wcfm_get_vendor_email_by_vendor( $vendor_id ) {
		...
		if ( ! $vendor_email || empty( $vendor_email ) ) {
			$vendor_email = '';
		}
	...
	public function wcfm_get_vendor_phone_by_vendor( $vendor_id ) {
		...
		if ( ! $vendor_phone || empty( $vendor_phone ) ) {
			$vendor_phone = '';
		}
		...
		if ( $marketplece == 'wcvendors' ) {
			...
			if ( ! empty( $gross_sales_whole_week ) ) {
				foreach ( $gross_sales_whole_week as $net_sale_whole_week ) {
							...
							foreach ( $line_items as $key => $line_item ) {
								if ( $line_item->get_product_id() == 0 ) {
									...
									if ( in_array( $_product_id, $valid_items ) || in_array( $_variation_id, $valid_items ) ) {
										...
										} elseif ( get_option( 'wcvendors_vendor_give_taxes' ) ) {
												$gross_sales += (float) sanitize_text_field( $line_item->get_total_tax() );
										}
									}
					...
					} elseif ( get_option( 'wcvendors_vendor_give_shipping' ) ) {
							$gross_sales += (float) $net_sale_whole_week->total_shipping;
					}
				}
			}
	...
	function wcfm_get_commission_by_vendor( $vendor_id = '', $interval = '7day', $is_paid = false, $order_id = 0, $filter_date_form = '', $filter_date_to = '' ) {
		...
		$sql .= ' WHERE 1=1';
		if ( $vendor_id ) {
			$sql .= $wpdb->prepare( " AND commission.{$vendor_handler} = %d", $vendor_id );
		}
		if ( $is_paid ) {
	...
	function wcfm_get_commission_by_order( $order_id = '', $is_paid = false ) {
		...
		$sql .= ' WHERE 1=1';
		if ( $order_id ) {
			$sql .= $wpdb->prepare( ' AND commission.order_id = %d', $order_id );
		}
		if ( $is_paid ) {
			$sql .= " AND (commission.{$status} = 'paid' OR commission.{$status} = 'completed')";
		}
	...
	function wcfm_is_article_from_vendor( $article_id, $current_vendor = '' ) {
		...
		if ( $author_id && ! $current_vendor ) {
			...
			if ( $current_vendor == $author_id ) {
				$is_article_from_vendor = true;
			}
		}
	...
	function wcfm_get_vendor_id_from_product( $product_id = '' ) {
		...
		if ( $product_id ) {
			if ( $WCFM->is_marketplace == 'wcmarketplace' ) {
				...
				if ( dokan_is_user_seller( $author ) ) {
					$vendor_id = $author;
				}
			} elseif ( $WCFM->is_marketplace == 'wcfmmarketplace' ) {
				$product = get_post( $product_id );
	...
	function wcfm_is_order_for_vendor( $order_id, $current_vendor = '' ) {
		...
		if ( $current_vendor ) {
			$sql .= $wpdb->prepare( " AND commission.{$vendor_handler} = %d", $current_vendor );
		}
		$vendor_order_data = $wpdb->get_results( $sql );
		if ( empty( $vendor_order_data ) ) {
			$is_order_for_vendor = false; }
	...
	function wcfm_is_component_for_vendor( $component_id, $component = '', $current_vendor = '' ) {
		...
		switch ( $component ) {
			...
			case 'booking':
				...
				if ( empty( $booking_vendor_id ) || ( $booking_vendor_id != $current_vendor ) ) {
					$is_component_for_vendor = false; }
				break;
			...
			case 'support':
				...
				if ( empty( $vendor_support_data ) ) {
					$is_component_for_vendor = false; }
				break;

			case 'delivery':
				$delivery_vendor_id = get_user_meta( $component_id, '_wcfm_vendor', true );
				if ( empty( $delivery_vendor_id ) || ( $delivery_vendor_id != $current_vendor ) ) {
					$is_component_for_vendor = false; }
				break;

			case 'customer':
				$customer_vendor_id = get_user_meta( $component_id, '_wcfm_vendor', true );
				if ( empty( $customer_vendor_id ) || ( $customer_vendor_id != $current_vendor ) ) {
					$is_component_for_vendor = false; }
				break;

			case 'staff':
				$staff_vendor_id = get_user_meta( $component_id, '_wcfm_vendor', true );
				if ( empty( $staff_vendor_id ) || ( $staff_vendor_id != $current_vendor ) ) {
					$is_component_for_vendor = false; }
				break;
		}
	...
	public function wcfm_vendor_allowed_element_capability( $vendor_id, $capability = '', $element = '' ) {
		...
		if ( is_array( $allowed_capabilities ) && ! empty( $allowed_capabilities ) ) {
			if ( ! in_array( $element, $allowed_capabilities ) ) {
				return false;
			}
		}
		return true;
	...
	public function wcfm_vendor_product_limit( $vendor_id ) {
		...
		if ( ( $productlimit == -1 ) || ( $productlimit == '-1' ) ) {
			...
			$productlimit = absint( $productlimit );
		}
	...
	public function wcfm_vendor_product_limit_stat( $vendor_id ) {
		...
		$total_products = wcfm_get_user_posts_count( $vendor_id, 'product', apply_filters( 'wcfm_limit_check_status', 'any' ) );  
	...
	public function wcfm_vendor_space_limit_stat( $vendor_id ) {
		...
		if ( ! $spacelimit ) {
			...
			if ( $used_space > $spacelimit ) {
				$used_space = $spacelimit;
			}
			$spacelimit .= ' MB';
		}
	...
	function wcmp_tabs_capability_disabled_by_wcfm( $wcmp_tabs ) {
		if ( apply_filters( 'wcfm_is_allow_wcmp_tabs_manage', true ) ) {
			if ( isset( $wcmp_tabs['capabilities'] ) ) {
				unset( $wcmp_tabs['capabilities'] );
			}
		}
		return $wcmp_tabs;
	}
