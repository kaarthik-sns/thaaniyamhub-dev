...
class WCFM_Dokan {
	...
	function dokan_store_name( $store_name ) {
		...
		$shop_link = dokan_get_store_url( $user_id );
		if ( $shop_name ) {
			$store_name = '<a target="_blank" href="' . apply_filters( 'dokan_vendor_shop_permalink', $shop_link ) . '">' . $shop_name . '</a>'; } else {
			$store_name = '<a target="_blank" href="' . apply_filters( 'dokan_vendor_shop_permalink', $shop_link ) . '">' . __( 'Shop', 'wc-frontend-manager' ) . '</a>'; }
			return $store_name;
	}
		...
		if ( empty( $gmw_saved_location ) ) {
			...
			if ( $map_location ) {
				...
				if ( is_array( $map_locations ) ) {
					if ( isset( $map_locations[0] ) ) {
						$store_lat = $map_locations[0];
					}
					if ( isset( $map_locations[1] ) ) {
						$store_lng = $map_locations[1];
					}
				}
			}
