...
class WCFM_WCMarketplace {
	...
	function wcmarketplace_store_name( $store_name ) {
		$vendor       = get_wcmp_vendor( $this->vendor_id );
		$shop_name    = wcfm_get_option( 'wcfm_my_store_label', __( 'My Store', 'wc-frontend-manager' ) );   
		$vmstore_name = wcfm_get_option( 'wcfm_my_store_label', __( 'My Store', 'wc-frontend-manager' ) );   
		if ( $shop_name ) {
			$store_name = '<a target="_blank" href="' . apply_filters( 'wcmp_vendor_shop_permalink', $vendor->permalink ) . '">' . $shop_name . '</a>'; } elseif ( $vmstore_name ) {
			$store_name = '<a target="_blank" href="' . apply_filters( 'wcmp_vendor_shop_permalink', $vendor->permalink ) . '">' . $vmstore_name . '</a>'; } else {
				$store_name = '<a target="_blank" href="' . apply_filters( 'wcmp_vendor_shop_permalink', $vendor->permalink ) . '">' . __( 'Shop', 'wc-frontend-manager' ) . '</a>'; }
			return $store_name;
	}
	...
	function wcmarketplace_product_shipping_class( $product_shipping_class ) {
		...
		foreach ( $product_shipping_class as $product_shipping ) {
			if ( $vendor_shipping_class_id != $product_shipping->term_id ) {
				continue;
			}
				...
				<td class="line_cost" width="1%">
					<div class="view">
						...
						if ( $order_line_due[0]->commission_id ) {
							...
							_e( 'N/A', 'wc-frontend-manager' ); }
						if ( $order_line_due[0]->total_shipping != 'NAN' ) {
							$line_total += $order_line_due[0]->total_shipping;
						}
						if ( $order_line_due[0]->tax != 'NAN' ) {
							$line_total += $order_line_due[0]->tax;
						}
						if ( $order_line_due[0]->shipping_tax_amount != 'NAN' ) {
			...
			<?php if ( ! $admin_fee_mode ) { ?>
				<tr>
					...
					<td class="total">
						...
						<?php
						if ( $order_due[0]->commission_id ) {
							echo wc_price( $order_due[0]->line_total, array( 'currency' => $order_currency ) );
						} else {
							_e( 'N/A', 'wc-frontend-manager' ); }
						?>
						</div>
