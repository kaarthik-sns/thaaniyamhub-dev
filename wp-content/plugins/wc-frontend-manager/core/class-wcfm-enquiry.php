...
class WCFM_Enquiry {
	...
	public function load_scripts( $end_point ) {
		...
		switch ( $end_point ) {
			case 'wcfm-enquiry':
				...
				wp_localize_script(
					'wcfm_enquiry_js',
					'wcfm_enquiry_auto_refresher',
					array(
						'is_allow' => apply_filters( 'wcfm_enquiry_is_allow_auto_refresher', true ),
						'duration' => apply_filters( 'wcfm_enquiry_auto_refresher_duration', 60000 ),
					)
				);
				break;
		...
		if ( isset( $_POST['enquiry_ids'] ) ) {
			...
			if ( is_array( $enquiry_ids ) && ! empty( $enquiry_ids ) ) {
				...
				if ( empty( $allowed_enquiry_ids ) ) {
					wp_send_json_error( array( 'message' => __( 'You don&#8217;t have permission to do this.', 'wc-frontend-manager' ) ) );
				}
	...
	function wcfm_enquiry_button() {
		...
		if ( apply_filters( 'wcfm_is_pref_enquiry', true ) && apply_filters( 'wcfm_is_pref_enquiry_button', true ) && apply_filters( 'wcfm_is_allow_product_enquiry_bubtton', true ) && ! defined( 'DOING_AJAX' ) ) {
			...
			if ( ! empty( $wcfm_store_color_settings ) ) {
				...
				if ( isset( $wcfm_options['wc_frontend_manager_button_background_color_settings'] ) ) {
				...
				if ( isset( $wcfm_options['wc_frontend_manager_button_text_color_settings'] ) ) {
					$button_style .= 'color: ' . $wcfm_options['wc_frontend_manager_button_text_color_settings'] . ';'; }
				if ( isset( $wcfm_options['wc_frontend_manager_base_highlight_color_settings'] ) ) {
	...
	function wcfm_enquiry_with_login( $is_allow ) {
		...
		if ( $wcfm_enquiry_with_login == 'yes' ) {
			$is_allow = true;
		}
		return $is_allow;
	}
