...
class WCFM_Notification {
	...
	public function wcfm_admin_notification_new_product( $vendor_id, $product_id ) {
		...
		if ( ! $wcfm_new_product_notified ) {
			...
			$author_is_vendor = 1;
			$message_to       = 0;
			$wcfm_messages    = sprintf( __( 'A new product <b>%1$s</b> added by <b>%2$s</b>', 'wc-frontend-manager' ), '<a class="wcfm_dashboard_item_title" href="' . get_permalink( $product_id ) . '">' . get_the_title( $product_id ) . '</a>', wcfm_get_vendor_store( $vendor_id ) );
	...
	function wcfm_message_on_product_reject( $product_id, $reason ) {
		...
		$wcfm_messages = sprintf( __( 'Product <b>%1$s</b> has been rejected.<br/>Reason: %2$s', 'wc-frontend-manager' ), '<a class="wcfm_dashboard_item_title" target="_blank" href="' . get_wcfm_edit_product_url( $product_id ) . '">' . get_the_title( $product_id ) . '</a>', $reason );
	...
	function wcfm_message_on_new_order( $order_id, $is_renewal = false ) {
		...
		if ( is_admin() && ! $is_renewal ) {
			return;
		}

		$order = wc_get_order( $order_id );
		if ( $order->get_meta( '_wcfm_new_order_notified' ) ) {
			return;
		}
					...
					<div id="wcfm_dashboard_notifications_expander" class="wcfm-content">
						...
						if ( ! empty( $wcfm_messages ) ) {
							...
							foreach ( $wcfm_messages as $wcfm_message ) {
								...
								if ( ! $wcfm_message->message_type ) {
									$wcfm_message->message_type = 'direct';
								}
								...
								if ( $wcfm_message->message_type == 'direct' ) {
									$message_text = substr( strip_tags( $message_text ), 0, $wcfm_dashboard_message_content_length ) . ' ...';
								}
	...
	public function wcfm_direct_message_count( $message_type = 'notice', $message_status = 'unread' ) {
		...
		} elseif ( ( $message_type == 'message' ) && apply_filters( 'wcfm_is_allow_notifications', true ) ) {
			if ( wcfm_is_vendor() || ( function_exists( 'wcfm_is_delivery_boy' ) && wcfm_is_delivery_boy() ) || ( function_exists( 'wcfm_is_affiliate' ) && wcfm_is_affiliate() ) ) {
	...
	public function wcfm_send_direct_message( $author_id, $message_to, $author_is_admin, $author_is_vendor, $wcfm_messages, $wcfm_messages_type = 'direct', $email_notification = true, $raw_message = array() ) {
		global $WCFM, $wpdb;
		...
		if ( ! apply_filters( 'wcfm_is_allow_send_notification', true, $wcfm_messages_type, $message_to ) ) {
			return;
		}
	...
	protected function wcfm_direct_message_body( $args ) {
		...
		if ( ( $args['message_to'] != -1 ) && ( $args['message_to'] != '-1' ) && apply_filters( 'wcfm_is_pref_notification', true ) && ( ! wcfm_is_vendor( $args['message_to'] ) || ( wcfm_is_vendor( $args['message_to'] ) && wcfm_vendor_has_capability( $args['message_to'], 'notification' ) ) ) ) {
			$notification_mail_body = '<br/>' .
				...
				'<br/><br/>' .
				sprintf( __( 'Check more details %1$shere%2$s.', 'wc-frontend-manager' ), '<a href="{notification_url}">', '</a>' ) .
	...
	function wcfm_message_notification() {
		...
		if ( isset( $_POST['limit'] ) && $_POST['limit'] ) {
			...
			if ( $wcfm_messages_json_arr ) {
				$wcfm_messages_json_arr = '<div class="wcfm_notification_wrapper"><span class="wcfmfa fa-times-circle wcfm_notification_close"></span><div class="wcfm-clearfix"></div>' . $wcfm_messages_json_arr . '</div>';
			}
		...
		if ( isset( $_POST['selected_messages'] ) ) {
			...
			if ( is_array( $selected_messages ) && ! empty( $selected_messages ) ) {
				foreach ( $selected_messages as $messageid ) {
					...
					if ( $resource_owner_id < 0 ) {
						$resource_owner_id = 0; }
					if ( ! wcfm_user_can_perform_request( (int) $resource_owner_id, 'message_delete' ) ) {
						continue; }
