...
class WCFM_Library {
	...
	public function load_scripts( $end_point ) {
		...
		switch ( $end_point ) {
			...
			case 'wcfm-products-manage':
				...
				$wcfm_product_type_default_tab = apply_filters(
					'wcfm_product_type_default_tab',
					array(
						'simple'                => 'wcfm_products_manage_form_inventory_head',
						'variable'              => 'wcfm_products_manage_form_variations_head',
			...
			case 'wcfm-orders':
				...
				wp_localize_script(
					'dataTables_js',
					'wcfm_datatable_columns',
					array(
						'defs'     => $wcfm_datatable_column_defs,
						'priority' => $wcfm_datatable_column_priority,
						'bFilter'  => apply_filters( 'wcfm_datatable_bfiltery', ( wcfm_is_vendor() ) ? true : true, 'order' ),
					)
				);
				...
				wp_localize_script(
					'wcfm_orders_js',
					'wcfm_orders_auto_refresher',
					array(
						'is_allow' => apply_filters( 'wcfm_orders_is_allow_auto_refresher', false ),
						'duration' => apply_filters( 'wcfm_order_auto_refresher_duration', 60000 ),
					)
				);
				break;
			...
			case 'wcfm-profile':
				...
				$wcfm_profile_params = array(
					...
					'Password_failed'   => __( 'Password strength should be atleast "Good".', 'wc-frontend-manager' ),
				);
			...
			case 'wcfm-settings':
				...
				if ( $WCFM->is_marketplace && wcfm_is_vendor() ) {
					...
					if ( $WCFM->is_marketplace == 'dokan' ) {
						...
						wp_localize_script(
							'wcfm_marketplace_settings_js',
							'wcfm_marketplace_setting_map_options',
							array(
								'search_location' => __( 'Insert your address ..', 'wc-multivendor-marketplace' ),
								'locate_svg'      => $WCFMmp->plugin_url . 'assets/images/locate.svg',
			...
			case 'wcfm-messages':
				...
				wp_localize_script(
					...
					array(
						'is_allow' => apply_filters( 'wcfm_notifications_is_allow_auto_refresher', true ),
	...
	function load_datatable_lib() {
		...
		wp_localize_script(
			...
			array(
				...
				'is_allow_hidden_export' => apply_filters( 'wcfm_is_allow_datatable_hidden_export', false ),
			)
		);
	...
	public function load_upload_lib() {
		...
		$uploads_language = array(
			'choose_media'   => __( 'Choose Media', 'wc-frontend-manager' ),
			'choose_image'   => __( 'Choose Image', 'wc-frontend-manager' ),
			'add_to_gallery' => __( 'Add to Gallery', 'wc-frontend-manager' ),
			'is_editable'    => apply_filters( 'wcfm_is_allow_image_edit', true ),
		);
	...
	public function load_daterangepicker_lib() {
		...
		$date_range_picker_lang = array(
			...
			'default-range'   => __( 'Please select a date range between %1$d and %2$d days', 'wc-frontend-manager' ),
	...
	public function load_google_map_lib() {
		...
		if ( $api_key ) {
			...
			wp_localize_script(
				...
				array(
					'lib'      => 'google',
					'map_type' => apply_filters( 'wcfm_google_map_type', 'roadmap' ),
				)
			);
		}
	}
	...
	public function load_mapbox_map_lib() {
		...
		wp_localize_script(
			...
			array(
				'lib'         => 'leaflet',
				'mapboxToekn' => 'sk.eyJ1Ijoid2Nsb3ZlcnMiLCJhIjoiY2s1OTNtcnhhMGhydDNqcmZobmVua2dneCJ9.ZjAIBgbSGmAHpNmBiWoShA',
			)
		);
	}
