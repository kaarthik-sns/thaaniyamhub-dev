...
class WCFM_Ajax {
	...
	function wcfm_add_taxonomy_new_term() {
		...
		if ( taxonomy_exists( $taxonomy ) ) {
			...
			if ( is_wp_error( $result ) ) {
				...
				$author_is_vendor = 1;
				$message_to       = 0;
				$wcfm_messages    = sprintf( __( 'A new %1$s <b>%2$s</b> added to the store by <b>%3$s</b>', 'wc-frontend-manager' ), ucfirst( $taxonomy ), $new_term, get_user_by( 'id', $author_id )->display_name );
	...
	function wcfm_product_approve() {
		...
		if ( isset( $_POST['proid'] ) && ! empty( $_POST['proid'] ) ) {
			...
			$update_product = apply_filters(
				'wcfm_product_content_before_update',
				array(
					'ID'          => $product_id,
					'post_status' => 'publish',
					'post_type'   => 'product',
	...
	public function wcfm_order_mark_complete() {
		...
		if ( wc_is_order_status( 'wc-completed' ) && $order_id ) {
			...
			$wcfm_messages    = sprintf( __( '<b>%1$s</b> order status updated to <b>%2$s</b> by <b>%3$s</b>', 'wc-frontend-manager' ), '#<a target="_blank" class="wcfm_dashboard_item_title" href="' . get_wcfm_view_order_url( $order_id ) . '">' . $order->get_order_number() . '</a>', wc_get_order_status_name( 'completed' ), $shop_name );
	...
	public function wcfm_modify_order_status() {
		...
		$wcfm_messages    = sprintf( __( 'Order status updated to <b>%1$s</b> by <b>%2$s</b>', 'wc-frontend-manager' ), wc_get_order_status_name( $new_status ), $shop_name );
		$is_customer_note = apply_filters( 'wcfm_is_allow_order_update_note_for_customer', '1' );
		...
		$wcfm_messages = sprintf( __( '<b>%1$s</b> order status updated to <b>%2$s</b> by <b>%3$s</b>', 'wc-frontend-manager' ), '#<a target="_blank" class="wcfm_dashboard_item_title" href="' . get_wcfm_view_order_url( $order_id ) . '">' . $order->get_order_number() . '</a>', wc_get_order_status_name( $new_status ), $shop_name );
	...
	function wcfm_json_search_taxonomies() {
		...
		if ( ! empty( $terms ) ) {
			foreach ( $terms as $term ) {
				if ( apply_filters( 'wcfm_is_allow_hide_uncatorized', false, $term->term_id ) && ( ( $term->term_id == $default_category_id ) || ( in_array( $term->slug, array( 'uncategorized', 'uncategorised' ) ) ) ) ) {
					continue;
				}
				if ( ! $parent ) {
	...
	function wcfm_vendor_disable() {
		...
		if ( isset( $_POST['memberid'] ) ) {
			...
			$wcfm_messages = sprintf( __( '<b>%1$s</b> (Store: <b>%2$s</b>) has been disabled.', 'wc-frontend-manager' ), $member_user->first_name, $vendor_store );
	...
	function wcfm_vendor_enable() {
		...
		if ( isset( $_POST['memberid'] ) ) {
			...
			$wcfm_messages = sprintf( __( '<b>%1$s</b> (Store: <b>%2$s</b>) has been enabled.', 'wc-frontend-manager' ), $member_user->first_name, $vendor_store );
	...
	function wcfm_login_popup_submit() {
		...
		$creds = array(
			'user_login'    => trim( wp_unslash( $wcfm_login_popup_form_data['wcfm_login_popup_username'] ) ),  
			'user_password' => $wcfm_login_popup_form_data['wcfm_login_popup_password'],  
			'remember'      => true,  
		);
