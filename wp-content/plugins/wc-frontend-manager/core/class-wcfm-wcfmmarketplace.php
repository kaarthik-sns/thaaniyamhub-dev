...
class WCFM_Marketplace {
	...
	function wcfmmp_store_name( $store_name ) {
		...
		$shop_name = wcfm_get_option( 'wcfm_my_store_label', $store_name );   
		if ( $shop_name ) {
			$store_name = $shop_name;
		}
		$shop_link = wcfmmp_get_store_url( $user_id );
		if ( $shop_name ) {
			$store_name = '<a ' . $store_open_by . ' href="' . apply_filters( 'wcfmmp_vendor_shop_permalink', $shop_link ) . '">' . $shop_name . '</a>'; } else {
			$store_name = '<a ' . $store_open_by . ' href="' . apply_filters( 'wcfmmp_vendor_shop_permalink', $shop_link ) . '">' . __( 'Shop', 'wc-frontend-manager' ) . '</a>'; }
			return $store_name;
	}
			...
			<?php if ( $get_tax ) { ?>
				...
				if ( apply_filters( 'wcfm_is_allow_vendor_order_details_tax_breakup', false ) ) {
					...
					if ( ! empty( $order_taxes ) ) {
						foreach ( $order_taxes as $tax_id => $tax_item ) {
							...
							$tax_breakups[ $tax_item->get_rate_id() ] = array(
								'label'    => $column_label,
								'subtotal' => 0,
								'total'    => 0,
							);
				...
				<tr>
					<th class="label" colspan="2" style="text-align:right; <?php echo esc_attr( $td_style ); ?>">
						<?php
						if ( apply_filters( 'wcfm_is_allow_vendor_order_details_tax_breakup', false ) ) {
							echo esc_html__( 'Total', 'wc-frontend-manager' ) . ' '; }
						?>
