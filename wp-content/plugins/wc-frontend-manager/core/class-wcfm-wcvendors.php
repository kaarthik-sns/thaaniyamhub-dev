...
class WCFM_WCVendors {
	...
	function wcvendors_store_name( $store_name ) {
		...
		$shop_link = WCV_Vendors::get_vendor_shop_page( wp_get_current_user()->user_login );
		if ( $shop_name ) {
			$store_name = '<a target="_blank" href="' . apply_filters( 'wcv_vendor_shop_permalink', $shop_link ) . '">' . $shop_name . '</a>'; } else {
			$store_name = '<a target="_blank" href="' . apply_filters( 'wcv_vendor_shop_permalink', $shop_link ) . '">' . __( 'Shop', 'wc-frontend-manager' ) . '</a>'; }
			return $store_name;
	}
	...
	function wcvendors_wcfm_add_product_url( $submit_link ) {
		...
		if ( $can_submit ) {
			$submit_link = get_wcfm_edit_product_url();
		}
		return $submit_link;
	}
	...
	function wcvendors_wcfm_edit_product_url( $edit_link ) {
		...
		if ( $can_submit ) {
			$edit_link = get_wcfm_products_url();
		}
		return $edit_link;
	}
	...
	function wcvendors_wcfm_dashboard_quick_links( $quick_links ) {
		if ( isset( $quick_links['shop_coupon'] ) ) {
			$quick_links['shop_coupon']['url'] = get_wcfm_coupons_manage_url();
		}
		return $quick_links;
	}
	...
	function wcvendors_wcfm_dashboard_pages_nav( $navs ) {
		...
		if ( isset( $navs['product'] ) ) {
			$navs['product']['slug'] = get_wcfm_products_url();
		}
		if ( isset( $navs['shop_coupon'] ) ) {
			$navs['shop_coupon']['slug'] = get_wcfm_coupons_url();
		}
		if ( isset( $navs['order'] ) ) {
			$navs['order']['slug'] = get_wcfm_orders_url();
		}
	...
	function wcvendors_is_allow_product_types( $product_types ) {
		...
		if ( ! empty( $product_panel['attribute'] ) ) {
	...
	function wcvendors_is_allow_inventory( $allow ) {
		...
		if ( ! empty( $product_panel['inventory'] ) ) {
			return false;
		}
		return $allow;
	}
	...
	function wcvendors_is_allow_shipping( $allow ) {
		...
		if ( ! empty( $product_panel['shipping'] ) ) {
			return false;
		}
		return $allow;
	}
	...
	function wcvendors_is_allow_linked( $allow ) {
		...
		if ( ! empty( $product_panel['linked_product'] ) ) {
			return false;
		}
		return $allow;
	}
