...
class WCFM_Capability {
	...
	public function __construct() {
		...
		add_filter( 'product_type_selector', array( &$this, 'wcfmcap_is_allow_product_types' ), 500 );  
		...
		add_filter( 'woocommerce_product_export_product_default_columns', array( &$this, 'wcfmcap_is_allow_product_columns' ), 500 );  
		...
		add_filter( 'show_customer_billing_address_in_export_orders', array( &$this, 'wcfmcap_is_allow_customer_billing_details' ), 500 );  
		add_filter( 'show_customer_shipping_address_in_export_orders', array( &$this, 'wcfmcap_is_allow_customer_shipping_details' ), 500 );  
		...
		add_filter( 'show_customer_details_in_export_orders', array( &$this, 'wcfmcap_is_allow_customer_details_orders' ), 500 );  
	...
	function wcfmcap_wcfm_menus( $menus ) {
		...
		if ( $manage_products == 'yes' ) {
			unset( $menus['wcfm-products'] );
		}
		if ( ( $manage_coupons == 'yes' ) ) {
		...
		if ( $manage_booking == 'yes' ) {
			unset( $menus['wcfm-bookings-dashboard'] );
		}

		if ( ! current_user_can( 'administrator' ) ) {
			unset( $menus['wcfm-capability'] );
		}
	...
	function wcfmcap_is_allow_product_limit( $allow ) {
		...
		if ( ( $productlimit == -1 ) || ( $productlimit == '-1' ) ) {
			...
			if ( $productlimit && ( $productlimit >= 0 ) ) {
				...
				if ( $productlimit <= $count_products ) {
					return false;
				}
			}
		}
	...
	function wcfmcap_products_limit_label( $label ) {
		...
		if ( current_user_can( 'administrator' ) || ! apply_filters( 'wcfm_is_allow_limit_label', true ) || ! apply_filters( 'wcfm_is_allow_product_limit_label', true ) ) {
			return '';
		}
	...
	function wcfmcap_is_allow_product_types( $product_types ) {
		...
		if ( $booking == 'yes' ) {
			unset( $product_types['booking'] );
		}
		if ( $accommodation == 'yes' ) {
			unset( $product_types['accommodation-booking'] );
		}
		if ( $appointment == 'yes' ) {
			unset( $product_types['appointment'] );
		}
		if ( $job_package == 'yes' ) {
			unset( $product_types['job_package'] );
		}
		if ( $resume_package == 'yes' ) {
			unset( $product_types['resume_package'] );
		}
		if ( $auction == 'yes' ) {
			unset( $product_types['auction'] );
		}
		if ( $rental == 'yes' ) {
			unset( $product_types['redq_rental'] );
		}
		if ( $lottery == 'yes' ) {
			unset( $product_types['lottery'] );
		}
		if ( $subscription == 'yes' ) {
			unset( $product_types['subscription'] );
		}
		if ( $variable_subscription == 'yes' ) {
			unset( $product_types['variable-subscription'] );
		}
		if ( $attributes == 'yes' ) {
			unset( $product_types['variable'] );
		}
	...
	function wcfmcap_is_allow_fields_images( $image_fields ) {
		...
		if ( $gallery_img == 'yes' ) {
			if ( isset( $image_fields['gallery_img'] ) ) {
				unset( $image_fields['gallery_img'] );
			}
		}
		return $image_fields;
	}
	...
	function wcfmcap_is_allow_variable( $allow ) {
		...
		if ( ( $attributes == 'yes' ) && ( $variable == 'yes' ) && ( $variable_subscription == 'yes' ) ) {
			return false;
		}
		return $allow;
	}
	...
	function wcfmcap_is_allow_article_limit( $allow ) {
		...
		if ( ( $articlelimit == -1 ) || ( $articlelimit == '-1' ) ) {
			...
			if ( $articlelimit ) {
				$articlelimit = absint( $articlelimit );
			}
			if ( $articlelimit && ( $articlelimit >= 0 ) ) {
				...
				if ( $articlelimit <= $count_articles ) {
					return false;
				}
			}
		}
	...
	function wcfmcap_articles_limit_label( $label ) {
		...
		if ( current_user_can( 'administrator' ) || ! apply_filters( 'wcfm_is_allow_limit_label', true ) || ! apply_filters( 'wcfm_is_allow_article_limit_label', true ) ) {
			return '';
		}
	...
	function wcfmcap_is_allow_product_columns( $product_columns ) {
		...
		if ( $inventory == 'yes' ) {
			unset( $product_columns['stock_status'] );
		}
		if ( $inventory == 'yes' ) {
			unset( $product_columns['stock'] );
		}
		if ( $inventory == 'yes' ) {
			unset( $product_columns['backorders'] );
		}
		if ( $inventory == 'yes' ) {
			unset( $product_columns['sold_individually'] );
		}

		if ( $shipping == 'yes' ) {
			unset( $product_columns['weight'] );
		}
		if ( $shipping == 'yes' ) {
			unset( $product_columns['length'] );
		}
		if ( $shipping == 'yes' ) {
			unset( $product_columns['width'] );
		}
		if ( $shipping == 'yes' ) {
			unset( $product_columns['height'] );
		}
		if ( $shipping == 'yes' ) {
			unset( $product_columns['shipping_class_id'] );
		}

		if ( $taxes == 'yes' ) {
			unset( $product_columns['tax_status'] );
		}
		if ( $taxes == 'yes' ) {
			unset( $product_columns['tax_class'] );
		}

		 

		if ( $advanced == 'yes' ) {
			unset( $product_columns['reviews_allowed'] );
		}
		if ( $advanced == 'yes' ) {
			unset( $product_columns['purchase_note'] );
		}

		if ( $linked == 'yes' ) {
			unset( $product_columns['upsell_ids'] );
		}
		if ( $linked == 'yes' ) {
			unset( $product_columns['cross_sell_ids'] );
		}

		if ( $downloadable == 'yes' ) {
			unset( $product_columns['download_limit'] );
		}
		if ( $downloadable == 'yes' ) {
			unset( $product_columns['download_expiry'] );
		}

		if ( $grouped == 'yes' ) {
			unset( $product_columns['grouped_products'] );
		}
		if ( $external == 'yes' ) {
			unset( $product_columns['product_url'] );
		}
		if ( $external == 'yes' ) {
			unset( $product_columns['button_text'] );
		}

		if ( $gallery == 'yes' ) {
			unset( $product_columns['images'] );
		}

		if ( $category == 'yes' ) {
			unset( $product_columns['category_ids'] );
		}
		if ( $tags == 'yes' ) {
			unset( $product_columns['tag_ids'] );
		}
	...
	function wcfmcap_is_allow_customer_limit( $allow ) {
		...
		if ( ( $customerlimit == -1 ) || ( $customerlimit == '-1' ) ) {
			...
			if ( $customerlimit ) {
				$customerlimit = absint( $customerlimit );
			}
			if ( $customerlimit && ( $customerlimit >= 0 ) ) {
				...
				if ( $customerlimit <= $count_customers ) {
					return false;
				}
			}
		}
	...
	function wcfmcap_customers_limit_label( $label ) {
		...
		if ( current_user_can( 'administrator' ) || ! apply_filters( 'wcfm_is_allow_limit_label', true ) || ! apply_filters( 'wcfm_is_allow_customer_limit_label', true ) ) {
			return '';
		}
	...
	function wcfmcap_is_allow_transaction_details( $allow ) {
		...
		if ( $vendor_transaction_details == 'yes' ) {
			return false;
		}
		return $allow;
	}
