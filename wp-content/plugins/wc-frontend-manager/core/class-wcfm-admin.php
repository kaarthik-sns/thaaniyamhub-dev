...
class WCFM_Admin {
	...
	function wcfm_view_metabox( $post ) {
		...
		if ( $post->ID && $post->post_type ) {
			if ( $post->post_type == 'product' ) {
				$wcfm_url = get_wcfm_edit_product_url( $post->ID );
			} elseif ( $post->post_type == 'shop_coupon' ) {
				$wcfm_url = get_wcfm_coupons_manage_url( $post->ID );
			} elseif ( 'shop_order' === OrderUtil::get_order_type( $post->ID ) ) {
				$wcfm_url = get_wcfm_view_order_url( $post->ID );
			}
		}
	...
	function wcfm_section_developers_cb( $args ) {
		...
		<p><?php printf( __( 'WCFM totally works from front-end ... check dashboard settings %1$shere >>%2$s', 'wc-frontend-manager' ), '<a class="primary" target="_blank" href="' . get_wcfm_settings_url() . '">', '</a>' ); ?></p>
		</div>
		<?php
