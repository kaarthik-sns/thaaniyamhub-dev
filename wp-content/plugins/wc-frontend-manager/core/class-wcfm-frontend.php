...
class WCFM_Frontend {
	...
	function wcfm_dashboard_template( $page_template ) {
		...
		if ( wc_post_content_has_shortcode( 'wc_frontend_manager' ) && is_user_logged_in() ) {
			...
			if ( $is_dashboard_full_view_disabled != 'yes' ) {
				...
				if ( $is_dashboard_theme_header_disabled == 'yes' ) {
	...
	function wcfm_home() {
		...
		if ( $is_quick_access_disabled == 'yes' ) {
			return;
		}
		...
		if ( ! apply_filters( 'wcfm_is_allow_catalog_quick_access', true ) ) {
			return;
		}
		$user          = wp_get_current_user();
		$allowed_roles = apply_filters( 'wcfm_allwoed_user_roles', array( 'administrator', 'shop_manager' ) );
	...
	function wcfm_product_manage() {
		...
		if ( ! apply_filters( 'wcfm_is_allow_catalog_product_manage', true ) ) {
			return;
		}
		$user          = wp_get_current_user();
		$allowed_roles = apply_filters( 'wcfm_allwoed_user_roles', array( 'administrator', 'shop_manager' ) );
		...
		if ( wcfm_is_vendor() && ( $current_user != $post->post_author ) ) {
			return;
		}
	...
	function wcfm_is_category_checklist( $is_checklist_view ) {
		...
		if ( $is_checklist_view_disabled == 'yes' ) {
	...
	function wcfm_save_page_analytics_data() {
				...
				if ( ! is_wp_error( $request ) && wp_remote_retrieve_response_code( $request ) == 200 ) {
					...
					if ( $data && is_object( $data ) ) {
						...
						if ( isset( $data['countryCode'] ) ) {
							$_SESSION['location']['country'] = $data['countryCode'];
						}
						if ( isset( $data['region'] ) ) {
							$_SESSION['location']['state'] = $data['region'];
						}
						if ( isset( $data['city'] ) ) {
							$_SESSION['location']['city'] = $data['city'];
						}
		...
		if ( is_shop() ) {
			...
			if ( ! isset( $_SESSION['wcfm_pages'] ) || ! isset( $_SESSION['wcfm_pages']['listings'] ) || ( isset( $_SESSION['wcfm_pages'] ) && isset( $_SESSION['wcfm_pages']['listings'] ) && ! in_array( $post->ID, $_SESSION['wcfm_pages']['listings'] ) ) ) {
				...
				if ( ! $wcfm_listing_views ) {
					$wcfm_listing_views = 1;
				} else {
					$wcfm_listing_views += 1;
				}
			...
			if ( ! isset( $_SESSION['wcfm_pages'] ) || ! isset( $_SESSION['wcfm_pages']['articles'] ) || ( isset( $_SESSION['wcfm_pages'] ) && isset( $_SESSION['wcfm_pages']['articles'] ) && ! in_array( $post->ID, $_SESSION['wcfm_pages']['articles'] ) ) ) {
				...
				if ( ! $wcfm_article_views ) {
					$wcfm_article_views = 1;
				} else {
					$wcfm_article_views += 1;
				}
	...
	function is_wcfmu_inactive_notice_show( $show ) {
		...
		if ( $is_ultimate_notice_disabled == 'yes' ) {
			$show = false;
		}
		if ( wcfm_is_vendor() ) {
			$show = false;
		...
		if ( ! empty( $attribute_taxonomies ) ) {
			foreach ( $attribute_taxonomies as $attribute_taxonomy ) {
				if ( ( 'text' !== $attribute_taxonomy->attribute_type ) && $attribute_taxonomy->attribute_name ) {
					...
					if ( ! apply_filters( 'wcfm_is_allowed_attributes', true, $att_taxonomy ) ) {
						continue;
					}
					...
					$WCFM->wcfm_fields->wcfm_generate_form_field(
						apply_filters(
							'wcfm_product_custom_attributes',
							apply_filters(
								'wcfm_product_custom_attribute_' . $att_taxonomy,
								array(
									'select_attributes_' . $att_taxonomy => array(
										'type'        => 'multiinput',
										'class'       => 'wcfm-text wcfm_select_attributes wcfm_ele simple variable external grouped booking',
										...
										'options'     => array(
											...
											'is_active'    => array(
												...
												'type'  => 'checkbox',
												'value' => 'enable',
												'attributes' => array( 'title' => __( 'Check to associate this attribute with the product', 'wc-frontend-manager' ) ),
											...
											'name'         => array(
												...
												'class' => 'variation_ele_hide',
												'label_class' => 'variation_ele_hide',
											),
											'value'        => array(
												'label' => wc_attribute_label( $att_taxonomy ),
												'type'  => 'select',
												'custom_attributes' => array( 'attrlimit' => $attrlimit ),
												'attributes' => array(
													'multiple' => 'multiple',
													'style'    => 'width: 60%;',
												),
												'class' => 'wcfm-select wcfm_ele simple variable external grouped booking ' . $allow_add_term,
												'label_class' => 'wcfm_title wcfm_ele attribute_ele simple variable external grouped booking',
											),
											'is_visible'   => array(
								...
								'select_attributes_' . $att_taxonomy,
								$att_taxonomy,
								$attributes
							),
							'select_attributes_' . $att_taxonomy,
