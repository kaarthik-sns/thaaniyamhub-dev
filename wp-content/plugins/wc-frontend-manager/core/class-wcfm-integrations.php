...
class WCFM_Integrations {
	...
	function wcfm_thirdparty_query_vars( $query_vars ) {
		...
		if ( $wcfm_allow_listings = apply_filters( 'wcfm_is_allow_listings', true ) ) {
			...
			} elseif ( get_option( 'wcfm_updated_end_point_wc_listings' ) ) {
	...
	function wcfmcap_product_types( $product_types, $handler = 'wcfm_capability_options', $wcfm_capability_options = array() ) {
		...
		if ( WCFM_Dependencies::wcfm_wc_paid_listing_active_check() ) {
			...
			$product_types['job_package'] = array(
				'label'       => __( 'Listing Package', 'wc-frontend-manager' ),
				'name'        => $handler . '[job_package]',
				'type'        => 'checkboxoffon',
		...
		if ( WCFM_Dependencies::wcfm_yith_auction_free_active_check() ) {
			...
			$product_types['auction'] = array(
				'label'       => __( 'Auction', 'wc-frontend-manager' ),
	...
	function wcfm_add_listing_page( $content ) {
		...
		if ( ( $add_listings_page && is_object( $post ) && ( $add_listings_page == $post->ID ) ) || ( $job_dashboard_page && is_object( $post ) && ( $job_dashboard_page == $post->ID ) && isset( $_GET['action'] ) && ( in_array( $_GET['action'], array( 'edit', 'mark_filled', 'mark_not_filled' ) ) ) ) ) {
			...
			} elseif ( isset( $_GET['action'] ) && ( in_array( $_GET['action'], array( 'mark_filled', 'mark_not_filled' ) ) ) ) {
	...
	function wcfm_wcpl_product_manage_fields_pricing( $general_fields, $product_id, $product_type, $wcfm_is_translated_product = false, $wcfm_wpml_edit_disable_element = '' ) {
		...
		$job_listing_package_fields = array(
			'_job_listing_package_subscription_type' => array(
				...
				'class'       => 'wcfm-select wcfm_ele job_package_price_ele job_package' . ' ' . $wcfm_wpml_edit_disable_element,
				'label_class' => 'wcfm_title wcfm_ele job_package' . ' ' . $wcfm_wpml_edit_disable_element,
				'hints'       => __( 'Choose how subscriptions affect this package', 'wp-job-manager-wc-paid-listings' ),
				'value'       => $_job_listing_package_subscription_type,
			),
			'_job_listing_limit'                     => array(
				'label'       => __( 'Job listing limit', 'wp-job-manager-wc-paid-listings' ),
				'placeholder' => __( 'Unlimited', 'wc-frontend-manager' ),
				'type'        => 'number',
				'class'       => 'wcfm-text wcfm_ele job_package_price_ele job_package' . ' ' . $wcfm_wpml_edit_disable_element,
				'label_class' => 'wcfm_title wcfm_ele job_package' . ' ' . $wcfm_wpml_edit_disable_element,
				'attributes'  => array(
					'min'  => '',
					'step' => '1',
				),
				'hints'       => __( 'The number of job listings a user can post with this package.', 'wp-job-manager-wc-paid-listings' ),
				'value'       => $_job_listing_limit,
			),
			'_job_listing_duration'                  => array(
				...
				'hints'       => __( 'The number of days that the job listing will be active.', 'wp-job-manager-wc-paid-listings' ),
				'value'       => $_job_listing_duration,
			),
			'_job_listing_featured'                  => array(
				'label'       => __( 'Feature Listings?', 'wp-job-manager-wc-paid-listings' ),
				'type'        => 'checkbox',
				'class'       => 'wcfm-checkbox wcfm_ele job_package_price_ele job_package' . ' ' . $wcfm_wpml_edit_disable_element,
				'label_class' => 'wcfm_title checkbox_title wcfm_ele job_package' . ' ' . $wcfm_wpml_edit_disable_element,
				'hints'       => __( 'Feature this job listing - it will be styled differently and sticky.', 'wp-job-manager-wc-paid-listings' ),
				'value'       => 'yes',
				'dfvalue'     => $_job_listing_featured,
			),
		);
		...
		<div class="wcfm-container redq_rental non-variable-subscription">
			<div id="wcfm_products_manage_form_redq_rental_expander" class="wcfm-content">
				...
				$WCFM->wcfm_fields->wcfm_generate_form_field(
					apply_filters(
						'wcfm_redq_rental_fields',
						array(
							'pricing_type'  => array(
								'label'       => __( 'Set Price Type', 'wc-frontend-manager' ),
								'type'        => 'select',
								'options'     => apply_filters( 'wcfm_redq_rental_pricing_options', array( 'general_pricing' => __( 'General Pricing', 'wc-frontend-manager' ) ) ),
								'class'       => 'wcfm-select wcfm_ele redq_rental',
								'label_class' => 'wcfm_title redq_rental',
								'value'       => $pricing_type,
								'hints'       => __( 'Choose a price type - this controls the schema.', 'wc-frontend-manager' ),
							),
							'hourly_price'  => array(
								'label'       => __( 'Hourly Price', 'wc-frontend-manager' ) . '(' . get_woocommerce_currency_symbol() . ')',
								'type'        => 'number',
								'class'       => 'wcfm-text wcfm_ele redq_rental',
								'label_class' => 'wcfm_title redq_rental',
								'value'       => $hourly_price,
								'hints'       => __( 'Hourly price will be applicabe if booking or rental days min 1day', 'wc-frontend-manager' ),
								'placeholder' => __( 'Enter price here', 'wc-frontend-manager' ),
							),
							'general_price' => array(
								'label'       => __( 'General Price', 'wc-frontend-manager' ) . '(' . get_woocommerce_currency_symbol() . ')',
								'type'        => 'number',
								'class'       => 'wcfm-text wcfm_ele rentel_pricing rental_general_pricing redq_rental',
					...
					'redq_rental_availability' => array(
						...
						'options'     => array(
							'type'     => array(
								...
								'type'        => 'select',
								'options'     => array( 'custom_date' => __( 'Custom Date', 'wc-frontend-manager' ) ),
								'class'       => 'wcfm-select wcfm_ele avail_range_type redq_rental',
								'label_class' => 'wcfm_title avail_rules_ele avail_rules_label redq_rental',
							),
							'from'     => array(
							...
							'rentable' => array(
								...
								'type'        => 'select',
								'options'     => array( 'no' => __( 'NO', 'wc-frontend-manager' ) ),
								'class'       => 'wcfm-select wcfm_ele avail_rules_ele avail_rules_text redq_rental',
	...
	function wcfm_yithauction_free_product_manage_fields( $product_id = 0, $product_type = null ) {
		...
		<div class="wcfm-container auction non-variable-subscription">
			<div id="wcfm_products_manage_form_yithauction_free_expander" class="wcfm-content">
				...
				$WCFM->wcfm_fields->wcfm_generate_form_field(
					array(
						'_yith_auction_for' => array(
							'label'       => __( 'Auction Date From', 'wc-frontend-manager' ),
							'type'        => 'text',
							'placeholder' => 'YYYY-MM-DD hh:mm:ss',
							'class'       => 'wcfm-text wcfm_ele auction',
							'label_class' => 'wcfm_title auction',
							'value'       => $_yith_auction_for,
							'attributes'  => array( 'pattern' => '[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01]) (0[0-9]|1[0-9]|2[0-3]):(0[0-9]|1[0-9]|2[0-9]|3[0-9]|4[0-9]|5[0-9]):(0[0-9]|1[0-9]|2[0-9]|3[0-9]|4[0-9]|5[0-9])' ),
						),
						'_yith_auction_to'  => array(
							'label'       => __( 'Auction Date To', 'wc-frontend-manager' ),
							'type'        => 'text',
							'placeholder' => 'YYYY-MM-DD hh:mm:ss',
							'class'       => 'wcfm-text wcfm_ele auction',
							'label_class' => 'wcfm_title auction',
	...
	function wcfm_wc_product_voucher_product_manage_fields_general( $general_fields, $product_id, $product_type, $wcfm_is_translated_product = false, $wcfm_wpml_edit_disable_element = '' ) {
		...
		$general_fields = array_slice( $general_fields, 0, 1, true ) +
													array(
														'_has_voucher' => array(
															'desc'       => __( 'Has Voucher', 'wc-frontend-manager' ),
															'type'       => 'checkbox',
															'class'      => 'wcfm-checkbox wcfm_ele wcfm_half_ele_checkbox simple non-variable-subscription non-job_package non-resume_package non-auction non-redq_rental non-appointment non-accommodation-booking' . ' ' . $wcfm_wpml_edit_disable_element,
															'desc_class' => 'wcfm_title wcfm_ele downloadable_ele_title checkbox_title simple non-variable-subscription non-job_package non-resume_package non-auction non-redq_rental non-appointment non-accommodation-booking' . ' ' . $wcfm_wpml_edit_disable_element,
															'value'      => 'yes',
															'dfvalue'    => $_has_voucher,
														),
													) +
	...
	function wcfm_wc_product_voucher_product_manage_fields_pricing( $pricing_fields, $product_id, $product_type, $wcfm_is_translated_product = false, $wcfm_wpml_edit_disable_element = '' ) {
		...
		$wcfm_voucher_template_fields = array(
			'_voucher_template_id' => array(
				'label'       => __( 'Voucher Template', 'wc-frontend-manager' ),
				'type'        => 'select',
				'options'     => $wc_voucher_templates_array,
				'class'       => 'wcfm-select wcfm_ele simple non-variable-subscription non-job_package non-resume_package non-auction non-redq_rental non-appointment non-accommodation-booking' . ' ' . $wcfm_wpml_edit_disable_element,
				'label_class' => 'wcfm_title simple non-variable-subscription non-job_package non-resume_package non-auction non-redq_rental non-appointment non-accommodation-booking' . ' ' . $wcfm_wpml_edit_disable_element,
				'value'       => $_voucher_template_id,
				'hints'       => __( 'Select a voucher template to make this into a voucher product.', 'wc-frontend-manager' ),
			),
		);
	...
	function wcfm_woocommerce_germanized_product_manage_fields_general( $general_fields, $product_id, $product_type, $wcfm_is_translated_product = false, $wcfm_wpml_edit_disable_element = '' ) {
		...
		$general_fields = array_slice( $general_fields, 0, 1, true ) +
													array(
														'_service' => array(
															'desc'       => __( 'Service', 'woocommerce-germanized' ),
															'type'       => 'checkbox',
															'class'      => 'wcfm-checkbox wcfm_ele wcfm_half_ele_checkbox simple non-variable-subscription non-job_package non-resume_package non-auction non-redq_rental non-appointment non-accommodation-booking' . ' ' . $wcfm_wpml_edit_disable_element,
															'desc_class' => 'wcfm_title wcfm_ele virtual_ele_title checkbox_title simple non-variable-subscription non-job_package non-resume_package non-auction non-redq_rental non-appointment non-accommodation-booking' . ' ' . $wcfm_wpml_edit_disable_element,
															'value'      => 'yes',
															'dfvalue'    => $_service,
														),
														'_differential_taxation' => array(
															'desc'       => __( 'Diff. Taxation', 'woocommerce-germanized' ),
															'type'       => 'checkbox',
															'class'      => 'wcfm-checkbox wcfm_ele simple variable external groupd wcfm_half_ele_checkbox simple non-variable-subscription non-job_package non-resume_package non-auction non-redq_rental non-appointment non-accommodation-booking' . ' ' . $wcfm_wpml_edit_disable_element,
															'desc_class' => 'wcfm_title wcfm_ele simple variable external groupd downloadable_ele_title checkbox_title simple non-variable-subscription non-job_package non-resume_package non-auction non-redq_rental non-appointment non-accommodation-booking' . ' ' . $wcfm_wpml_edit_disable_element,
															'value'      => 'yes',
															'dfvalue'    => $_differential_taxation,
														),
													) +
											array_slice( $general_fields, 1, count( $general_fields ) - 1, true );
	...
	function wcfm_woocommerce_germanized_product_manage_fields_content( $content_fields, $product_id, $product_type ) {
		...
		$woocommerce_germanized_content_fields = array(
			'_mini_desc' => array(
				'label'       => __( 'Optional Mini Description', 'woocommerce-germanized' ),
				'type'        => 'textarea',
				'class'       => 'wcfm-textarea wcfm_ele wcfm_full_ele simple external non-variable-subscription non-auction non-redq_rental non-accommodation-booking',
				'label_class' => 'wcfm_ele wcfm_title simple external non-variable-subscription non-auction non-redq_rental non-accommodation-booking',
	...
	function wcfm_woocommerce_germanized_product_manage_fields_pricing( $pricing_fields, $product_id, $product_type, $wcfm_is_translated_product = false, $wcfm_wpml_edit_disable_element = '' ) {
		...
		$woocommerce_germanized_pricing_fields = array(
			'_sale_price_label'         => array(
				...
				'class'       => 'wcfm-select wcfm_ele simple variable external non-variable-subscription non-auction non-redq_rental non-accommodation-booking' . ' ' . $wcfm_wpml_edit_disable_element,
				'label_class' => 'wcfm_ele wcfm_title simple variable external non-variable-subscription non-auction non-redq_rental non-accommodation-booking' . ' ' . $wcfm_wpml_edit_disable_element,
				'hints'       => __( 'If the product is on sale you may want to show a price label right before outputting the old price to inform the customer.', 'woocommerce-germanized' ),
				'value'       => $_sale_price_label,
			),
			'_sale_price_regular_label' => array(
				'label'       => __( 'Sale Regular Label', 'woocommerce-germanized' ),
				'type'        => 'select',
				'options'     => array_merge( array( '-1' => __( 'Select Price Label', 'woocommerce-germanized' ) ), WC_germanized()->price_labels->get_labels() ),
				'class'       => 'wcfm-select wcfm_ele simple variable external non-variable-subscription non-auction non-redq_rental non-accommodation-booking' . ' ' . $wcfm_wpml_edit_disable_element,
				'label_class' => 'wcfm_ele wcfm_title simple variable external non-variable-subscription non-auction non-redq_rental non-accommodation-booking' . ' ' . $wcfm_wpml_edit_disable_element,
				'hints'       => __( 'If the product is on sale you may want to show a price label right before outputting the new price to inform the customer.', 'woocommerce-germanized' ),
				'value'       => $_sale_price_regular_label,
			),
			'_unit'                     => array(
				...
				'class'       => 'wcfm-select wcfm_ele simple variable external non-variable-subscription non-auction non-redq_rental non-accommodation-booking' . ' ' . $wcfm_wpml_edit_disable_element,
				'label_class' => 'wcfm_ele wcfm_title simple variable external non-variable-subscription non-auction non-redq_rental non-accommodation-booking' . ' ' . $wcfm_wpml_edit_disable_element,
				'hints'       => __( 'Needed if selling on a per unit basis', 'woocommerce-germanized' ),
				'value'       => $_unit,
			),
			'_unit_product'             => array(
				'label'       => __( 'Product Units', 'woocommerce-germanized' ),
				'type'        => 'text',
				'class'       => 'wcfm-text wcfm_ele simple variable external non-variable-subscription non-auction non-redq_rental non-accommodation-booking' . ' ' . $wcfm_wpml_edit_disable_element,
				'label_class' => 'wcfm_ele wcfm_title variable simple external non-variable-subscription non-auction non-redq_rental non-accommodation-booking' . ' ' . $wcfm_wpml_edit_disable_element,
			...
			'_unit_base'                => array(
				...
				'class'       => 'wcfm-text wcfm_ele simple variable external non-variable-subscription non-auction non-redq_rental non-accommodation-booking' . ' ' . $wcfm_wpml_edit_disable_element,
				'label_class' => 'wcfm_ele wcfm_title variable simple external non-variable-subscription non-auction non-redq_rental non-accommodation-booking' . ' ' . $wcfm_wpml_edit_disable_element,
				'hints'       => __( 'Base price units. Example base price: 0,99 € / 100 ml. Insert 100 as base price unit amount.', 'woocommerce-germanized' ),
				'value'       => $_unit_base,
			),
			'_unit_price_auto'          => array(
				'label'       => __( 'Calculation', 'woocommerce-germanized' ),
				'type'        => 'checkbox',
				'class'       => 'wcfm-checkbox wcfm_ele simple variable external non-variable-subscription non-auction non-redq_rental non-accommodation-booking' . ' ' . $wcfm_wpml_edit_disable_element,
				'label_class' => 'wcfm_ele variable checkbox_title wcfm_title simple variable external non-variable-subscription non-auction non-redq_rental non-accommodation-booking' . ' ' . $wcfm_wpml_edit_disable_element,
				'hints'       => __( 'Calculate base prices automatically.', 'woocommerce-germanized' ),
				'dfvalue'     => $_unit_price_auto,
				'value'       => 'yes',
			),
			...
			'_ts_gtin'                  => array(
				'label'       => _x( 'GTIN', 'trusted-shops', 'woocommerce-germanized' ),
				'type'        => 'text',
				'class'       => 'wcfm-text wcfm_ele simple variable external non-variable-subscription non-auction non-redq_rental non-accommodation-booking' . ' ' . $wcfm_wpml_edit_disable_element,
				'label_class' => 'wcfm_ele wcfm_title simple variable external non-variable-subscription non-auction non-redq_rental non-accommodation-booking' . ' ' . $wcfm_wpml_edit_disable_element,
			...
			'_ts_mpn'                   => array(
				'label'       => _x( 'MPN', 'trusted-shops', 'woocommerce-germanized' ),
				'type'        => 'text',
				'class'       => 'wcfm-text wcfm_ele simple variable external non-variable-subscription non-auction non-redq_rental non-accommodation-booking' . ' ' . $wcfm_wpml_edit_disable_element,
				'label_class' => 'wcfm_ele wcfm_title simple variable external non-variable-subscription non-auction non-redq_rental non-accommodation-booking' . ' ' . $wcfm_wpml_edit_disable_element,
				'hints'       => _x( 'If you don\'t have a GTIN for your products, you can pass the brand name and the MPN on to Google to use the Trusted Shops Google Integration.', 'trusted-shops', 'woocommerce-germanized' ),
				'value'       => $_ts_mpn,
			),
	...
	function wcfm_woocommerce_germanized_product_manage_fields_shipping( $shipping_fields, $product_id ) {
		...
		if ( $product_id ) {
			...
			if ( $delivery_time ) {
				$delivery_time = $delivery_time->term_id;
			}
		}
		...
		$woocommerce_germanized_shipping_fields = array(
			'delivery_time'  => array(
				'label'       => __( 'Delivery Time', 'woocommerce-germanized' ),
				'type'        => 'select',
				'options'     => $delivery_time_list,
				'class'       => 'wcfm-select wcfm_ele simple external non-variable-subscription non-auction non-redq_rental non-accommodation-booking',
				'label_class' => 'wcfm_ele wcfm_title simple external non-variable-subscription non-auction non-redq_rental non-accommodation-booking',
				'value'       => $delivery_time,
			),
			'_free_shipping' => array(
				'label'       => __( 'Free shipping?', 'woocommerce-germanized' ),
				'type'        => 'checkbox',
				'class'       => 'wcfm-checkbox wcfm_ele simple variable external non-variable-subscription non-auction non-redq_rental non-accommodation-booking',
				'label_class' => 'wcfm_ele checkbox_title wcfm_title simple variable external non-variable-subscription non-auction non-redq_rental non-accommodation-booking',
				'hints'       => __( 'This option disables the "plus shipping costs" notice on product page', 'woocommerce-germanized' ),
				'value'       => 'yes',
				'dfvalue'     => $_free_shipping,
			),
		);
	...
	function wcfm_woocommerce_germanized_product_manage_fields_variations( $variation_fileds, $variations, $variation_shipping_option_array, $variation_tax_classes_options ) {
		...
		$woocommerce_germanized_fields = array(
			'_sale_price_label'         => array(
				...
				'type'        => 'select',
				'options'     => array_merge( array( '-1' => __( 'Select Price Label', 'woocommerce-germanized' ) ), WC_germanized()->price_labels->get_labels() ),
				'class'       => 'wcfm-select wcfm_ele simple variable external non-variable-subscription non-auction non-redq_rental non-accommodation-booking',
				'label_class' => 'wcfm_ele wcfm_title simple variable external non-variable-subscription non-auction non-redq_rental non-accommodation-booking',
				'hints'       => __( 'If the product is on sale you may want to show a price label right before outputting the old price to inform the customer.', 'woocommerce-germanized' ),
			),
			'_sale_price_regular_label' => array(
				'label'       => __( 'Sale Regular Label', 'woocommerce-germanized' ),
			...
			'_unit_product'             => array(
				'label'       => __( 'Product Units', 'woocommerce-germanized' ),
				'type'        => 'text',
				'class'       => 'wcfm-text wcfm_ele simple variable external non-variable-subscription non-auction non-redq_rental non-accommodation-booking',
				'label_class' => 'wcfm_ele wcfm_title simple variable external non-variable-subscription non-auction non-redq_rental non-accommodation-booking',
				'hints'       => __( 'Number of units included per default product price. Example: 1000 ml.', 'woocommerce-germanized' ),
			),
			'_unit_price_regular'       => array(
				'label'       => __( 'Regular Base Price', 'woocommerce-germanized' ) . ' (' . get_woocommerce_currency_symbol() . ')',
				'type'        => 'text',
				'class'       => 'wcfm-text wcfm_ele variable non-variable-subscription non-auction non-redq_rental non-accommodation-booking',
				'label_class' => 'wcfm_ele wcfm_title variable non-variable-subscription non-auction non-redq_rental non-accommodation-booking',
			),
			'_unit_price_sale'          => array(
				'label'       => __( 'Sale Base Price', 'woocommerce-germanized' ) . ' (' . get_woocommerce_currency_symbol() . ')',
				'type'        => 'text',
			...
			'_min_age'                  => array(
				...
				'type'        => 'select',
				'options'     => $age_select,
				'class'       => 'wcfm-select wcfm_ele variable non-variable-subscription non-auction non-redq_rental non-accommodation-booking',
				'label_class' => 'wcfm_ele wcfm_title variable non-variable-subscription non-auction non-redq_rental non-accommodation-booking',
				'hints'       => __( 'Adds an age verification checkbox while purchasing this product.', 'woocommerce-germanized' ),
			),
			'delivery_time'             => array(
			...
			'_service'                  => array(
				...
				'type'        => 'checkbox',
				'class'       => 'wcfm-checkbox wcfm_ele variable non-variable-subscription non-job_package non-resume_package non-auction non-redq_rental non-appointment non-accommodation-booking',
				'label_class' => 'wcfm_title wcfm_ele variable checkbox_title non-variable-subscription non-job_package non-resume_package non-auction non-redq_rental non-appointment non-accommodation-booking',
				'value'       => 'yes',
			),
			'_mini_desc'                => array(
				'label'       => __( 'Optional Mini Description', 'woocommerce-germanized' ),
				'type'        => 'textarea',
				'class'       => 'wcfm-textarea wcfm_ele wcfm_full_ele variable non-variable-subscription non-auction non-redq_rental non-accommodation-booking',
				'label_class' => 'wcfm_ele wcfm_title wcfm_full_ele variable non-variable-subscription non-auction non-redq_rental non-accommodation-booking',
				'hints'       => __( 'This content will be shown as short product description within checkout and emails.', 'woocommerce-germanized' ),
			),
		);
	...
	function wcfm_wc_product_scheduler_product_manage_views() {
		...
		<div class="wcfm-container simple variable external booking">
			<div id="wcfm_products_manage_form_wc_product_scheduler_expander" class="wcfm-content">
				...
				$WCFM->wcfm_fields->wcfm_generate_form_field(
					apply_filters(
						'wcfm_wc_product_scheduler_fields',
						array(

							'wpas_select_status'    => array(
								'label'       => __( 'Status', 'wc-frontend-manager' ),
								'type'        => 'select',
								'options'     => array(
									'0' => __( 'Disable', 'wc-frontend-manager' ),
									'1' => __( 'Enable', 'wc-frontend-manager' ),
								),
								'class'       => 'wcfm-select wcfm_ele simple variable external booking',
								'label_class' => 'wcfm_title simple variable external booking',
								'value'       => $status,
							),

							'wpas_st_date'          => array(
								'label'             => __( 'Start Time', 'wc-frontend-manager' ),
								'type'              => 'text',
								'placeholder'       => __( 'From', 'wc-frontend-manager' ) . '... YYYY-DD-MM',
								'custom-attributes' => array( 'date_format' => 'yy-mm-dd' ),
								'class'             => 'wcfm-text wcfm_ele wcfm_small_ele wcfm_datepicker simple variable external booking',
								'label_class'       => 'wcfm_title wcfm_ele simple variable external booking',
								'value'             => $start_date,
							),
							'wpas_st_hh'            => array(
								'type'        => 'number',
								'class'       => 'wcfm-text wcfm_ele wcfm_small_ele wcfm_non_negative_input simple variable external booking',
								'label_class' => 'wcfm_title simple variable external booking',
								'attributes'  => array(
									'min'  => 0,
									'max'  => 12,
							...
							'wpas_end_date'         => array(
								'label'             => __( 'End Time', 'wc-frontend-manager' ),
								'type'              => 'text',
								'placeholder'       => __( 'Upto', 'wc-frontend-manager' ) . '... YYYY-DD-MM',
								'custom-attributes' => array( 'date_format' => 'yy-mm-dd' ),
								'class'             => 'wcfm-text wcfm_ele wcfm_small_ele wcfm_datepicker simple variable external booking',
								'label_class'       => 'wcfm_title wcfm_ele simple variable external booking',
								'value'             => $end_date,
							...
							'wpas_enable_countdown' => array(
								'label'       => __( 'CountDown', 'wc-frontend-manager' ),
								'type'        => 'select',
								'options'     => array(
									'0' => __( 'Disable', 'wc-frontend-manager' ),
									'1' => __( 'Enable', 'wc-frontend-manager' ),
				...
				<?php
				_e( 'Note: Start time and End time will be on GMT, Current GMT time is', 'wc-frontend-manager' );
	...
	function wcfm_woo_country_based_restriction_product_manage_views() {
		...
		<div class="wcfm-container simple variable external booking">
			<div id="wcfm_products_manage_form_woo_country_based_restriction_expander" class="wcfm-content">
				...
				$WCFM->wcfm_fields->wcfm_generate_form_field(
					apply_filters(
						'wcfm_woo_country_based_restriction_fields',
						array(

							'_fz_country_restriction_type' => array(
								...
								'class'       => 'wcfm-select wcfm_ele simple variable external booking',
								'label_class' => 'wcfm_title simple variable external booking',
								'value'       => $_fz_country_restriction_type,
							),

							'_restricted_countries'        => array(
								'label'                 => __( 'Select countries', 'woo-product-country-base-restrictions' ),
								'type'                  => 'country',
								'wcfm_shipping_country' => true,
								'attributes'            => array( 'multiple' => true ),
								'class'                 => 'wcfm-select wcfm_ele simple variable external booking',
								'label_class'           => 'wcfm_title wcfm_ele simple variable external booking',
								'value'                 => $_restricted_countries,
							),
	...
	function wcfm_wc_tiered_price_table_product_manage_fields_variations( $variation_fileds, $variations, $variation_shipping_option_array, $variation_tax_classes_options ) {
		...
		if ( WCFM_Dependencies::wcfm_wc_tiered_price_table_premium_active_check() ) {
			$price_rules_types = array(
				'fixed'      => __( 'Fixed', 'tier-pricing-table' ),
		...
		$wc_tiered_price_fields = array(
			...
			'tiered_price_rules_type'    => array(
				...
				'class'       => 'wcfm-select wcfm_ele variable variation_tiered_price_rules_type',
				'label_class' => 'wcfm_title variable',
			),
			...
			'tiered_fixed_price_rules'   => array(
				...
				'type'        => 'multiinput',
				'class'       => 'wcfm-text wcfm_ele variable tiered_price_rule_type tiered_price_rule_type_fixed',
			...
			'tiered_percent_price_rules' => array(
				'label'       => __( 'Tiered price', 'tier-pricing-table' ),
				'type'        => 'multiinput',
				'class'       => 'wcfm-text wcfm_ele variable tiered_price_rule_type tiered_price_rule_type_percentage',
	...
	function wcfm_wc_german_market_product_pricing_fields( $pricing_fields, $product_id, $product_type ) {
		...
		$_sale_label_options = array(
			'-2' => __( 'Select', 'woocommerce-german-market' ),
			'-1' => __( 'Use the default', 'woocommerce-german-market' ),
		);
		foreach ( $terms as $i ) {
			$_sale_label_options[ $i->term_id ] = $i->name;
		}

		$wcfm_wc_german_market_pricing_fields = apply_filters(
			'wcfm_wc_german_market_pricing_fields',
			array(
				'_lieferzeit'                       => array(
					'label'       => __( 'Delivery Time:', 'woocommerce-german-market' ),
					'type'        => 'select',
					'options'     => $_lieferzeit_options,
					'class'       => 'wcfm-select wcfm_ele simple variable',
					'label_class' => 'wcfm_title simple variable',
					'value'       => $_lieferzeit,
				),
				'_alternative_shipping_information' => array(
					'label'       => __( 'Alternative Shipping Information', 'woocommerce-german-market' ),
					'type'        => 'text',
					'class'       => 'wcfm-text wcfm_ele simple variable',
					'label_class' => 'wcfm_title simple variable',
					'value'       => $_alternative_shipping_information,
				...
				'_suppress_shipping_notice'         => array(
					'label'       => __( 'Disable Shipping Information', 'woocommerce-german-market' ),
					'type'        => 'checkbox',
					'class'       => 'wcfm-checkbox wcfm_ele simple variable wcfm_non_negative_input',
					'label_class' => 'wcfm_title checkbox-title checkbox_title simple variable',
					'value'       => 'on',
					'dfvalue'     => $_suppress_shipping_notice,
					'hints'       => __( 'Don’t display shipping information for this product (e.g. if it is virtual/digital).', 'woocommerce-german-market' ),
				),
				'_sale_label'                       => array(
					'label'       => __( 'Sale Label:', 'woocommerce-german-market' ),
					'type'        => 'select',
					'options'     => $_sale_label_options,
					'class'       => 'wcfm-select wcfm_ele simple variable',
					'label_class' => 'wcfm_title simple variable',
					'value'       => $_sale_label,
				),
		...
		if ( get_option( 'gm_gtin_activation', 'off' ) == 'on' ) {
			$wcfm_wc_german_market_pricing_fields['_gm_gtin'] = array(
				'label'       => __( 'GTIN', 'woocommerce-german-market' ),
				'type'        => 'text',
				'class'       => 'wcfm-text wcfm_ele simple',
				'label_class' => 'wcfm_title wcfm_ele simple',
				'value'       => $_gm_gtin,
			);
		}
	...
	function wcfm_wc_german_market_product_manage_fields_variations( $variation_fileds, $variations, $variation_shipping_option_array, $variation_tax_classes_options ) {
		...
		$wcfm_wc_german_market_pricing_fields = apply_filters(
			'wcfm_wc_german_market_variation_pricing_fields',
			array(
				'_lieferzeit'                          => array(
					'label'       => __( 'Delivery Time:', 'woocommerce-german-market' ),
					'type'        => 'select',
				...
				'variable_used_setting_ppu'            => array(
					'label'       => __( 'Price per Unit', 'woocommerce-german-market' ),
					'type'        => 'select',
					'options'     => array( '-1' => __( 'Same as parent', 'woocommerce-german-market' ) ),
					'class'       => 'wcfm-text wcfm_ele',
					'label_class' => 'wcfm_title wcfm_ele',
				),
				'_variable_used_setting_shipping_info' => array(
					...
					<td>
						<select name="vendor_data_asal_kota" id="vendor_data_asal_kota" style="width: 50%">
							...
							foreach ( $origins as $origin ) {
								...
								<?php
								echo '"' . $idx . '"';
								if ( $vendor_data_asal_kota === $idx ) {
									echo ' selected';}
								?>
								><?php echo esc_html( $origin['kota_kabupaten'] ); ?></option>
								<?php
							}
							?>
						</select>
				...
				<tr>
					...
					<td>
						<table>
							<tr>
								<td style="width: 33%">
								<input type="checkbox" class="wcfm-checkbox" style="margin-right: 5%;" name="vendor_jne_reg" 
								<?php
								if ( $vendor_jne_reg === 'on' ) {
									echo ' checked';}
								?>
									> JNE REG</input><br>
								<input type="checkbox" class="wcfm-checkbox" style="margin-right: 5%;" name="vendor_jne_oke" 
								<?php
								if ( $vendor_jne_oke === 'on' ) {
									echo ' checked';}
								?>
									> JNE OKE</input><br>
								<input type="checkbox" class="wcfm-checkbox" style="margin-right: 5%;" name="vendor_jne_yes" 
								<?php
								if ( $vendor_jne_yes === 'on' ) {
									echo ' checked';}
								?>
									> JNE YES</input><br>
								</td>
								<td style="width: 33%">
								<input type="checkbox" class="wcfm-checkbox" style="margin-right: 5%;" name="vendor_tiki_reg" 
								<?php
								if ( $vendor_tiki_reg === 'on' ) {
									echo ' checked';}
								?>
									> TIKI REG</input><br>
								<input type="checkbox" class="wcfm-checkbox" style="margin-right: 5%;" name="vendor_tiki_eco" 
								<?php
								if ( $vendor_tiki_eco === 'on' ) {
									echo ' checked';}
								?>
									> TIKI ECO</input><br>
								<input type="checkbox" class="wcfm-checkbox" style="margin-right: 5%;" name="vendor_tiki_ons" 
								<?php
								if ( $vendor_tiki_ons === 'on' ) {
									echo ' checked';}
								?>
									> TIKI ONS</input><br>
								</td>
								<td style="width: 33%">
								<input type="checkbox" class="wcfm-checkbox" style="margin-right: 5%;" name="vendor_pos_kilat_khusus" 
								<?php
								if ( $vendor_pos_kilat_khusus === 'on' ) {
									echo ' checked';}
								?>
									> POS KILAT KHUSUS</input><br>
								<input type="checkbox" class="wcfm-checkbox" style="margin-right: 5%;" name="vendor_pos_express_next_day" 
								<?php
								if ( $vendor_pos_express_next_day === 'on' ) {
									echo ' checked';}
								?>
									> POS Express Next Day</input><br>
								<input type="checkbox" class="wcfm-checkbox" style="margin-right: 5%;" name="vendor_pos_valuable_goods" 
								<?php
								if ( $vendor_pos_valuable_goods === 'on' ) {
									echo ' checked';}
								?>
									> POS Valuable Goods</input><br>
								</td>
								</tr>
								<tr>
								<td style="width: 33%">
								<input type="checkbox" class="wcfm-checkbox" style="margin-right: 5%;" name="vendor_jnt_ez" 
								<?php
								if ( $vendor_jnt_ez === 'on' ) {
									echo ' checked';}
								?>
								...
								<td style="width: 33%">
								<input type="checkbox" class="wcfm-checkbox" style="margin-right: 5%;" name="vendor_sicepat_reg" 
								<?php
								if ( $vendor_sicepat_reg === 'on' ) {
									echo ' checked';}
								?>
									> SICEPAT REG</input><br>
								<input type="checkbox" class="wcfm-checkbox" style="margin-right: 5%;" name="vendor_sicepat_best" 
								<?php
								if ( $vendor_sicepat_best === 'on' ) {
									echo ' checked';}
								?>
									> SICEPAT BEST</input><br>
								</td>
								<td style="width: 33%">
								<input type="checkbox" class="wcfm-checkbox" style="margin-right: 5%;" name="vendor_wahana" 
								<?php
								if ( $vendor_wahana === 'on' ) {
									echo ' checked';}
								?>
	...
	function wcfm_wcepeken_vendor_settings_update( $user_id, $wcfm_settings_form ) {
		...
		$vendor_data_asal_kota       = ( empty( $wcfm_settings_form['vendor_data_asal_kota'] ) ) ? '' : $wcfm_settings_form['vendor_data_asal_kota'];  
	...
	public function wcfm_woo_product_size_chart_product_manage_views( $product_id ) {
		...
		if ( $product_id ) {
			...
			$chart_id = ( is_array( $chart_id ) ) ? $chart_id : array( $chart_id );
		...
		$WCFM->wcfm_fields->wcfm_generate_form_field(
			apply_filters(
				'wcfm_woo_product_size_chart_fields',
				array(
	...
	function wcfm_woo_product_post_expirator_product_manage_views( $product_id ) {
		...
		echo _postExpiratorExpireType(
			array(
				'type'     => 'page',
				'name'     => 'expirationdate_expiretype',
				'selected' => $expireType,
				'disabled' => $disabled,
				'onchange' => 'expirationdate_toggle_category(this)',
			)
		);
		echo '<br/>';
