...
class WCFM_Enquiry_Manage_Controller {
	...
	public function processing() {
		...
		if ( isset( $_POST['inquiry_reply'] ) && ! empty( $_POST['inquiry_reply'] ) ) {
			...
			if ( ! $wcfm_enquiry_row || ! wcfm_user_can_perform_request( $wcfm_enquiry_row->vendor_id, 'enquiry' ) ) {
			...
			$wcfm_create_enquiry_reply = $wpdb->prepare(
				"INSERT into {$wpdb->prefix}wcfm_enquiries_response 
			...
			if ( $enquiry_reply_id ) {
				...
				if ( ! empty( $attchments ) && isset( $attchments['inquiry_attachments'] ) && ! empty( $attchments['inquiry_attachments'] ) ) {
					...
					$wcfm_enuquiry_meta_update = $wpdb->prepare(
						"INSERT into {$wpdb->prefix}wcfm_enquiries_response_meta 
				...
				if ( isset( $wcfm_enquiry_reply_form_data['inquiry_stick'] ) && ! empty( $wcfm_enquiry_reply_form_data['inquiry_stick'] ) ) {
					$wcfm_update_enquiry = $wpdb->prepare(
						"UPDATE {$wpdb->prefix}wcfm_enquiries 
																		SET 
																		`reply` = %s,
																		`reply_by` = %d,
																		`is_private` = 0, 
				...
				if ( $inquiry_customer_email ) {
					...
					if ( $inquiry_vendor_id ) {
						$enquiry_for_label = wcfm_get_vendor_store_name( $inquiry_vendor_id ) . ' ' . apply_filters( 'wcfm_sold_by_label', $inquiry_vendor_id, __( 'Store', 'wc-frontend-manager' ) );
					}
					if ( $inquiry_product_id ) {
						$enquiry_for_label = get_the_title( $inquiry_product_id );
					}

					$enquiry_for = __( 'Store', 'wc-frontend-manager' );
					if ( $inquiry_vendor_id ) {
						$enquiry_for = wcfm_get_vendor_store( $inquiry_vendor_id );
					}
					if ( $inquiry_product_id ) {
						$enquiry_for = '<a target="_blank" class="wcfm_dashboard_item_title" href="' . esc_url( get_permalink( $inquiry_product_id ) ) . '">' . get_the_title( $inquiry_product_id ) . '</a>';
					}
					...
					if ( $inquiry_customer_id ) {
						$reply_mail_body .= sprintf( __( 'See details %1$shere%2$s.', 'wc-frontend-manager' ), '<a href="{enquiry_url}">', '</a>' );
					}
