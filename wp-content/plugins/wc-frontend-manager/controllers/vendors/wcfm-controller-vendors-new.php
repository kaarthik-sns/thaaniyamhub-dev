		...
		if ( isset( $wcfm_vendor_form_data['user_name'] ) && ! empty( $wcfm_vendor_form_data['user_name'] ) ) {
			if ( isset( $wcfm_vendor_form_data['user_email'] ) && ! empty( $wcfm_vendor_form_data['user_email'] ) ) {
				if ( isset( $wcfm_vendor_form_data['store_name'] ) && ! empty( $wcfm_vendor_form_data['store_name'] ) ) {
					...
					} elseif ( email_exists( $wcfm_vendor_form_data['user_email'] ) == false ) {
					...
					if ( ! $has_error ) {
						...
						$user_data = array(
							'user_login'    => $wcfm_vendor_form_data['user_name'],
						...
						if ( ! $vendor_id ) {
							...
							if ( apply_filters( 'wcfm_is_pref_vendor_vacation', true ) ) {
								if ( ! isset( $wcfm_vendor_form_data['wcfm_vacation_mode'] ) ) {
									$wcfm_vendor_form_data['wcfm_vacation_mode'] = 'no';
								}
								if ( ! isset( $wcfm_vendor_form_data['wcfm_disable_vacation_purchase'] ) ) {
									$wcfm_vendor_form_data['wcfm_disable_vacation_purchase'] = 'no';
								}
						...
						if ( ! $has_error ) {
							echo '{"status": true, "message": "' . esc_html( $wcfm_vendor_messages['vendor_saved'] ) . '", "redirect": "' . esc_url( apply_filters( 'wcfm_vendors_new_redirect', get_wcfm_vendors_manage_url( $vendor_id ), $vendor_id ) ) . '"}'; } else {
							echo '{"status": false, "message": "' . esc_html( $wcfm_vendor_messages['vendor_failed'] ) . '"}'; }
					}
				} else {
