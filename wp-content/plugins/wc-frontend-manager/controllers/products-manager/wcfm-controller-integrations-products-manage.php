...
class WCFM_Integrations_Products_Manage_Controller {
	...
	function wcfm_wc_sku_generator_product_meta_save( $new_product_id, $wcfm_products_manage_form_data ) {
		...
		if ( $product->is_type( 'variable' ) && 'never' !== get_option( 'wc_sku_generator_variation' ) ) {
			...
			$args = apply_filters(
				'wc_sku_generator_variation_query_args',
				array(
	...
	function wcfm_wc_product_scheduler_product_meta_save( $new_product_id, $wcfm_products_manage_form_data ) {
		...
		if ( ! empty( $wcfm_products_manage_form_data['wpas_st_hh'] ) ) {
			$wpas_st_hh = sanitize_text_field( $wcfm_products_manage_form_data['wpas_st_hh'] );
		}
		if ( absint( $wpas_st_hh ) > 12 ) {
			$wpas_st_hh = 11;
		}
		if ( ! empty( $wcfm_products_manage_form_data['wpas_st_mn'] ) ) {
			$wpas_st_mn = sanitize_text_field( $wcfm_products_manage_form_data['wpas_st_mn'] );
		}
		if ( absint( $wpas_st_mn ) > 60 ) {
			$wpas_st_mn = 59;
		}

		$wpas_end_date = sanitize_text_field( $wcfm_products_manage_form_data['wpas_end_date'] );
		if ( ! empty( $wcfm_products_manage_form_data['wpas_end_hh'] ) ) {
			$wpas_end_hh = sanitize_text_field( $wcfm_products_manage_form_data['wpas_end_hh'] );
		}
		if ( absint( $wpas_end_hh ) > 12 ) {
			$wpas_end_hh = 11;
		}
		if ( ! empty( $wcfm_products_manage_form_data['wpas_end_mn'] ) ) {
			$wpas_end_mn = sanitize_text_field( $wcfm_products_manage_form_data['wpas_end_mn'] );
		}
		if ( absint( $wpas_end_mn ) > 60 ) {
			$wpas_end_mn = 59;
		}
