...
class WCFM_Articles_Manage_Controller {
	...
	public function processing() {
		...
		if ( isset( $wcfm_articles_manage_form_data['title'] ) && ! empty( $wcfm_articles_manage_form_data['title'] ) ) {
			...
			} elseif ( apply_filters( 'wcfm_is_allow_publish_articles', true ) ) {
			...
			$new_article = apply_filters(
				'wcfm_article_content_before_save',
				array(
			...
			if ( isset( $wcfm_articles_manage_form_data['article_id'] ) && $wcfm_articles_manage_form_data['article_id'] == 0 ) {
				...
				if ( wcfm_is_marketplace() && ( ! function_exists( 'wcfmmp_get_store_url' ) || wcfm_is_vendor() ) ) {
				...
				} elseif ( ( get_post_status( $new_article['ID'] ) == 'draft' ) && ( $article_status != 'draft' ) ) {
					$is_publish = true;
				}
			...
			if ( ! is_wp_error( $new_article_id ) ) {
				...
				if ( $is_update ) {
					$new_article_id = $wcfm_articles_manage_form_data['article_id'];
				}
				...
				if ( ! $has_error ) {
					if ( get_post_status( $new_article_id ) == 'publish' ) {
						...
						if ( ! $has_error ) {
							...
							echo '{"status": true, "message": "' . esc_html( apply_filters( 'article_saved_message', $wcfm_articles_manage_messages['article_saved'], $new_article_id ) ) . '", "redirect": "' . esc_url( apply_filters( 'wcfm_article_save_draft_redirect', get_wcfm_articles_manage_url( $new_article_id ), $new_article_id ) ) . '", "id": "' . $new_article_id . '"}';
					}
				}
				die;
			}
