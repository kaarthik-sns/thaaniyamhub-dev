...
class WCFM_Listings_Controller {
	...
	public function processing() {
		...
		$wcfm_listings_status = apply_filters(
			'wcfm_listings_menus',
			array(
		...
		if ( isset( $_POST['listing_status'] ) && ! empty( $_POST['listing_status'] ) && ( $_POST['listing_status'] != 'all' ) ) {
			$args['post_status'] = wc_clean( $_POST['listing_status'] );
		}
		...
		if ( ! empty( $wcfm_listings_array ) ) {
			...
			foreach ( $wcfm_listings_array as $wcfm_listings_single ) {
				...
				if ( $wcfm_allow_listings_edit = apply_filters( 'wcfm_is_allow_listings_edit', true ) ) {
					$wcfm_listings_json_arr[ $index ][] = '<a target="_blank" href="' . add_query_arg(
						array(
							'action' => 'edit',
							'job_id' => $wcfm_listings_single->ID,
						),
						$jobs_dashboard_url
					) . '" class="wcfm_listing_title">' . $wcfm_listings_single->post_title . '</a>';
				} else {
				...
				if ( WCFM_Dependencies::wcfm_wp_job_manager_applications_plugin_active_check() ) {
					...
					if ( $jobs_dashboard ) {
						$wcfm_listings_json_arr[ $index ][] = ( $count = get_job_application_count( $wcfm_listings_single->ID ) ) ? '<a target="_permalink" class="application_count" href="' . add_query_arg(
							array(
				...
				if ( in_array( $wcfm_listings_single->post_status, array( 'pending', 'pending_payment' ), true ) && ! wcfm_is_vendor() ) {
					$actions .= '<a class="wcfm-action-icon" href="' . wp_nonce_url(
						add_query_arg(
							array(
								'action'     => 'approve_listing',
								'listing_id' => $wcfm_listings_single->ID,
							),
							WC()->ajax_url()
						),
						'wcfm_ajax_nonce',
						'wcfm_ajax_nonce'
					) . '"><span class="wcfmfa fa-check text_tip" data-tip="' . esc_attr__( 'Approve', 'wp-job-manager' ) . '"></span></a>';
				}
				...
				if ( $wcfm_listings_single->post_status == 'publish' ) {
					...
					if ( $wcfm_allow_listings_edit = apply_filters( 'wcfm_is_allow_listings_edit', true ) ) {
						$actions .= '<a class="wcfm-action-icon" href="' . add_query_arg(
							array(
								'action' => 'edit',
								'job_id' => $wcfm_listings_single->ID,
							),
					...
					if ( $wcfm_allow_listings_mark_filled = apply_filters( 'wcfm_is_allow_listings_mark_filled', true ) ) {
						if ( is_position_filled( $wcfm_listings_single->ID ) ) {
							$actions .= '<a class="wcfm-action-icon" href="' . wp_nonce_url(
								add_query_arg(
									array(
										'action' => 'mark_not_filled',
										'job_id' => $wcfm_listings_single->ID,
									),
									$jobs_dashboard_url
								),
								'job_manager_my_job_actions'
							) . '"><span class="wcfmfa fa-times-circle text_tip" data-tip="' . esc_attr__( 'Mark not filled', 'wp-job-manager' ) . '"></span></a>';
						} else {
							$actions .= '<a class="wcfm-action-icon" href="' . wp_nonce_url(
								add_query_arg(
									array(
								...
								'job_manager_my_job_actions'
							) . '"><span class="wcfmfa fa-check-circle text_tip" data-tip="' . esc_attr__( 'Mark filled', 'wp-job-manager' ) . '"></span></a>';
						}
					}
					...
					if ( apply_filters( 'wcfm_is_allow_featured_listing', true ) ) {
						...
						} elseif ( apply_filters( 'wcfm_has_featured_listing_limit', true ) ) {
					...
					if ( $wcfm_allow_listings_duplicate = apply_filters( 'wcfm_is_allow_listings_duplicate', true ) ) {
						$actions .= '<a target="_blank" class="wcfm-action-icon" href="' . wp_nonce_url(
							add_query_arg(
								array(
									'action' => 'duplicate',
									'job_id' => $wcfm_listings_single->ID,
							...
							'job_manager_my_job_actions'
						) . '"><span class="wcfmfa fa-copy text_tip" data-tip="' . esc_attr__( 'Duplicate', 'wp-job-manager' ) . '"></span></a>';
					}
				} else {
					...
					if ( $wcfm_listings_single->post_status != 'expired' ) {
						$actions .= '<a class="wcfm-action-icon" href="' . wp_nonce_url(
							...
							'job_manager_my_job_actions'
						) . '"><span class="wcfmfa fa-edit text_tip" data-tip="' . esc_attr__( 'Continue Submission', 'wc-frontend-manager' ) . '"></span></a>';
					}
						...
						if ( $wcfm_listings_single->post_status == 'expired' ) {
							if ( job_manager_get_permalink( 'submit_job_form' ) ) {
								$actions .= '<a class="wcfm-action-icon" href="' . wp_nonce_url(
									...
									'job_manager_my_job_actions'
								) . '"><span class="wcfmfa fa-retweet text_tip" data-tip="' . esc_attr__( 'Relist', 'wp-job-manager' ) . '"></span></a>';
							}
						}
					}
				}
				...
				if ( $wcfm_allow_listings_delete = apply_filters( 'wcfm_is_allow_listings_delete', true ) ) {
					$actions .= '<a class="wcfm-action-icon" href="' . wp_nonce_url(
						...
						'job_manager_my_job_actions'
					) . '"><span class="wcfmfa fa-trash-alt text_tip" data-tip="' . esc_attr__( 'Delete', 'wc-frontend-manager' ) . '"></span></a>';
				}
		...
		if ( ! empty( $wcfm_listings_json_arr ) ) {
			$wcfm_listings_json .= json_encode( $wcfm_listings_json_arr );
		} else {
			$wcfm_listings_json .= '[]';
		}
		$wcfm_listings_json .= '
