...
class WCFM_Applications_Controller {
	...
	public function processing() {
		...
		if ( ! empty( $wcfm_applications_array ) ) {
			...
			foreach ( $wcfm_applications_array as $wcfm_applications_single ) {
				...
				if ( $wcfm_allow_applications_edit = apply_filters( 'wcfm_is_allow_applications_edit', true ) ) {
					$wcfm_applications_json_arr[ $index ][] = '<a target="_blank" href="' . add_query_arg(
						array(
							'action' => 'edit',
							'job_id' => $wcfm_applications_single->ID,
						),
						$jobs_dashboard_url
					) . '" class="wcfm_listing_title">' . $wcfm_applications_single->post_title . '</a>';
				} else {
				...
				if ( $wcfm_applications_single->post_status == 'publish' ) {
					...
					if ( $wcfm_allow_applications_mark_filled = apply_filters( 'wcfm_is_allow_applications_mark_filled', true ) ) {
						if ( is_position_filled( $wcfm_applications_single->ID ) ) {
							...
							) . '"><span class="wcfmfa fa-check-circle text_tip" data-tip="' . esc_attr__( 'Mark not filled', 'wp-job-manager' ) . '"></span></a>';
						} else {
							$actions .= '<a class="wcfm-action-icon" href="' . wp_nonce_url(
								add_query_arg(
									array(
		...
		if ( ! empty( $wcfm_applications_json_arr ) ) {
			$wcfm_applications_json .= json_encode( $wcfm_applications_json_arr );
		} else {
			$wcfm_applications_json .= '[]';
		}
		$wcfm_applications_json .= '
