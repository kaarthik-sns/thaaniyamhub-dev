...
class WCFM_Withdrawal_Controller {
	...
	public function processing() {
		...
		if ( ! $filtered_withdrawal_count ) {
			$filtered_withdrawal_count = 0;
		}
		...
		$wcfm_withdrawals_array = $wpdb->get_results(
			$wpdb->prepare(
				$sql,
				$this->vendor_id,
				$length,
		...
		if ( ! empty( $wcfm_withdrawals_array ) ) {
			...
			foreach ( $wcfm_withdrawals_array as $wcfm_withdrawals_single ) {
				...
				if ( apply_filters( 'wcfm_is_show_commission_restrict_check', false, $order_id, $wcfm_withdrawals_single ) ) {
					continue;
				}
