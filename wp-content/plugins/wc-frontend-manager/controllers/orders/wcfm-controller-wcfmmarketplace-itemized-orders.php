...
class WCFM_Orders_WCFMMarketplace_Controller {
	...
	public function processing() {
		...
		if ( ! empty( $_POST['search']['value'] ) ) {
			...
			if ( ! empty( $_POST['commission_status'] ) && ! empty( $_POST['order_status'] ) ) {
				$sql = $wpdb->prepare( $sql, $commission_status, $order_status );
			}
			if ( ! empty( $_POST['commission_status'] ) && empty( $_POST['order_status'] ) ) {
				$sql = $wpdb->prepare( $sql, $commission_status );
			}
			if ( ! empty( $_POST['order_status'] ) && empty( $_POST['commission_status'] ) ) {
		...
		if ( ! empty( $wcfm_orders_json_arr ) ) {
			$wcfm_orders_json .= json_encode( apply_filters( 'wcfm_orders_controller_data', $wcfm_orders_json_arr, 'vendor', $this->vendor_id ) );
		} else {
			$wcfm_orders_json .= '[]';
		}
		$wcfm_orders_json .= '
