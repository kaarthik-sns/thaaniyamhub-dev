			...
			foreach ( $wcfm_orders_array as $wcfm_orders_single ) {
				...
				if ( $wcfm_is_allow_order_status_update = apply_filters( 'wcfm_is_allow_order_status_update', true ) ) {
					...
					if ( ! in_array( $order_status, array( 'failed', 'cancelled', 'refunded', 'completed' ) ) ) {
						$actions = '<a class="wcfm_order_mark_complete wcfm-action-icon" href="#" data-orderid="' . $wcfm_orders_single->order_id . '"><span class="wcfmfa fa-check-circle text_tip" data-tip="' . esc_attr__( 'Mark as Complete', 'wc-frontend-manager' ) . '"></span></a>';
					}
				}
