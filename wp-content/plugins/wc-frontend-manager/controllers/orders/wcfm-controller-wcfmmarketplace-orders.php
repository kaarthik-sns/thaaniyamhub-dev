...
class WCFM_Orders_WCFMMarketplace_Controller {
	...
	public function processing() {
		...
		if ( ! empty( $_POST['search']['value'] ) ) {
			...
			if ( ! empty( $_POST['order_product'] ) && ! empty( $_POST['commission_status'] ) && ! empty( $_POST['order_status'] ) ) {
				$sql = $wpdb->prepare( $sql, $order_product, $commission_status, $order_status );
			}
			if ( ! empty( $_POST['order_product'] ) && ! empty( $_POST['commission_status'] ) && empty( $_POST['order_status'] ) ) {
				$sql = $wpdb->prepare( $sql, $order_product, $commission_status );
			}
			if ( ! empty( $_POST['order_product'] ) && ! empty( $_POST['order_status'] ) && empty( $_POST['commission_status'] ) ) {
				$sql = $wpdb->prepare( $sql, $order_product, $order_status );
			}
			if ( ! empty( $_POST['commission_status'] ) && ! empty( $_POST['order_status'] ) && empty( $_POST['order_product'] ) ) {
				$sql = $wpdb->prepare( $sql, $commission_status, $order_status );
			}
			if ( ! empty( $_POST['order_product'] ) && empty( $_POST['order_status'] ) && empty( $_POST['commission_status'] ) ) {
				$sql = $wpdb->prepare( $sql, $order_product );
			}
			if ( ! empty( $_POST['commission_status'] ) && empty( $_POST['order_status'] ) && empty( $_POST['order_product'] ) ) {
				$sql = $wpdb->prepare( $sql, $commission_status );
			}
			if ( ! empty( $_POST['order_status'] ) && empty( $_POST['commission_status'] ) && empty( $_POST['order_product'] ) ) {
				$sql = $wpdb->prepare( $sql, $order_status );
			}
		}
		...
		if ( ! empty( $order_summary ) ) {
			...
			foreach ( $order_summary as $order ) {
				...
				if ( apply_filters( 'wcfm_is_show_order_restrict_check', false, $order->order_id, $order->product_id, $order ) ) {
					continue;
				}
				...
				if ( apply_filters( 'wcfm_is_allow_order_status_update', true ) && ! in_array( 'requested', $refund_statuses ) && in_array( 0, $is_refundeds ) ) {
					...
					if ( in_array( 'wc-completed', array_keys( $allowed_order_status ) ) && ! in_array( $order_status, $status_update_block_statuses ) && ! in_array( $order_status, array( 'failed', 'cancelled', 'refunded', 'completed', 'request', 'proposal', 'proposal-sent', 'proposal-expired', 'proposal-rejected', 'proposal-canceled', 'proposal-accepted' ) ) ) {
