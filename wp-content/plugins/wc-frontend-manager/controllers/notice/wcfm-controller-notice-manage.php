		...
		if ( isset( $wcfm_notice_manager_form_data['title'] ) && ! empty( $wcfm_notice_manager_form_data['title'] ) ) {
			...
			if ( ! is_wp_error( $new_notice_id ) ) {
				...
				if ( ! $is_update ) {
					...
					$wcfm_messages = sprintf( __( 'A new announcement submitted. <a target="_blank" class="wcfm_dashboard_item_title" href="%1$s">%2$s</a>', 'wc-frontend-manager' ), get_wcfm_notice_view_url( $new_notice_id ), $wcfm_notice_manager_form_data['title'] );
