...
class WCFM_Settings_WCMarketplace_Controller {
	...
	public function processing() {
		...
		if ( ! apply_filters( 'wcfm_is_allow_store_logo', true ) ) {
			if ( isset( $wcfm_setting_store_fields['_vendor_image'] ) ) {
				unset( $wcfm_setting_store_fields['_vendor_image'] ); }
		}

		if ( ! apply_filters( 'wcfm_is_allow_store_name', true ) ) {
			if ( isset( $wcfm_setting_store_fields['_vendor_page_title'] ) ) {
				unset( $wcfm_setting_store_fields['_vendor_page_title'] ); }
			if ( isset( $wcfm_setting_store_fields['_vendor_page_slug'] ) ) {
				unset( $wcfm_setting_store_fields['_vendor_page_slug'] ); }
		}
		...
		if ( apply_filters( 'wcfm_is_allow_store_name', true ) ) {
			...
			if ( $vendor ) {
				...
				wp_update_user(
					array(
						'ID'           => $user_id,
						'display_name' => wc_clean( $wcfm_settings_form['shop_name'] ),
					)
				);
		...
		$wcfm_settings_store_fields = array(
			'_vendor_phone'     => 'shop_phone',
		...
		if ( ! apply_filters( 'wcfm_is_allow_store_banner', true ) ) {
			if ( isset( $wcfm_setting_store_fields['_vendor_banner'] ) ) {
				unset( $wcfm_setting_store_fields['_vendor_banner'] ); }
		}

		if ( ! apply_filters( 'wcfm_is_allow_store_phone', true ) ) {
			if ( isset( $wcfm_setting_store_fields['_vendor_phone'] ) ) {
				unset( $wcfm_setting_store_fields['_vendor_phone'] ); }
		}

		if ( ! apply_filters( 'wcfm_is_allow_store_address', true ) ) {
			if ( isset( $wcfm_setting_store_fields['_vendor_address_1'] ) ) {
				unset( $wcfm_setting_store_fields['_vendor_address_1'] ); }
			if ( isset( $wcfm_setting_store_fields['_vendor_address_2'] ) ) {
				unset( $wcfm_setting_store_fields['_vendor_address_2'] ); }
			if ( isset( $wcfm_setting_store_fields['_vendor_country'] ) ) {
				unset( $wcfm_setting_store_fields['_vendor_country'] ); }
			if ( isset( $wcfm_setting_store_fields['_vendor_city'] ) ) {
				unset( $wcfm_setting_store_fields['_vendor_city'] ); }
			if ( isset( $wcfm_setting_store_fields['_vendor_state'] ) ) {
				unset( $wcfm_setting_store_fields['_vendor_state'] ); }
			if ( isset( $wcfm_setting_store_fields['_vendor_postcode'] ) ) {
				unset( $wcfm_setting_store_fields['_vendor_postcode'] ); }
			if ( isset( $wcfm_setting_store_fields['_find_address'] ) ) {
				unset( $wcfm_setting_store_fields['_find_address'] ); }
			if ( isset( $wcfm_setting_store_fields['_store_location'] ) ) {
				unset( $wcfm_setting_store_fields['_store_location'] ); }
			if ( isset( $wcfm_setting_store_fields['_store_lat'] ) ) {
				unset( $wcfm_setting_store_fields['_store_lat'] ); }
			if ( isset( $wcfm_setting_store_fields['_store_lng'] ) ) {
				unset( $wcfm_setting_store_fields['_store_lng'] ); }
		}
		...
		$wcfm_setting_bank_fields = array(
			...
			if ( ! empty( $table_rate_data_rules ) && is_array( $table_rate_data_rules ) ) {
				foreach ( $table_rate_data_rules as $shipping_method_id => $table_rate_datas ) {
					...
					foreach ( $table_rate_datas as $table_rate_data ) {
						...
						$rate_cost                 = isset( $table_rate_data['rate_cost'] ) ? rtrim( rtrim( number_format( (float) $table_rate_data['rate_cost'], 4, '.', '' ), '0' ), '.' ) : '';
						$rate_cost_per_item        = isset( $table_rate_data['rate_cost_per_item'] ) ? rtrim( rtrim( number_format( (float) $table_rate_data['rate_cost_per_item'], 4, '.', '' ), '0' ), '.' ) : '';
						$rate_cost_per_weight_unit = isset( $table_rate_data['rate_cost_per_weight_unit'] ) ? rtrim( rtrim( number_format( (float) $table_rate_data['rate_cost_per_weight_unit'], 4, '.', '' ), '0' ), '.' ) : '';
						$rate_cost_percent         = isset( $table_rate_data['rate_cost_percent'] ) ? rtrim( rtrim( number_format( (float) str_replace( '%', '', $table_rate_data['rate_cost_percent'] ), 2, '.', '' ), '0' ), '.' ) : '';
