	...
	public function processing() {
		...
		if ( isset( $_POST['search'] ) && ! empty( $_POST['search']['value'] ) ) {
			...
			$args['meta_query'] = array(
				apply_filters(
					'wcfm_get_customers_meta_search',
					array(
						'relation' => 'OR',
						array(
							'key'     => 'first_name',
		...
		if ( ! empty( $wcfm_customers_array ) ) {
			foreach ( $wcfm_customers_array as $wcfm_customers_single ) {
				...
				if ( $value ) {
					$wcfm_customers_json_arr[ $index ][] = apply_filters( 'wcfm_customers_display_location_data', $value, $wcfm_customers_single->ID );
				} else {
					$wcfm_customers_json_arr[ $index ][] = apply_filters( 'wcfm_customers_display_location_data', '&ndash;', $wcfm_customers_single->ID );
				}
		...
		if ( ! empty( $wcfm_customers_json_arr ) ) {
			$wcfm_customers_json .= json_encode( $wcfm_customers_json_arr );
		} else {
			$wcfm_customers_json .= '[]';
		}
		$wcfm_customers_json .= '
