		...
		if ( isset( $wcfm_customer_form_data['user_name'] ) && ! empty( $wcfm_customer_form_data['user_name'] ) ) {
			if ( isset( $wcfm_customer_form_data['user_email'] ) && ! empty( $wcfm_customer_form_data['user_email'] ) ) {
				...
				if ( isset( $wcfm_customer_form_data['customer_id'] ) && $wcfm_customer_form_data['customer_id'] != 0 ) {
					...
					$is_update = true;
				} elseif ( username_exists( $wcfm_customer_form_data['user_name'] ) ) {
						$has_error = true;
						echo '{"status": false, "message": "' . $wcfm_customer_messages['username_exists'] . '"}';
				} elseif ( email_exists( $wcfm_customer_form_data['user_email'] ) == false ) {
				} else {
					$has_error = true;
				...
				if ( ! $has_error ) {
					...
					if ( ! $customer_id ) {
						...
						if ( ! $is_update ) {
							...
							$author_is_vendor = 1;
							$message_to       = 0;
							$wcfm_messages    = sprintf( __( 'A new customer <b>%1$s</b> added to the store by <b>%2$s</b>', 'wc-frontend-manager' ), '<a class="wcfm_dashboard_item_title" href="' . esc_url( get_wcfm_customers_details_url( $customer_id ) ) . '">' . $wcfm_customer_form_data['first_name'] . ' ' . $wcfm_customer_form_data['last_name'] . '</a>', get_user_by( 'id', $author_id )->display_name );
