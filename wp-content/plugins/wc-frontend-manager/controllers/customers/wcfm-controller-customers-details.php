			...
			foreach ( $wcfm_orders_array as $wcfm_orders_single ) {
				...
				if ( $wcfm_is_allow_order_status_update = apply_filters( 'wcfm_is_allow_order_status_update', true ) ) {
					...
					if ( ! in_array( $order_status, array( 'failed', 'cancelled', 'refunded', 'completed' ) ) ) {
						$actions = '<a class="wcfm_order_mark_complete wcfm-action-icon" href="#" data-orderid="' . $order_id . '"><span class="wcfmfa fa-check-circle text_tip" data-tip="' . esc_attr__( 'Mark as Complete', 'wc-frontend-manager' ) . '"></span></a>';
					}
				}
...
class WCFM_Customers_Details_Bookings_Controller {
	...
	public function processing() {
		...
		$wc_get_booking_status_name = array(
		...
		if ( ! empty( $wcfm_bookings_array ) ) {
			...
			foreach ( $wcfm_bookings_array as $wcfm_bookings_single ) {
				...
				if ( $the_booking->has_status( array( 'was-in-cart', 'in-cart' ) ) ) {
					continue;
				}
				...
				if ( current_user_can( 'manage_bookings_settings' ) || current_user_can( 'manage_bookings' ) ) {
					if ( WCFM_Dependencies::wcfmu_plugin_active_check() ) {
						if ( in_array( $the_booking->get_status(), array( 'pending-confirmation' ) ) ) {
							$actions = '<a class="wcfm_booking_mark_confirm wcfm-action-icon" href="#" data-bookingid="' . $wcfm_bookings_single->ID . '"><span class="wcfmfa fa-check-circle text_tip" data-tip="' . esc_attr__( 'Mark as Confirmed', 'wc-frontend-manager' ) . '"></span></a>';
						}
					}
				}
...
class WCFM_Customers_Details_Appointments_Controller {
	...
	public function processing() {
		...
		$wc_get_appointment_status_name = array(
			'paid'                 => __( 'Paid', 'wc-frontend-manager' ),
			'pending-confirmation' => __( 'Pending Confirmation', 'wc-frontend-manager' ),
			'unpaid'               => __( 'Un-paid', 'wc-frontend-manager' ),
			'cancelled'            => __( 'Cancelled', 'wc-frontend-manager' ),
			'complete'             => __( 'Complete', 'wc-frontend-manager' ),
		...
		if ( ! empty( $wcfm_appointments_array ) ) {
			...
			foreach ( $wcfm_appointments_array as $wcfm_appointments_single ) {
				...
				if ( $the_appointment->has_status( array( 'was-in-cart', 'in-cart' ) ) ) {
					continue;
				}
				...
				if ( current_user_can( 'manage_appointments' ) ) {
					if ( in_array( $the_appointment->get_status(), array( 'pending-confirmation' ) ) ) {
						$actions = '<a class="wcfm_appointment_mark_confirm wcfm-action-icon" href="#" data-appointmentid="' . $wcfm_appointments_single->ID . '"><span class="wcfmfa fa-check-circle text_tip" data-tip="' . esc_attr__( 'Mark as Confirmed', 'wc-frontend-manager' ) . '"></span></a>';
					}
				}
