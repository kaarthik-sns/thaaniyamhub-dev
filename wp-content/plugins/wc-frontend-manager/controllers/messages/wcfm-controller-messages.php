	...
	public function processing() {
		...
		if ( ! empty( $wcfm_messages ) ) {
			foreach ( $wcfm_messages as $wcfm_message ) {
				...
				$wcfm_messages_json_arr[ $index ][] = $WCFM->wcfm_notification->get_wcfm_notification_icon( $wcfm_message->message_type );  
				...
				} elseif ( $wcfm_message->message_to == 0 ) {
					$wcfm_messages_json_arr[ $index ][] = apply_filters( 'wcfm_sold_by_label', '', __( 'Store', 'wc-frontend-manager' ) );
		...
		if ( ! empty( $wcfm_messages_json_arr ) ) {
			$wcfm_messages_json .= json_encode( $wcfm_messages_json_arr );
		} else {
			$wcfm_messages_json .= '[]';
		}
		$wcfm_messages_json .= '
