	...
	public function processing() {
		...
		if ( isset( $wcfm_coupon_manager_form_data['title'] ) && ! empty( $wcfm_coupon_manager_form_data['title'] ) ) {
			...
			if ( isset( $_POST['status'] ) && ( $_POST['status'] == 'draft' ) ) {
				$coupon_status = 'draft';
			} elseif ( current_user_can( 'publish_shop_coupons' ) && apply_filters( 'wcfm_is_allow_publish_coupons', true ) ) {
			...
			$new_coupon = apply_filters(
				'wcfm_coupon_content_before_save',
				array(
			...
			if ( isset( $wcfm_coupon_manager_form_data['coupon_id'] ) && $wcfm_coupon_manager_form_data['coupon_id'] == 0 ) {
				...
				} elseif ( ( get_post_status( $new_coupon['ID'] ) == 'draft' ) && ( $coupon_status != 'draft' ) ) {
					$is_publish = true;
				}
			...
			if ( ! is_wp_error( $new_coupon_id ) ) {
				...
				if ( $is_update ) {
					$new_coupon_id = $wcfm_coupon_manager_form_data['coupon_id'];
				}
				...
				if ( ! $has_error ) {
					if ( get_post_status( $new_coupon_id ) == 'draft' ) {
						if ( ! $has_error ) {
							echo '{"status": true, "message": "' . esc_html( $wcfm_coupon_messages['coupon_saved'] ) . '", "id": "' . $new_coupon_id . '"}';
						}
					} elseif ( ! $has_error ) {
						echo '{"status": true, "message": "' . esc_html( $wcfm_coupon_messages['coupon_published'] ) . '", "redirect": "' . esc_url( get_wcfm_coupons_manage_url( $new_coupon_id ) ) . '"}';
					}
				}
					die;
			}
