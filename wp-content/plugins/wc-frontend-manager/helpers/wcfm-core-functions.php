...
if ( ! function_exists( 'wcfm_woocommerce_inactive_notice' ) ) {
	function wcfm_woocommerce_inactive_notice() {
		...
		<p><?php printf( __( '%1$sWooCommerce Frontend Manager is inactive.%2$s The %3$sWooCommerce plugin%4$s must be active for the WooCommerce Frontend Manager to work. Please %5$sinstall & activate WooCommerce%6$s', 'wc-frontend-manager' ), '<strong>', '</strong>', '<a target="_blank" href="http://wordpress.org/extend/plugins/woocommerce/">', '</a>', '<a href="' . admin_url( 'plugin-install.php?tab=search&s=woocommerce' ) . '">', '&nbsp;&raquo;</a>' ); ?></p>
		</div>
		<?php
	}
}
...
if ( ! function_exists( 'wcfm_woocommerce_version_notice' ) ) {
	function wcfm_woocommerce_version_notice() {
		?>
		<div id="message" class="error">
		<p><?php printf( __( '%1$sOops ..!!!%2$s You are using %3$sWC %4$s. WCFM works only with %5$sWC 3.0+%6$s. PLease upgrade your WooCommerce version now to make your life easier and peaceful by using WCFM.', 'wc-frontend-manager' ), '<strong>', '</strong>', '<strong>', WC_VERSION . '</strong>', '<strong>', '</strong>' ); ?></p>
		</div>
		<?php
	}
}
		...
		if ( wcfm_is_vendor() ) {
			...
			?>
				...
				<?php
					...
					printf( __( '%1$s%2$s%3$s: Upgrade your WCFM to %4$sWCFM - Ultimate%5$s to access this feature. Disable this notice from settings panel using "Disable Ultimate Notice" option.', 'wc-frontend-manager' ), '<strong>', $feature, '</strong>', '<a target="_blank" href="https://wclovers.com/product/woocommerce-frontend-manager-ultimate/?utm_source=wp-admin&utm_medium=banner&utm_campaign=promotion&utm_content=ultimate"><strong>', '</strong></a>' );
				?>
					</p>
				</div>
				<?php
		...
		if ( wcfm_is_vendor() ) {
			...
			?>
				...
				<?php
					...
					printf( __( '%1$s%2$s%3$s: Associate your WCFM with %4$sWCFM - Groups & Staffs%5$s to access this feature.', 'wc-frontend-manager' ), '<strong>', $feature, '</strong>', '<a target="_blank" href="https://wclovers.com/product/woocommerce-frontend-manager-groups-staffs/?utm_source=wp-admin&utm_medium=banner&utm_campaign=promotion&utm_content=groups-staffs"><strong>', '</strong></a>' );
				?>
					</p>
				</div>
				<?php
		...
		if ( wcfm_is_vendor() ) {
			...
			?>
				...
				<?php
					...
					printf( __( '%1$s%2$s%3$s: Associate your WCFM with %4$sWCFM - Analytics%5$s to access this feature.', 'wc-frontend-manager' ), '<strong>', $feature, '</strong>', '<a target="_blank" href="http://wclovers.com/product/woocommerce-frontend-manager-analytics/"><strong>', '</strong></a>' );
				?>
					</p>
				</div>
				<?php
	...
	function wcfm_is_vendor( $vendor_id = '' ) {
		...
		if ( $is_marketplace ) {
			if ( 'wcvendors' == $is_marketplace ) {
				...
				if ( function_exists( 'is_user_wcmp_vendor' ) && is_user_wcmp_vendor( $vendor_id ) ) {
					return true;
				}
			} elseif ( 'wcpvendors' == $is_marketplace ) {
				if ( WC_Product_Vendors_Utils::is_vendor( $vendor_id ) && ! WC_Product_Vendors_Utils::is_pending_vendor( $vendor_id ) ) {
					return true;
				}
				$vendor_data = get_term( $vendor_id, WC_PRODUCT_VENDORS_TAXONOMY );
				if ( $vendor_data && ! is_wp_error( $vendor_data ) ) {
					return true;}
				...
				if ( is_a( $user, 'WP_User' ) ) {
					if ( in_array( 'wcfm_vendor', (array) $user->roles ) ) {
						return apply_filters( 'wcfm_is_vendor', true, $vendor_id );
					}
				}
			}
		}
...
if ( ! function_exists( 'wcfm_is_mobile' ) ) {
	function wcfm_is_mobile() {
		...
		include_once __DIR__ . '/wcfm-mobile-detect.php';
...
if ( ! function_exists( 'get_wcfm_products_url' ) ) {
	function get_wcfm_products_url( $product_status = '', $product_vendor = '' ) {
		...
		if ( $product_status ) {
			$wcfm_products_url = add_query_arg( 'product_status', $product_status, $wcfm_products_url );
		}
		if ( $product_vendor ) {
			$wcfm_products_url = add_query_arg( 'product_vendor', $product_vendor, $wcfm_products_url );
		}
...
if ( ! function_exists( 'get_wcfm_stock_manage_url' ) ) {
	function get_wcfm_stock_manage_url( $product_status = '' ) {
		...
		if ( $product_status ) {
			$wcfm_stock_manage_url = add_query_arg( 'product_status', $product_status, $wcfm_stock_manage_url );
		}
...
if ( ! function_exists( 'get_wcfm_import_product_url' ) ) {
	function get_wcfm_import_product_url( $step = '' ) {
		...
		if ( $step ) {
			$wcfm_import_product_url = add_query_arg( 'step', $step, $wcfm_import_product_url );
		}
...
if ( ! function_exists( 'get_wcfm_orders_url' ) ) {
	function get_wcfm_orders_url( $order_status = '', $order_vendor = '' ) {
		...
		if ( $order_status ) {
			$wcfm_orders_url = add_query_arg( 'order_status', $order_status, $wcfm_orders_url );
		}
		if ( $order_vendor ) {
			$wcfm_orders_url = add_query_arg( 'order_vendor', $order_vendor, $wcfm_orders_url );
		}
...
if ( ! function_exists( 'get_wcfm_reports_url' ) ) {
	function get_wcfm_reports_url( $range = '', $report_type = 'wcfm-reports-sales-by-date', $vendor_id = '' ) {
		...
		if ( $range ) {
			$get_wcfm_reports_url = add_query_arg( 'range', $range, $get_wcfm_reports_url );
		}
		if ( $report_type == 'wcfm-reports-sales-by-date' ) {
			$get_wcfm_reports_url = apply_filters( 'wcfm_default_reports_url', $get_wcfm_reports_url );
		}
...
if ( ! function_exists( 'get_wcfm_analytics_url' ) ) {
	function get_wcfm_analytics_url( $range = '' ) {
		...
		if ( $range ) {
			$get_wcfm_analytics_url = add_query_arg( 'range', $range, $get_wcfm_analytics_url );
		}
...
if ( ! function_exists( 'get_wcfm_messages_url' ) ) {
	function get_wcfm_messages_url( $message_type = '' ) {
		...
		if ( $message_type ) {
			$get_wcfm_messages_url = add_query_arg( 'message_type', $message_type, $get_wcfm_messages_url );
		}
...
if ( ! function_exists( 'get_wcfm_articles_url' ) ) {
	function get_wcfm_articles_url( $article_status = '' ) {
		...
		if ( $article_status ) {
			$get_wcfm_articles_url = add_query_arg( 'article_status', $article_status, $get_wcfm_articles_url );
		}
...
if ( ! function_exists( 'get_wcfm_listings_url' ) ) {
	function get_wcfm_listings_url( $listing_status = '' ) {
		...
		if ( $listing_status ) {
			$wcfm_listings_dashboard_url = add_query_arg( 'listing_status', $listing_status, $wcfm_listings_dashboard_url );
		}
...
if ( ! function_exists( 'get_wcfm_applications_url' ) ) {
	function get_wcfm_applications_url( $listing_id = '' ) {
		...
		if ( $listing_id ) {
			$wcfm_applications_dashboard_url = add_query_arg( 'listing_id', $listing_id, $wcfm_applications_dashboard_url );
		}
...
if ( ! function_exists( 'get_wcfm_articles_manager_messages' ) ) {
	function get_wcfm_articles_manager_messages() {
		...
		$messages = apply_filters(
			'wcfm_validation_messages_article_manager',
			array(
...
if ( ! function_exists( 'get_wcfm_products_manager_messages' ) ) {
	function get_wcfm_products_manager_messages() {
		...
		$messages = apply_filters(
			'wcfm_validation_messages_product_manager',
			array(
...
function wcfm_get_endpoint_url( $endpoint, $value = '', $permalink = '', $lang_code = '' ) {
	...
	if ( $endpoint == 'booking' ) {
		$endpoint = 'bookinglist';
	}
	if ( $endpoint == 'bookings' ) {
		$endpoint = 'bookingslist';
	}
	if ( $endpoint == 'subscriptions' ) {
		$endpoint = 'subscriptionslist';
	}
	if ( $endpoint == 'sell-items-catalog' ) {
		$endpoint = 'add-to-my-store-catalog';
	}
...
function wcfm_get_user_posts_count( $user_id = 0, $post_type = 'product', $post_status = 'publish', $custom_args = array() ) {
	...
	if ( ! $user_id && ! current_user_can( 'administrator' ) && ! current_user_can( 'shop_manager' ) ) {
...
function wcfm_enquiry_product_tab( $tabs ) {
	...
	if ( apply_filters( 'wcfm_is_pref_enquiry_tab', true ) && apply_filters( 'wcfm_is_pref_enquiry', true ) ) {
		...
		$tabs['wcfm_enquiry_tab'] = apply_filters(
			'wcfm_enquiry_tab_element',
			array(
...
function wcfm_policies_product_tab( $tabs ) {
	...
	if ( apply_filters( 'wcfm_is_pref_policies', true ) && apply_filters( 'wcfm_is_allow_product_policies', true ) ) {
		unset( $tabs['policies'] );
		$tabs['wcfm_policies_tab'] = apply_filters(
			'wcfm_policies_tab_element',
			array(
...
function bp_wcfm_user_nav_item() {
	...
	if ( ! $bp || ! $bp->displayed_user || ! property_exists( $bp->displayed_user, 'userdata' ) || ! $bp->displayed_user->userdata || ! $bp->displayed_user->id ) {
		return;
	}
...
if ( apply_filters( 'wcfm_is_pref_buddypress', true ) && WCFM_Dependencies::wcfm_biddypress_plugin_active_check() ) {
	...
	add_filter(
		'bp_user_can',
		function ( $retval, $user_id, $capability, $site_id, $args ) {
			if ( wcfm_is_vendor( $user_id ) && ( $capability == 'bp_moderate' ) ) {
				$retval = false; }
			return $retval;
		},
		50,
		...
		if ( is_array( $file_data['name'] ) ) {
			foreach ( $file_data['name'] as $file_data_key => $file_data_value ) {
				if ( $file_data['name'][ $file_data_key ] ) {
					$type              = wp_check_filetype( $file_data['name'][ $file_data_key ] );  
					$files_to_upload[] = array(
...
function wcfm_video_tutorial( $video_url ) {
	...
	if ( ! apply_filters( 'wcfm_is_allow_video_tutorial', true ) ) {
		return;
	}
	?>
...
function wcfm_get_option( $key, $default_val = '', $lang_code = '' ) {
	...
	if ( defined( 'ICL_SITEPRESS_VERSION' ) && ! ICL_PLUGIN_INACTIVE && class_exists( 'SitePress' ) ) {
		...
		} elseif ( apply_filters( 'wcfm_use_default_language_if_current_language_value_not_found', true, 'option' ) ) {
...
function wcfm_get_post_meta( $post_id, $key, $is_single = true ) {
	...
	if ( defined( 'ICL_SITEPRESS_VERSION' ) && ! ICL_PLUGIN_INACTIVE && class_exists( 'SitePress' ) ) {
		...
		} elseif ( apply_filters( 'wcfm_use_default_language_if_current_language_value_not_found', true, 'postmeta' ) ) {
				$default_language = $sitepress->get_default_language();
...
function wcfm_get_user_meta( $user_id, $key, $is_single = true ) {
	...
	if ( defined( 'ICL_SITEPRESS_VERSION' ) && ! ICL_PLUGIN_INACTIVE && class_exists( 'SitePress' ) ) {
		...
		if ( $current_val ) {
			$wpml_meta_val = $current_val;
		} elseif ( apply_filters( 'wcfm_use_default_language_if_current_language_value_not_found', true, 'usermeta' ) ) {
				$default_language = $sitepress->get_default_language();
...
add_filter(
	'wcfm_editor_content_before_save',
	function ( $content ) {
