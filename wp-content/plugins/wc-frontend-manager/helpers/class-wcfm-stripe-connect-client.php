...
class WCFM_Stripe_Connect_Client {
	...
	public function __construct( $client_id = '', $secret_key = '' ) {
		...
		if ( class_exists( 'WCFMmp_Stripe_Client_Factory' ) ) {
			...
			$this->stripe = WCFMmp_Stripe_Client_Factory::client();
		} else {
	...
	public function get_stripe_accounts_args() {
		...
		$stripe_connect_args['capabilities'] = array(
			'card_payments' => array(
				'requested' => true,
			),
			'transfers'     => array(
		...
		if (
			...
			$stripe_connect_args['tos_acceptance'] = array(
				'service_agreement' => 'recipient',
			);
			 
			 
			update_user_meta( $this->user_id, 'stripe_service_agreement', 'recipient' );
		}
	...
	public function get_stripe_account_links_args( $stripe_user_id ) {
		return apply_filters(
			'wcfm_stripe_account_links_args',
			array(
	...
	public function create_account( $params = null, $opts = null ) {
		...
		if ( $account && isset( $account->id ) ) {
			...
			$this->store_account_meta( $account );
		}
	...
	public function get_european_countries() {
		...
		$non_eu_sepa_countries = array( 'AD', 'CH', 'GB', 'MC', 'SM', 'VA' );
	...
	public function store_account_meta( $account ) {
		if ( ! $this->user_id || ! is_object( $account ) ) {
			return;
		}
		...
		update_user_meta( $this->user_id, 'stripe_account_capabilities', $capabilities );
		update_user_meta( $this->user_id, 'stripe_payouts_enabled', ! empty( $account->payouts_enabled ) ? 'yes' : 'no' );
		update_user_meta( $this->user_id, 'stripe_charges_enabled', ! empty( $account->charges_enabled ) ? 'yes' : 'no' );
		update_user_meta( $this->user_id, 'stripe_account_country', isset( $account->country ) ? $account->country : '' );
		$agreement = isset( $account->tos_acceptance->service_agreement ) ? $account->tos_acceptance->service_agreement : 'full';
		update_user_meta( $this->user_id, 'stripe_service_agreement', $agreement );
		update_user_meta( $this->user_id, 'stripe_account_updated', time() );
	}
	...
	public function delete_stripe_data() {
		...
		delete_user_meta( $this->user_id, 'stripe_payouts_enabled' );
		delete_user_meta( $this->user_id, 'stripe_charges_enabled' );
		delete_user_meta( $this->user_id, 'stripe_account_country' );
		delete_user_meta( $this->user_id, 'stripe_service_agreement' );
		delete_user_meta( $this->user_id, 'stripe_account_updated' );
	}
	...
	public function get_cross_border_supported_countries() {
		return apply_filters(
			'wcfm_stripe_cross_border_supported_countries',
			array(
	...
	public function get_stripe_supported_direct_charge_countries() {
		return apply_filters(
			'wcfm_stripe_supported_direct_charge_countries',
			array(
				'AU' => __( 'Australia', 'woocommerce' ),
				'AT' => __( 'Austria', 'woocommerce' ),
				'BE' => __( 'Belgium', 'woocommerce' ),
