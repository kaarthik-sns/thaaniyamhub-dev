...
if ( isset( $wp->query_vars['wcfm-enquiry-manage'] ) && ! empty( $wp->query_vars['wcfm-enquiry-manage'] ) ) {
	...
	$enquiry_datas = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}wcfm_enquiries WHERE `ID` = %d", $inquiry_id ) );
...
<div class="collapse wcfm-collapse">
	...
	<div class="wcfm-collapse-content">
		...
		<div class="wcfm-container wcfm-top-element-container">
			...
			<?php
			if ( $inquiry_id ) {
				_e( 'Edit Enquiry', 'wc-frontend-manager' );
			} else {
				_e( 'Add Enquiry', 'wc-frontend-manager' ); }
			?>
			</h2>
			...
			if ( $inquiry_id && $product_id ) {
				echo '<a class="add_new_wcfm_ele_dashboard text_tip" target="_permalink" href="' . get_permalink( $product_id ) . '" data-tip="' . __( 'View Product', 'wc-frontend-manager' ) . '"><span class="wcfmfa fa-cube"></span></a>'; }
			?>
		...
		if ( $wcfm_is_allow_view_enquiry_reply_view = apply_filters( 'wcfmcap_is_allow_enquiry_reply_view', true ) ) {
			$wcfm_enquiry_replies = $wpdb->get_results( $wpdb->prepare( "SELECT * from {$wpdb->prefix}wcfm_enquiries_response WHERE `enquiry_id` = %d", $inquiry_id ) );
			...
			<form id="wcfm_inquiry_reply_form" class="wcfm">
				...
				<div class="wcfm-container">
					<div id="wcfm_new_reply_listing_expander" class="wcfm-content">
						...
						$inquiry_stick_class = '';
						if ( ! $inquiry_product_id ) {
							$inquiry_stick_class = ' wcfm_custom_hide';
						}
						$wcfm_enquiry_reply_fields = apply_filters(
							'wcfm_enquiry_reply_fields',
							array(
								'inquiry_reply'          => array(
								...
								'inquiry_reply_break2'   => array(
									'type'  => 'html',
									'value' => '<div class="wcfm-clearfix" style="margin-bottom: 15px;"></div>',
								),
								'inquiry_stick'          => array(
									'label'       => __( 'Stick at Product Page', 'wc-frontend-manager' ),
									'type'        => 'checkbox',
									'class'       => 'wcfm-checkbox' . $inquiry_stick_class,
									'label_class' => 'wcfm_title checkbox_title' . $inquiry_stick_class,
									'value'       => 'yes',
									'hints'       => __( 'Enable to stick this reply at product page', 'wc-frontend-manager' ),
								),
								'inquiry_id'             => array(
									'type'  => 'hidden',
									'value' => $inquiry_id,
								),
								'inquiry_product_id'     => array(
									'type'  => 'hidden',
									'value' => $inquiry_product_id,
								),
								'inquiry_vendor_id'      => array(
									'type'  => 'hidden',
									'value' => $inquiry_vendor_id,
								),
								'inquiry_customer_id'    => array(
									'type'  => 'hidden',
									'value' => $inquiry_customer_id,
								),
								'inquiry_customer_name'  => array(
									'type'  => 'hidden',
									'value' => $inquiry_customer_name,
								),
								'inquiry_customer_email' => array(
									'type'  => 'hidden',
									'value' => $inquiry_customer_email,
								),
							),
							$inquiry_id
						);
						...
						if ( ( $wcfm_enquiry_allow_attachment == 'no' ) || ! apply_filters( 'wcfm_is_allow_enquiry_reply_attachment', true ) ) {
							if ( isset( $wcfm_enquiry_reply_fields['inquiry_attachments'] ) ) {
								unset( $wcfm_enquiry_reply_fields['inquiry_attachments'] );
							}
							if ( isset( $wcfm_enquiry_reply_fields['inquiry_reply_break2'] ) ) {
								unset( $wcfm_enquiry_reply_fields['inquiry_reply_break2'] );
							}
						}
