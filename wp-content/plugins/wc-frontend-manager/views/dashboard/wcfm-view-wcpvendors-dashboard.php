...
$query_from = apply_filters(
	'wcpv_report_low_in_stock_query_from',
	$wpdb->prepare(
		"FROM {$wpdb->posts} as posts
...
$query_from = apply_filters(
	'wcpv_report_out_of_stock_query_from',
	$wpdb->prepare(
		"FROM {$wpdb->posts} as posts
	INNER JOIN {$wpdb->postmeta} AS postmeta ON posts.ID = postmeta.post_id
	INNER JOIN {$wpdb->postmeta} AS postmeta2 ON posts.ID = postmeta2.post_id
								...
								<?php if ( $wcfm_is_allow_reports = apply_filters( 'wcfm_is_allow_reports', true ) ) { ?>
									...
									<li class="best-seller-this-month">
										<a href="<?php echo apply_filters( 'sales_by_product_report_url', get_wcfm_reports_url(), $top_seller_id ); ?>">
											...
											<?php printf( __( '%1$s top seller this month (sold %2$d)', 'wc-frontend-manager' ), '<strong>' . $top_seller_title . '</strong> - ', $top_seller_qty ); ?>
										</a>
									</li>
								<?php } ?>
