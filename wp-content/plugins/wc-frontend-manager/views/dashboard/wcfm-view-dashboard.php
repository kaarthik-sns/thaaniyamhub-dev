...
require_once $WCFM->plugin_path . 'includes/reports/class-wcfm-report-sales-by-date.php';
...
require_once $WCFM->plugin_path . 'includes/reports/class-wcfm-report-analytics.php';
$wcfm_report_analytics               = new WCFM_Report_Analytics();
$wcfm_report_analytics->chart_colors = apply_filters(
	'wcfm_report_analytics_chart_colors',
	array(
		'view_count' => '#C79810',
	)
);
						...
						<div class="wcfm_dashboard_stats_block">
							<a href="<?php echo get_wcfm_reports_url(); ?>">
								...
								<div>
									...
									<?php
									if ( $admin_fee_mode ) {
										_e( 'admin fees in last 7 days', 'wc-frontend-manager' );
									} else {
										_e( 'commission in last 7 days', 'wc-frontend-manager' ); }
									?>
								</div>
							</a>
						</div>
										...
										if ( current_user_can( 'view_woocommerce_reports' ) && ( $top_seller = $WCFM->library->get_top_seller() ) && $top_seller->qty ) {
											...
											<li class="best-seller-this-month">
												<a href="<?php echo apply_filters( 'sales_by_product_report_url', get_wcfm_reports_url(), $top_seller->product_id ); ?>">
													<span class="wcfmfa fa-cube"></span>
													<?php printf( __( '%1$s top seller in last 7 days (sold %2$d)', 'wc-frontend-manager' ), '<strong>' . get_the_title( $top_seller->product_id ) . '</strong> - ', $top_seller->qty ); ?>
												</a>
											</li>
											<?php
										}
