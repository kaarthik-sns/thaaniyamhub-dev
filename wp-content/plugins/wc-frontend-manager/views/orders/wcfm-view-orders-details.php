...
<div class="collapse wcfm-collapse" id="wcfm_order_details">
	...
	<div class="wcfm-collapse-content">
		...
		<div class="wcfm-container wcfm-top-element-container">
			...
			echo $theorder->get_order_number();
			?>
			</h2>
			...
			<div id="orders_details_general_expander" class="wcfm-content">
				...
				<?php if ( apply_filters( 'wcfm_allow_order_customer_details', true ) ) { ?>
					<p class="form-field form-field-wide wc-customer-user">
						<label for="customer_user"><?php _e( 'Customer:', 'wc-frontend-manager' ); ?>
						<?php
				...
				<?php if ( apply_filters( 'wcfm_allow_customer_billing_details', true ) || apply_filters( 'wcfm_allow_customer_shipping_details', true ) || apply_filters( 'wcfm_is_allow_view_customer', true ) ) { ?>
									...
									<td>
										...
										if ( apply_filters( 'wcfm_allow_view_customer_email', true ) ) {
											foreach ( $WCFM->library->billing_fields as $key => $field ) {
												...
												if ( ! apply_filters( 'wcfm_allow_view_customer_' . $field_name, true ) ) {
													continue;
												}
					...
					?>
					<?php
					if ( apply_filters( 'wcfm_is_allow_order_details_after_order_table', true ) ) {
			...
			<div id="orders_details_items_expander" class="wcfm-content">
				<table cellpadding="0" cellspacing="0" class="woocommerce_order_items">
					...
					foreach ( $line_items as $item_id => $item ) {
						...
						} elseif ( apply_filters( 'wcfm_is_allow_show_product_permalink', true ) && apply_filters( 'wcfm_is_allow_order_details_product_permalink', true ) ) {
					...
					<tbody id="order_refunds">
						...
						if ( $refunds = $order->get_refunds() ) {
							...
							foreach ( $refunds as $refund ) {
								...
								if ( wcfm_is_vendor() && ( ! $who_refunded || ( $who_refunded && ( $who_refunded->ID != $cur_vendor_id ) ) ) ) {
									continue;
								}
								?>
							...
							?>
								<div class="wc-used-coupons">
									...
									foreach ( $coupons as $item_id => $item ) {
										...
										if ( apply_filters( 'wcfm_is_allow_show_only_vendor_coupon_to_vendors', true ) && wcfm_is_vendor() && ( ! wcfm_get_vendor_id_by_post( $post_id ) || ( wcfm_get_vendor_id_by_post( $post_id ) && ( $WCFMmp->vendor_id != wcfm_get_vendor_id_by_post( $post_id ) ) ) ) ) {
											continue;
										}

										$link = $post_id ? get_wcfm_coupons_manage_url( $post_id ) : get_wcfm_coupons_url();

										if ( ! apply_filters( 'wcfm_is_allow_manage_coupons', true ) || wcfm_is_vendor() ) {
											$link = '#'; }
