...
$wcfm_product_types         = apply_filters(
	'wcfm_product_types',
	array(
...
if ( $wcfm_product_custom_fields && is_array( $wcfm_product_custom_fields ) && ! empty( $wcfm_product_custom_fields ) ) {
	foreach ( $wcfm_product_custom_fields as $wpcf_index => $wcfm_product_custom_field ) {
		...
		if ( ! apply_filters( 'wcfm_allowed_custom_fields', true, $sanitize_block_name ) ) {
			continue;
		}
					...
					foreach ( $wcfm_product_custom_block_fields as $wcfm_product_custom_block_field ) {
						...
						if ( $is_group == 'yes' ) {
							...
							if ( $product_id ) {
								...
								} elseif ( isset( $group_value[ $wcfm_product_custom_block_field['name'] ] ) ) {
						...
						if ( isset( $wcfm_product_custom_block_field['required'] ) && $wcfm_product_custom_block_field['required'] ) {
						...
						switch ( $wcfm_product_custom_block_field['type'] ) {
							case 'text':
								$WCFM->wcfm_fields->wcfm_generate_form_field(
									apply_filters(
										'wcfm_pm_custom_field',
										array(
											$field_id => array(
												'label' => __( $wcfm_product_custom_block_field['label'], 'wc-frontend-manager' ),
												'name'  => $field_name,
												...
												'value' => $field_value,
												'hints' => __( $wcfm_product_custom_block_field['help_text'], 'wc-frontend-manager' ),
											),
										),
										$product_id,
										$wcfm_product_custom_block_field
									)
								);
								break;
									...
									apply_filters(
										...
										array(
											$field_id => array(
												...
												'custom_attributes' => $custom_attributes,
												'type'  => $wpeditor,
												'class' => 'wcfm-textarea wcfm_ele simple variable external grouped booking wcfm_custom_field_editor ' . $rich_editor,
												'label_class' => 'wcfm_title',
												'value' => $field_value,
												'hints' => __( $wcfm_product_custom_block_field['help_text'], 'wc-frontend-manager' ),
											),
									...
									apply_filters(
										...
										array(
											$field_id => array(
												...
												'type'    => 'select',
												'class'   => 'wcfm-select wcfm_ele simple variable external grouped booking ' . $field_class,
												'label_class' => 'wcfm_title',
