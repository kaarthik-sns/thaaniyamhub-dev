...
if ( isset( $wp->query_vars['wcfm-products-manage'] ) && ! empty( $wp->query_vars['wcfm-products-manage'] ) ) {
	...
	if ( $product && ! empty( $product ) ) {
		...
		if ( ! in_array( $product_type, $wcfm_downloadable_product_types ) ) {
			$is_downloadable = '';
		}
...
<div class="collapse wcfm-collapse" id="wcfm_products_manage">
	...
	<div class="wcfm-collapse-content">
		...
		<div class="wcfm-container wcfm-top-element-container">
			...
			<?php
			if ( $product_id ) {
				_e( 'Edit Product', 'wc-frontend-manager' );
			} else {
			...
			if ( $product_id ) {
				?>
				<span class="product-status product-status-<?php echo $wcfm_products_single->post_status; ?>">
				<?php
				if ( $wcfm_products_single->post_status == 'publish' ) {
					_e( 'Published', 'wc-frontend-manager' );
				} else {
					_e( ucfirst( $wcfm_products_single->post_status ), 'wc-frontend-manager' ); }
				?>
				</span>
				<?php
			...
			if ( $product_id && ( $wcfm_products_single->post_status == 'publish' ) ) {
				if ( apply_filters( 'wcfm_is_allow_featured_product', true ) ) {
					if ( WCFM_Dependencies::wcfmu_plugin_active_check() ) {
						...
						} elseif ( apply_filters( 'wcfm_has_featured_product_limit', true ) ) {
		...
		<form id="wcfm_products_manage_form" class="wcfm">
					...
					<?php do_action( 'wcfm_product_manager_left_panel_before', $product_id ); ?>
							...
							$WCFM->wcfm_fields->wcfm_generate_form_field(
								apply_filters(
									...
									array(
										...
										'is_virtual'      => array(
											'desc'       => __( 'Virtual', 'wc-frontend-manager' ),
											'type'       => 'checkbox',
											'class'      => 'wcfm-checkbox wcfm_ele wcfm_half_ele_checkbox simple booking non-variable-subscription non-job_package non-resume_package non-redq_rental non-accommodation-booking non-pw-gift-card' . ' ' . $wcfm_wpml_edit_disable_element,
											'desc_class' => 'wcfm_title wcfm_ele virtual_ele_title checkbox_title simple booking non-variable-subscription non-job_package non-resume_package non-redq_rental non-accommodation-booking non-pw-gift-card' . ' ' . $wcfm_wpml_edit_disable_element,
											'value'      => 'enable',
											'dfvalue'    => $is_virtual,
										),
										'is_downloadable' => array(
											'desc'       => __( 'Downloadable', 'wc-frontend-manager' ),
											'type'       => 'checkbox',
										...
										'pro_title'       => array(
											'placeholder' => __( 'Product Title', 'wc-frontend-manager' ),
											'type'        => 'text',
								...
								apply_filters(
									...
									array(
										'product_url'    => array(
											...
											'type'        => 'text',
											'class'       => 'wcfm-text wcfm_ele wcfm_ele_hide wcfm_half_ele external' . ' ' . $wcfm_wpml_edit_disable_element,
											'label_class' => 'wcfm_ele wcfm_ele_hide wcfm_half_ele_title wcfm_title external' . ' ' . $wcfm_wpml_edit_disable_element,
											'value'       => $product_url,
											'hints'       => __( 'Enter the external URL to the product.', 'wc-frontend-manager' ),
										),
										'button_text'    => array(
											'label'       => __( 'Button Text', 'wc-frontend-manager' ),
											'type'        => 'text',
											'class'       => 'wcfm-text wcfm_ele wcfm_ele_hide wcfm_half_ele wcfm_half_ele_right external' . ' ' . $wcfm_wpml_edit_disable_element,
											'label_class' => 'wcfm_ele wcfm_ele_hide wcfm_half_ele_title wcfm_title external' . ' ' . $wcfm_wpml_edit_disable_element,
											'value'       => $button_text,
											'hints'       => __( 'This text will be shown on the button linking to the external product.', 'wc-frontend-manager' ),
										),
										'regular_price'  => array(
											'label'       => __( 'Price', 'wc-frontend-manager' ) . ' (' . get_woocommerce_currency_symbol() . ')',
											'type'        => 'number',
											'class'       => 'wcfm-text wcfm_ele wcfm_non_negative_input wcfm_half_ele simple external non-subscription non-variable-subscription non-auction non-redq_rental non-accommodation-booking non-lottery non-pw-gift-card' . ' ' . $wcfm_wpml_edit_disable_element,
											'label_class' => 'wcfm_ele wcfm_half_ele_title wcfm_title simple external non-subscription non-variable-subscription non-auction non-redq_rental non-accommodation-booking non-lottery non-pw-gift-card' . ' ' . $wcfm_wpml_edit_disable_element,
											'value'       => $regular_price,
											'attributes'  => array(
												'min'  => '0.1',
												'step' => '0.1',
										...
										'sale_price'     => array(
											...
											'type'        => 'number',
											'class'       => 'wcfm-text wcfm_ele wcfm_non_negative_input wcfm_half_ele wcfm_half_ele_right simple external non-variable-subscription non-auction non-redq_rental non-accommodation-booking non-lottery non-pw-gift-card' . ' ' . $wcfm_wpml_edit_disable_element,
											'label_class' => 'wcfm_ele wcfm_half_ele_title wcfm_title simple external non-variable-subscription non-auction non-redq_rental non-accommodation-booking non-lottery non-pw-gift-card' . ' ' . $wcfm_wpml_edit_disable_element,
											'value'       => $sale_price,
											'desc_class'  => 'wcfm_ele simple external non-variable-subscription non-auction non-redq_rental non-accommodation-booking non-lottery non-pw-gift-card sales_schedule' . ' ' . $wcfm_wpml_edit_disable_element,
										...
										'sale_date_from' => array(
											...
											'class'       => 'wcfm-text wcfm_ele wcfm_ele_hide wcfm_half_ele sales_schedule_ele simple external non-variable-subscription non-auction non-redq_rental non-accommodation-booking' . ' ' . $wcfm_wpml_edit_disable_element,
											'label_class' => 'wcfm_ele wcfm_ele_hide wcfm_half_ele_title sales_schedule_ele wcfm_title simple external non-variable-subscription non-auction non-redq_rental non-accommodation-booking' . ' ' . $wcfm_wpml_edit_disable_element,
											'value'       => $sale_date_from,
										),
										'sale_date_upto' => array(
											'label'       => __( 'Upto', 'wc-frontend-manager' ),
											'type'        => 'text',
											'placeholder' => __( 'To', 'wc-frontend-manager' ) . '... YYYY-DD-MM',
											'custom-attributes' => array( 'date_format' => 'yy-mm-dd' ),
											'class'       => 'wcfm-text wcfm_ele wcfm_ele_hide wcfm_half_ele sales_schedule_ele simple external non-variable-subscription non-auction non-redq_rental non-accommodation-booking' . ' ' . $wcfm_wpml_edit_disable_element,
											'label_class' => 'wcfm_ele wcfm_ele_hide wcfm_half_ele_title sales_schedule_ele wcfm_title simple external non-variable-subscription non-auction non-redq_rental non-accommodation-booking' . ' ' . $wcfm_wpml_edit_disable_element,
						...
						<?php if ( ! apply_filters( 'wcfm_is_category_checklist', true ) ) { ?>
							...
							if ( $wcfm_is_allow_tags = apply_filters( 'wcfm_is_allow_tags', true ) ) {
								if ( apply_filters( 'wcfm_is_tags_input', true ) ) {
									$WCFM->wcfm_fields->wcfm_generate_form_field(
										apply_filters(
											'wcfm_product_simple_fields_tag',
											array(
												'product_tags' => array(
													'label' => __( 'Tags', 'wc-frontend-manager' ),
													'type' => 'text',
													'class' => 'wcfm-text wcfm_ele simple variable external grouped booking',
													'label_class' => 'wcfm_title',
													'value' => $product_tags,
													'placeholder' => __( 'Separate Product Tags with commas', 'wc-frontend-manager' ),
													...
													'desc' => __( 'Choose from the most used tags', 'wc-frontend-manager' ),
													'desc_class' => 'wcfm_full_ele wcfm_fetch_tag_cloud',
												),
											),
											$product_id,
											$product_type
											...
											if ( ! in_array( $product_taxonomy->name, array( 'product_cat', 'product_tag', 'wcpv_product_vendors' ) ) && apply_filters( 'wcfm_is_allow_product_taxonomy', true, $product_taxonomy->name ) && apply_filters( 'wcfm_is_allow_taxonomy_' . $product_taxonomy->name, true ) ) {
												if ( $product_taxonomy->public && $product_taxonomy->show_ui && $product_taxonomy->meta_box_cb && ! $product_taxonomy->hierarchical ) {
													if ( apply_filters( 'wcfm_is_allow_custom_taxonomy_' . $product_taxonomy->name, true ) ) {
														...
														$WCFM->wcfm_fields->wcfm_generate_form_field(
															array(
																$product_taxonomy->name => array(
																	'label'       => $product_taxonomy->label,
																	'name'        => 'product_custom_taxonomies_flat[' . $product_taxonomy->name . '][]',
																	'type'        => 'textarea',
																	...
																	'placeholder' => __( 'Separate Product ' . $product_taxonomy->label . ' with commas', 'wc-frontend-manager' ),
																),
															)
														);
													}
						...
						<?php if ( apply_filters( 'wcfm_is_category_checklist', true ) ) { ?>
							...
							<div class="wcfm_product_manager_content_fields">
								...
								$WCFM->wcfm_fields->wcfm_generate_form_field(
									apply_filters(
										'wcfm_product_manage_fields_content',
										array(
											'excerpt'     => array(
											...
											<div class="wcfm_add_new_category_box wcfm_add_new_taxonomy_box">
												...
												<div class="wcfm_add_new_taxonomy_form wcfm_add_new_taxonomy_form_hide">
													...
													$WCFM->wcfm_fields->wcfm_generate_form_field(
														array(
															'wcfm_new_cat' => array(
																'placeholder' => __( 'Category Name', 'wc-frontend-manager' ),
																'type' => 'text',
													...
													if ( apply_filters( 'wcfm_is_allow_add_new_product_cat_parent', true ) ) {
														$args = apply_filters(
															'wcfm_wp_dropdown_categories_args',
															array(
																'show_option_all'    => '',
																...
																<div class="wcfm_add_new_category_box wcfm_add_new_taxonomy_box">
																	...
																	<div class="wcfm_add_new_taxonomy_form wcfm_add_new_taxonomy_form_hide">
																		...
																		$WCFM->wcfm_fields->wcfm_generate_form_field(
																			array(
																				'wcfm_new_' . $product_taxonomy->name => array(
																					'placeholder' => apply_filters( 'wcfm_add_taxonomy_custom_label', __( $product_taxonomy->label, 'wc-frontend-manager' ), $product_taxonomy->name ) . ' ' . __( 'Name', 'wc-frontend-manager' ),
																					'type'        => 'text',
																					'class'       => 'wcfm-text wcfm_new_tax_ele wcfm_full_ele',
																				),
																			)
																		...
																		if ( apply_filters( 'wcfm_is_allow_add_new_' . $product_taxonomy->name . '_parent', true ) ) {
																			$args = apply_filters(
																				'wcfm_wp_dropdown_' . $product_taxonomy->name . '_args',
																				array(
																					'show_option_all'    => '',
										...
										apply_filters(
											...
											array(
												'product_tags' => array(
													'label' => __( 'Tags', 'wc-frontend-manager' ),
													'type' => 'textarea',
													'class' => 'wcfm-textarea wcfm_ele wcfm_full_ele product_tags_ele simple variable external grouped booking',
													'label_class' => 'wcfm_title wcfm_full_ele product_tags_ele',
													'value' => $product_tags,
													'placeholder' => __( 'Separate Product Tags with commas', 'wc-frontend-manager' ),
													'hints' => __( 'Product tags are descriptive labels you can add to your products. Popular search engines can use tags to get information about your store. You can add more than one tag separating them with a comma.', 'wc-frontend-manager' ),
													'desc' => __( 'Choose from the most used tags', 'wc-frontend-manager' ),
													'desc_class' => 'wcfm_full_ele wcfm_side_tag_cloud wcfm_fetch_tag_cloud',
												),
											),
											$product_id,
											$product_type
										...
										apply_filters(
											...
											array(
												'product_tags' => array(
													'label' => __( 'Tags', 'wc-frontend-manager' ),
													'type' => 'select',
													'class' => 'wcfm-select wcfm_ele wcfm_full_ele product_tags_as_dropdown product_tags_ele simple variable external grouped booking',
													'label_class' => 'wcfm_title wcfm_full_ele product_tags_ele',
													'value' => $product_tags,
													'options' => $product_all_tags,
													'attributes' => array( 'multiple' => true ),
			...
			<div class="wcfm-tabWrap">
				<div class="wcfm-tabWrap-content <?php echo $wcfm_tab_class; ?>">
					...
					<?php require 'wcfm-view-products-manage-tabs.php'; ?>
