...
if ( ( 'on' === get_option( 'german_market_gpsr_pre_fill_only_new_producs', 'on' ) && ! $product_id ) ||
...
<div class="wcfm-container simple variable">
	<div id="wcfm_products_manage_form_wc_german_market_gpsr_expander" class="wcfm-content">
		...
		$wcfm_wc_german_market_gpsr_fields = apply_filters(
			'wcfm_wc_german_market_gpsr_fields',
			array(
				'_german_market_gpsr_ignore_defaults'    => array(
					'label'       => $gpsr_labels['ignore_defaults'],
					'type'        => 'checkbox',
					'class'       => 'wcfm-checkbox simple variable',
					'label_class' => 'wcfm_title',
					'value'       => 'yes',
					'dfvalue'     => $gpsr_val['ignore_defaults'],
				),
				'_german_market_gpsr_manufacturer'       => array(
					'label'       => $gpsr_labels['manufacturer'],
					'type'        => 'textarea',
					'class'       => 'wcfm-textarea field_type_select_options',
					'label_class' => 'wcfm_title',
					'hint'        => __( 'Name, trademark, address and electronic address (url of the website or e-mail address)', 'woocommerce-german-market' ),
					'value'       => $gpsr_val['manufacturer'],
				),
				'_german_market_gpsr_responsible_person' => array(
					'label'       => $gpsr_labels['responsible_person'],
					'type'        => 'textarea',
					'class'       => 'wcfm-textarea field_type_select_options',
					'label_class' => 'wcfm_title',
					'hint'        => __( 'If the manufacturer is not established in the EU: name, address and electronic address (url of the website or e-mail address) of an economic operator established in the EU who is responsible for the manufacturer\'s obligations.', 'woocommerce-german-market' ),
					'value'       => $gpsr_val['responsible_person'],
				),
				'_german_market_gpsr_warnings_and_safety_information' => array(
					'label'       => $gpsr_labels['warnings_and_safety_information'],
					'type'        => $wpeditor,
					'class'       => 'wcfm-textarea wcfm_custom_field_editor ' . $rich_editor,
					'label_class' => 'wcfm_title',
					'value'       => $gpsr_val['warnings_and_safety_information'],
				),
			),
			$product_id
		);
...
<div class="wcfm-container simple variable">
	<div id="wcfm_products_manage_form_wc_german_market_expander" class="wcfm-content">
		<?php
		$wcfm_wc_german_market_fields = apply_filters(
			'wcfm_wc_german_market_fields',
			array(
				'_unit_regular_price_per_unit'        => array(
					'label'       => __( 'Scale Unit', 'woocommerce-german-market' ),
					'type'        => 'select',
					'options'     => $regular_price_units,
					'class'       => 'wcfm-select wcfm_ele simple variable',
					'label_class' => 'wcfm_title simple variable',
					'value'       => $_unit_regular_price_per_unit,
				),
				'_auto_ppu_complete_product_quantity' => array(
					'label'       => __( 'Complete product quantity', 'woocommerce-german-market' ),
					'type'        => 'number',
					'class'       => 'wcfm-text wcfm_ele simple variable wcfm_non_negative_input',
					'label_class' => 'wcfm_title simple variable',
					'value'       => $_auto_ppu_complete_product_quantity,
				),
				'_unit_regular_price_per_unit_mult'   => array(
					'label'       => __( 'Quantity to display', 'woocommerce-german-market' ),
					'type'        => 'number',
					'class'       => 'wcfm-text wcfm_ele simple variable wcfm_non_negative_input',
					'label_class' => 'wcfm_title simple variable',
					'value'       => $_unit_regular_price_per_unit_mult,
				),
			),
			$product_id
		);
...
<?php if ( get_option( 'german_market_age_rating', 'off' ) == 'on' ) { ?>
	...
	<div class="wcfm-container simple variable">
		<div id="wcfm_products_manage_form_wc_german_market_age_restriction_expander" class="wcfm-content">
			<?php
			$wcfm_wc_german_market_fields = apply_filters(
				'wcfm_wc_german_market_age_restriction_fields',
				array(
					'_age_rating_age' => array(
						'label'       => __( 'Required age to buy this product', 'woocommerce-german-market' ) . ' (' . __( 'Years', 'woocommerce-german-market' ) . ')',
						'type'        => 'number',
						'class'       => 'wcfm-text wcfm_ele simple variable wcfm_non_negative_input',
						'label_class' => 'wcfm_title simple variable',
						'value'       => $_age_rating_age,
					),
				),
				$product_id
			);
