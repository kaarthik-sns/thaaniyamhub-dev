	...
	<div class="wcfm-container simple variable nonvirtual booking">
		<div id="wcfm_products_manage_form_epeken_expander" class="wcfm-content">
				...
				<tr>
					...
					<select name="epeken_valid_origin_option" class="wcfm-select" id="epeken_valid_origin_option">
							...
							foreach ( $origins as $origin ) {
								?>
									<option value=<?php echo esc_attr( $origin['origin_code'] ); ?>
									<?php
									if ( $product_origin === $origin['origin_code'] ) {
										echo ' selected';}
									?>
			...
			<div style="margin-top: 10px;">
				<table>
					<tr><td valign="top">
					<input type="checkbox" name="epeken_product_insurance_mandatory" class="wcfm-checkbox" id="epeken_product_insurance_mandatory" 
					<?php
					if ( $product_insurance_mandatory === 'on' ) {
						echo 'checked';}
					?>
					/></td><td> Wajib Dikirim Menggunakan Asuransi
					</td></tr>
					<tr><td valign="top">
					...
					if ( $product_wood_pack_mandatory === 'on' ) {
						echo 'checked';}
					?>
					/></td><td> Wajib Dikirim Menggunakan Packing Kayu. Untuk mewajibkan packing kayu pada item ini, pastikan Anda sudah melakukan Enable Packing Kayu di WooCommerce > Shipping > Epeken Courier > Packing Kayu Settings.
					</td></tr>
					<tr><td valign="top">
					<input type="checkbox" name="epeken_product_free_ongkir" class="wcfm-checkbox" id="epeken_product_free_ongkir" 
					<?php
					if ( $product_free_ongkir === 'on' ) {
						echo 'checked';}
					?>
					/> </td><td>Gratiskan Ongkos Kirim Untuk Produk Ini
					</td></tr>
		</table>
		</div>
