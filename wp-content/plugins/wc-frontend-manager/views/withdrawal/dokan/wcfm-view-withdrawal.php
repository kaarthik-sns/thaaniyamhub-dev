		...
		if ( $status ) {
					...
					<div class="wcfm-container">
						<div id="wcfm_withdrawal_listing_expander" class="wcfm-content">
							...
							$WCFM->wcfm_fields->wcfm_generate_form_field(
								apply_filters(
									'wcfm_withdrawal_fields_dokan',
									array(
										'withdraw_amount' => array(
											'label'       => __( 'Withdraw Amount', 'dokan-lite' ),
											'type'        => 'number',
											'attributes'  => array( 'min' => $withdraw_limit ),
			...
			<?php } else { ?>
				<div class="dokan-alert dokan-alert-warning">
						<strong><?php printf( '%s <a href="%s">%s</a>', __( 'No withdraw method is available. Please update your payment method to withdraw funds.', 'dokan-lite' ), esc_url( get_wcfm_settings_url() ), __( 'Payment Settings Setup', 'dokan-lite' ) ); ?></strong>
				</div>
			<?php } ?> 
		<?php } ?> 
