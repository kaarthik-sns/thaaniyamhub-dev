			...
			<div class="wcfm-tabWrap">
				...
				<div class="wcfm-container">
					<div id="wcfm_settings_form_style_expander" class="wcfm-content">
						<?php
							$settings_fields_store = apply_filters(
								'wcfm_wcpvendors_settings_fields_store',
								array(
									'wcfm_logo'        => array(
									...
									'email'            => array(
										'label'       => __( 'Vendor Email', 'wc-frontend-manager' ),
										'type'        => 'text',
										'class'       => 'wcfm-text wcfm_ele',
										'label_class' => 'wcfm_title wcfm_ele',
										'value'       => $email,
										'hints'       => __( 'Enter the email for this vendor. This is the email where all notifications will be send such as new orders and customer inquiries. You may enter more than one email separating each with a comma.', 'wc-frontend-manager' ),
									),
									'shop_description' => array(
										'label'       => __( 'Profile', 'wc-frontend-manager' ),
										'type'        => $wpeditor,
										'class'       => 'wcfm-textarea wcfm_ele ' . $rich_editor,
										'label_class' => 'wcfm_title',
										'value'       => $profile,
										'hints'       => __( 'Enter the profile information you would like for customer to see.', 'wc-frontend-manager' ),
									),
								)
							);

							if ( ! apply_filters( 'wcfm_is_allow_store_logo', true ) ) {
								if ( isset( $settings_fields_store['wcfm_logo'] ) ) {
									unset( $settings_fields_store['wcfm_logo'] ); }
							}

							if ( ! apply_filters( 'wcfm_is_allow_store_name', true ) ) {
								if ( isset( $settings_fields_store['shop_name'] ) ) {
									unset( $settings_fields_store['shop_name'] ); }
							}

							if ( ! apply_filters( 'wcfm_is_allow_store_description', true ) ) {
								if ( isset( $settings_fields_store['shop_description'] ) ) {
									unset( $settings_fields_store['shop_description'] ); }
							}
				...
				<?php if ( $wcfm_is_allow_billing_settings = apply_filters( 'wcfm_is_allow_billing_settings', true ) ) { ?>
					...
					<div class="wcfm-container">
						<div id="wcfm_settings_form_payment_expander" class="wcfm-content">
							...
							$WCFM->wcfm_fields->wcfm_generate_form_field(
								apply_filters(
									'wcfm_wcpvendors_settings_fields_billing',
									array(
										'paypal' => array(
											...
											'label_class' => 'wcfm_title wcfm_ele',
											'value'       => $paypal,
											'hints'       => __( 'PayPal email account where you will receive your commission.', 'wc-frontend-manager' ),
										),
										'vendor_commission' => array(
											...
											'hints'       => __( 'Default commission you will receive per product sale. Please note product level commission can override this. Check your product to confirm.', 'wc-frontend-manager' ),
										),
									)
								)
							);
