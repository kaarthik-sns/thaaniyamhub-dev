...
$is_responsive_float_menu_disabled = 'yes';  
...
if ( ! $wp_user_avatar && apply_filters( 'wcfm_is_pref_buddypress', true ) && WCFM_Dependencies::wcfm_biddypress_plugin_active_check() ) {
	$wp_user_avatar = bp_core_fetch_avatar(
		array(
			'html'    => false,
			'item_id' => $user_id,
		)
...
<div class="wcfm_header_panel">
	...
	<a href="<?php echo get_wcfm_profile_url(); ?>" class="wcfm_header_panel_profile 
		<?php
		if ( isset( $wp->query_vars['wcfm-profile'] ) ) {
			echo 'active';}
		?>
	"><img class="wcfm_header_panel_profile_img  text_tip" src="<?php echo esc_url( $wp_user_avatar ); ?>" data-tip="<?php _e( 'Profile', 'wc-frontend-manager' ); ?>" /></a>
	<?php } ?>
	...
	<a href="<?php echo get_wcfm_messages_url(); ?>" class="wcfm_header_panel_messages text_tip 
		<?php
		if ( isset( $wp->query_vars['wcfm-messages'] ) ) {
			echo 'active';}
		?>
	" data-tip="<?php _e( 'Notification Board', 'wc-frontend-manager' ); ?>"><i class="wcfmfa fa-bell"></i><span class="unread_notification_count message_count"><?php echo esc_attr( $unread_message ); ?></span><div class="notification-ring"></div></a>
	<?php } ?>
  
	<?php if ( apply_filters( 'wcfm_is_pref_enquiry', true ) && apply_filters( 'wcfm_is_allow_enquiry', true ) ) { ?>
	<a href="<?php echo get_wcfm_enquiry_url(); ?>" class="wcfm_header_panel_enquiry text_tip 
		<?php
		if ( isset( $wp->query_vars['wcfm-enquiry'] ) || isset( $wp->query_vars['wcfm-enquiry-manage'] ) ) {
			echo 'active';}
		?>
	" data-tip="<?php _e( 'Inquiry Board', 'wc-frontend-manager' ); ?>"><i class="wcfmfa fa-question-circle"></i><span class="unread_notification_count enquiry_count"><?php echo esc_attr( $unread_enquiry ); ?></span><div class="notification-ring"></div></a>
	<?php } ?>
  
	<?php if ( apply_filters( 'wcfm_is_pref_notice', true ) && apply_filters( 'wcfm_is_allow_notice', true ) ) { ?>
	<a href="<?php echo get_wcfm_notices_url(); ?>" class="wcfm_header_panel_notice text_tip 
		<?php
		if ( isset( $wp->query_vars['wcfm-notices'] ) || isset( $wp->query_vars['wcfm-notice-manage'] ) || isset( $wp->query_vars['wcfm-notice-view'] ) ) {
			echo 'active';}
		?>
	" data-tip="<?php _e( 'Announcement', 'wc-frontend-manager' ); ?>"><i class="wcfmfa fa-bullhorn"></i>
		<?php
		if ( wcfm_is_vendor() ) {
			?>
	...
	<a href="<?php echo get_wcfm_knowledgebase_url(); ?>" class="wcfm_header_panel_knowledgebase text_tip 
		<?php
		if ( isset( $wp->query_vars['wcfm-knowledgebase'] ) || isset( $wp->query_vars['wcfm-knowledgebase-manage'] ) ) {
			echo 'active';}
		?>
	" data-tip="<?php _e( 'Knowledgebase', 'wc-frontend-manager' ); ?>"><i class="wcfmfa fa-book"></i></a>
	<?php } ?>
