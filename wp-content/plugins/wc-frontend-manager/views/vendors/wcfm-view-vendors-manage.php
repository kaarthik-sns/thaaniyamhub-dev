				...
				if ( $wcfm_is_products_vendor_filter = apply_filters( 'wcfm_is_products_vendor_filter', true ) ) {
					...
					if ( $is_marketplace ) {
						if ( ! wcfm_is_vendor() ) {
							$vendor_arr = array();
							if ( $vendor_id ) {
								$vendor_arr[ $vendor_id ] = strip_tags( $vendor_store );
							}
				...
				<div class="wcfm_dashboard_stats_block">
					<a href="<?php echo esc_url( $stat_box_link ); ?>">
						...
						<div>
							...
							<?php
							if ( $admin_fee_mode ) {
								_e( 'admin fees in this month', 'wc-frontend-manager' );
							} else {
								_e( 'earnings in this month', 'wc-frontend-manager' ); }
							?>
						</div>
					</a>
				</div>
				...
				<div id="vendors_manage_general_expander" class="wcfm-content">
					...
					if ( $vendor_id ) {
						$WCFM->wcfm_fields->wcfm_generate_form_field(
							array(
								'user_name' => array(
									'label'       => __( 'Store Admin', 'wc-frontend-manager' ),
									'type'        => 'text',
									'attributes'  => array( 'readonly' => true ),
									'class'       => 'wcfm-text wcfm_ele ',
									'label_class' => 'wcfm_ele wcfm_title',
									'value'       => $first_name . ' ' . $last_name . ' (#' . $vendor_id . ' - ' . $user_name . ')',
								),
							)
						);
					} else {
						...
						$WCFM->wcfm_fields->wcfm_generate_form_field(
							apply_filters(
								'wcfm_vendor_vendor_fields_general',
								array(
									'user_email'    => array(
									...
									'store_address' => array(
										'label'       => __( 'Address', 'wc-frontend-manager' ),
										'type'        => 'html',
						...
						} elseif ( apply_filters( 'wcfm_is_allow_disable_enable_vendor', true ) ) {
			...
			<div class="wcfm-container">
				<div id="wcfm_vendor_manage_form_profile_expander" class="wcfm-content">
					<form id="wcfm_vendor_manage_profile_form" class="wcfm">
						...
						$WCFM->wcfm_fields->wcfm_generate_form_field(
							apply_filters(
								'wcfm_profile_field_vendor_manage',
								array(
									'first_name' => array(
				...
				<?php if ( apply_filters( 'wcfm_is_allow_direct_message', true ) ) { ?>
					...
					<div class="wcfm-container">
						<div id="wcfm_vendor_manage_form_message_expander" class="wcfm-content">
							...
							$WCFM->wcfm_fields->wcfm_generate_form_field(
								apply_filters(
									'wcfm_messages_field_vendor_manage',
									array(
										'wcfm_messages' => array(
											'type'        => $wpeditor,
