...
<div class="collapse wcfm-collapse" id="">
	...
	<div class="wcfm-collapse-content">
		...
		<div class="wcfm-container wcfm-top-element-container">
			...
			<?php
			if ( $article_id ) {
				_e( 'Edit Article', 'wc-frontend-manager' );
			} else {
				_e( 'Add Article', 'wc-frontend-manager' ); }
			?>
			</h2>
			<?php
			if ( $article_id ) {
				?>
				<span class="article-status article-status-<?php echo $wcfm_articles_single->post_status; ?>">
				<?php
				if ( $wcfm_articles_single->post_status == 'publish' ) {
					_e( 'Published', 'wc-frontend-manager' );
				} else {
					_e( ucfirst( $wcfm_articles_single->post_status ), 'wc-frontend-manager' ); }
				?>
				</span>
				<?php
				if ( $wcfm_articles_single->post_status == 'publish' ) {
		...
		<form id="wcfm_articles_manage_form" class="wcfm">
				...
				<div id="wcfm_articles_manage_form_general_expander" class="wcfm-content">
					<div class="wcfm_article_manager_general_fields">
						<?php
							$WCFM->wcfm_fields->wcfm_generate_form_field(
								apply_filters(
									'wcfm_article_manage_fields_general',
									array(
										'title' => array(
											'placeholder' => __( 'Article Title', 'wc-frontend-manager' ),
											'type'        => 'text',
											'class'       => 'wcfm-text wcfm_ele wcfm_article_title wcfm_full_ele',
											'value'       => $title,
										),
									),
									$article_id
									...
									apply_filters(
										...
										array(
											'article_tags' => array(
												...
												'value' => $article_tags,
												'placeholder' => __( 'Separate Article Tags with commas', 'wc-frontend-manager' ),
										...
										foreach ( $article_taxonomies as $article_taxonomy ) {
											if ( ! in_array( $article_taxonomy->name, array( 'category', 'post_tag' ) ) ) {
												if ( $article_taxonomy->public && $article_taxonomy->show_ui && $article_taxonomy->meta_box_cb && ! $article_taxonomy->hierarchical ) {
													...
													$WCFM->wcfm_fields->wcfm_generate_form_field(
														array(
															$article_taxonomy->name => array(
																'label' => $article_taxonomy->label,
																'name' => 'article_custom_taxonomies_flat[' . $article_taxonomy->name . '][]',
																'type' => 'textarea',
																'class' => 'wcfm-textarea wcfm_ele wcfm_full_ele',
																'label_class' => 'wcfm_title wcfm_full_ele',
																'value' => $taxonomy_values,
																'placeholder' => __( 'Separate Article ' . $article_taxonomy->label . ' with commas', 'wc-frontend-manager' ),
															),
														)
													);
												}
						...
						<?php if ( $wcfm_is_category_checklist = apply_filters( 'wcfm_is_category_checklist', true ) ) { ?>
							...
							<div class="wcfm_article_manager_content_fields">
								...
								$WCFM->wcfm_fields->wcfm_generate_form_field(
									apply_filters(
										'wcfm_article_manage_fields_content',
										array(
											'excerpt'     => array(
						...
						if ( function_exists( 'wcfmmp_get_store_url' ) && ! wcfm_is_vendor() ) {
							$WCFM->wcfm_fields->wcfm_generate_form_field(
								array(
									'wcfm_vendor' => array(
										'label'       => apply_filters( 'wcfm_sold_by_label', $wcfm_vendor, __( 'Store', 'wc-frontend-manager' ) ),
										'type'        => 'select',
										'options'     => $vendor_arr,
					...
					<div class="wcfm_article_manager_gallery_fields">
						...
						if ( $wcfm_is_allow_featured = apply_filters( 'wcfm_is_allow_featured', true ) ) {
							$WCFM->wcfm_fields->wcfm_generate_form_field(
								apply_filters(
									'wcfm_article_manage_fields_gallery',
									array(
										'featured_img' => array(
											'type'        => 'upload',
											'class'       => 'wcfm-article-feature-upload wcfm_ele',
											'label_class' => 'wcfm_title',
											'prwidth'     => 250,
											'value'       => $featured_img,
										),
										...
										apply_filters(
											...
											array(
												'article_tags' => array(
													'label' => __( 'Tags', 'wc-frontend-manager' ),
													'type' => 'textarea',
													'class' => 'wcfm-textarea wcfm_ele wcfm_full_ele article_tags_ele',
			...
			<div class="wcfm-tabWrap">
				...
				<?php require 'wcfm-view-articles-manage-tabs.php'; ?>
