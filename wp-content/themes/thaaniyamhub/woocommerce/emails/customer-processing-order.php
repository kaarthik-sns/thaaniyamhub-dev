<?php
/**
 * Customer Processing Order Email — Thaaniyam Hub
 *
 * Override: yourtheme/woocommerce/emails/customer-processing-order.php
 *
...
$subheadline = 'Your order has been received and is now being processed.';
            ...
            <table width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;">
              <tr>
                ...
                <td width="33%" align="center" style="padding:18px 10px;vertical-align:top;">
                  ...
                  <?php if($has_refund):?><div style="font-size:11px;font-weight:700;color:#C0392B;font-family:Arial,Helvetica,sans-serif;margin-top:2px;">(Refund: -<?php echo wc_price($refunded_total);?>)</div><?php endif;?>
                </td>
              </tr>
            </table>
          </td>
            ...
            <table class="order-items-table" width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;width:100%;">
              ...
              <tbody>
                <?php foreach($order->get_items() as $item_id=>$item):
                  ...
                  $vendor_name='';
                  if($_product){
                    $product_id=$_product->get_id();$vendor_id=0;
                    if(method_exists($item,'get_meta')&&$item->get_meta('_vendor_id')){$vendor_id=(int)$item->get_meta('_vendor_id');}
                    if(!$vendor_id&&function_exists('wcfm_get_vendor_id_by_post')){$vendor_id=(int)wcfm_get_vendor_id_by_post($product_id);}
                    if(!$vendor_id){$vendor_id=(int)get_post_field('post_author',$product_id);}
                    if($vendor_id){
                      if(function_exists('wcfm_get_vendor_store_name')){$vendor_name=wcfm_get_vendor_store_name($vendor_id);}
                      if(!$vendor_name&&function_exists('thaaniyamhub_get_vendor_name_by_vendor_id')){$vendor_name=thaaniyamhub_get_vendor_name_by_vendor_id($vendor_id);}
                    }
                  }?>
                <tr>
