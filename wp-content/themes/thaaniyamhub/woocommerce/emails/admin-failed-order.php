<?php
/**
 * Override: yourtheme/woocommerce/emails/admin-failed-order.php
 * @version 10.4.0
 */
...
if ( function_exists( 'get_wcfm_view_order_url' ) ) { $admin_url = get_wcfm_view_order_url( $order->get_id() ); }
elseif ( method_exists( $order, 'get_edit_order_url' ) ) { $admin_url = $order->get_edit_order_url(); }
else { $admin_url = admin_url( 'post.php?post=' . $order->get_id() . '&action=edit' ); }
$badge_label = 'Failed Payment';
$headline    = 'Payment Failed: Order #' . $order_number;
$customer_parts = array();
if ( ! empty( $full_name ) ) {
    $customer_parts[] = esc_html( $full_name );
}
if ( ! empty( $customer_email ) ) {
    $customer_parts[] = '&lt;' . esc_html( $customer_email ) . '&gt;';
}
$subheadline = ! empty( $customer_parts ) ? 'Customer: ' . implode( ' ', $customer_parts ) : '';
...
?>
          ...
          <td style="background-color:#ffffff;border-left:1px solid #E0EBD8;border-right:1px solid #E0EBD8;padding:0;border-bottom:1px solid #E8F0E4;">
            <table width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;">
              <tr>
                ...
                <td width="33%" align="center" style="padding:18px 10px;vertical-align:top;"><div style="font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#8BA888;font-family:Arial,Helvetica,sans-serif;margin-bottom:5px;">Total</div><div style="font-size:16px;font-weight:700;color:#2D6E3E;font-family:Arial,Helvetica,sans-serif;"><?php echo wc_price($order_total_val);?></div></td>
              </tr>
            </table>
          </td>
        </tr>
        <tr>
          <td class="email-body-td" style="background-color:#ffffff;border-left:1px solid #E0EBD8;border-right:1px solid #E0EBD8;padding:28px 32px;">
            <p style="font-size:11px;letter-spacing:2.5px;text-transform:uppercase;color:#8BA888;font-family:Arial,Helvetica,sans-serif;margin:0 0 18px 0;text-align:center;">Order Items</p>
            <table class="order-items-table" width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;">
              ...
              <tbody>
                <?php foreach($order->get_items() as $item_id=>$item):
                  ...
                  $vid=0;$vendor_name='';
                  if($_product){if(method_exists($item,'get_meta')&&$item->get_meta('_vendor_id')){$vid=(int)$item->get_meta('_vendor_id');}if(!$vid&&function_exists('wcfm_get_vendor_id_by_post')){$vid=(int)wcfm_get_vendor_id_by_post($_product->get_id());}if(!$vid){$vid=(int)get_post_field('post_author',$_product->get_id());}if($vid){if(function_exists('wcfm_get_vendor_store_name')){$vendor_name=wcfm_get_vendor_store_name($vid);}if(!$vendor_name&&function_exists('thaaniyamhub_get_vendor_name_by_vendor_id')){$vendor_name=thaaniyamhub_get_vendor_name_by_vendor_id($vid);}}}?>
                <tr>
                  ...
                  <td valign="middle" style="padding:14px 8px;border-bottom:1px solid #F4F7F0;"><div style="font-size:14px;color:#1E3A24;font-family:Arial,Helvetica,sans-serif;font-weight:600;"><?php echo esc_html($item->get_name());?></div><?php if($sku):?><div style="font-size:11px;color:#8BA888;font-family:Arial,Helvetica,sans-serif;margin-top:3px;">SKU: <?php echo esc_html($sku);?></div><?php endif;?><?php if(!empty($vendor_name)):?><div style="font-size:11px;color:#2D6E3E;font-weight:700;font-family:Arial,Helvetica,sans-serif;margin-top:3px;">Vendor: <?php echo esc_html($vendor_name);?></div><?php endif;?></td>
            ...
            <table width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;margin-top:14px;">
              ...
              <tr><td style="padding:6px 0;color:#8BA888;font-size:13px;">Shipping</td><?php if($shipping_total>0):?><td align="right" style="padding:6px 0;color:#3A5A3A;font-size:13px;"><?php echo wc_price($shipping_total);?></td><?php else:?><td align="right" style="padding:6px 0;color:#6FCF97;font-size:13px;font-weight:700;">Free</td><?php endif;?></tr>
        ...
        <tr><td align="center" style="background-color:#ffffff;border-left:1px solid #E0EBD8;border-right:1px solid #E0EBD8;border-top:1px solid #F0F5EC;padding:24px 32px;"><table cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;margin:0 auto;"><tr><td align="center" bgcolor="#1E4D2B" style="background-color:#1E4D2B;border-radius:6px;padding:14px 36px;"><a href="<?php echo esc_url($admin_url);?>" style="font-family:Arial,Helvetica,sans-serif;font-size:13px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;color:#ffffff;text-decoration:none;">View Order in Admin &rarr;</a></td></tr></table></td></tr>
        <tr><td class="email-section-td" style="background-color:#ffffff;border-left:1px solid #E0EBD8;border-right:1px solid #E0EBD8;border-top:1px solid #F0F5EC;padding:24px 32px;"><div style="font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#8BA888;font-family:Arial,Helvetica,sans-serif;margin-bottom:6px;">Customer</div><div style="font-size:13px;color:#3A5A3A;font-family:Arial,Helvetica,sans-serif;line-height:1.8;"><?php echo esc_html($full_name);?><br/><a href="mailto:<?php echo esc_attr($billing_email);?>" style="color:#2D6E3E;"><?php echo esc_html($billing_email);?></a><?php if($billing_phone):?><br/><?php echo esc_html($billing_phone);?><?php endif;?></div></td></tr>
